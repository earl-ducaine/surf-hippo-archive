	...
	(bbox-to-clip-mask-components
	 agg-bbox (g-value gob :left) (g-value gob :top) (g-value gob :width) (g-value gob :height)))
      (let ((default-graphics-color (g-value a-window :default-graphics-color))
	    (color-shading (g-value a-window :segment-color-shading))
	  ...
	  (when (and (bbox-valid-p item-bbox)
	    ...
	    (let* ((item-values (the simple-array (aref (the simple-array item-array) n)))
		   ...
		   (lstyle
			...
			(or (aref item-values 4)
			    (sh::access-*line-styles-array*-for-segments-fast
			     thickness default-graphics-color color-shading
			     ;; Can't use segment-index macro here since it is  not defined yet
			     (typecase seg
			       (sh::segment (sh::node-color-index (sh::segment-node-2 seg)))
			       (t nil))))))
		   (x1 (the fixnum (aref item-values 0)))
