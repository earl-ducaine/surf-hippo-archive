...
(channel-type-def
 '(na-wdy-fit
   ...
   (e-rev . 65.0)
   (ion-permeabilities . ((NA 1.0)))
   (v-particles . ((nam-wdy-fit 3) (nah-wdy-fit 1)))))
...
(channel-type-def
 '(ca-wdy-fit 
   ...
   (CONC-INT-TYPE-E-REV-PARAMS . ((ca-in1-wdy (1 1))))
   (v-particles . ((cas-wdy-fit 2) (car-wdy-fit 1)))))
...
(defun kctc-wdy-ss (concentration voltage type &optional return-rate)
  (declare (optimize (speed 3) (space 0)) )
  (let ((conc-arg (conc-particle-concentration-arg concentration type)))
    (declare (double-float conc-arg voltage))
...
(defun kctc-wdy-ss (concentration voltage type &optional return-rate)
  ...
  (let ((conc-arg (conc-particle-concentration-arg concentration type)))
    (declare (double-float conc-arg voltage))
    ;; CONC-ARG is in mM, while paper uses uM
    (let* ((v-shift (- (* 40 (the df (DF-REAL-LOG (the df (* conc-arg 1000))
						  10.0d0))) 105))
...
(conc-particle-type-def
 `(kctc-wdy
   (class . wdy)
   (ss-function . ,#'kctc-wdy-ss)
   (tau-function . ,#'(lambda (concentration voltage type)
			(declare (ignore voltage concentration type))
			1.1d0))
...
(defun kahpq-wdy-ss (concentration voltage type)
  (declare (optimize (speed 3) (space 0)) 
	   (ignore voltage))
    (let* ((conc-arg (conc-particle-concentration-arg concentration type))
	 (log-conc (* 10 (the df (DF-REAL-LOG (* conc-arg 1000) 10.0d0)))))
    (declare (double-float conc-arg log-conc))
...
(channel-type-def
 '(ka-wdy-fit 
   ...
   (e-rev . -80.0)
   (ion-permeabilities . ((k 1.0)))
   (v-particles . ((kaa-wdy-fit 1)(kab-wdy-fit 1)))))
