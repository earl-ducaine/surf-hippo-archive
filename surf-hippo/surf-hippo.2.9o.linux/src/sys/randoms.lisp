...
(defun modulated-poisson-events (LAMBDA-SPEC start stop &key (step 1.0) (time-offset 0.0) (min-interval-value 0.0) (lambda-coefficient 1.0))
  "Returns a list of event times generated by a Poisson process with a modulated rate constant lambda
over a total interval given by START and STOP [ms]. Calculation of lambda at any given time depends
on LAMBDA-SPEC. When LAMBDA-SPEC is a function, lambda is given by a funcall of LAMBDA-SPEC with the
time as the argument \(this function should take a single numeric argument and return a
single-float\). When LAMBDA-SPEC is a sequence lambda is the interpolated value of LAMBDA-SPEC
appropriate for the current time. Finally, if LAMBDA-SPEC is a number this gives directly the
constant value of lambda. In all cases, the evaluation of LAMBDA-SPEC is taken to be in 1/ms.  When
LAMBDA-SPEC is a function, STEP [ms] should be a value for which the function is relatively
constant. When LAMBDA-SPEC is a sequence, then STEP is the time base. If LAMBDA-SPEC is a number,
then the list of events is generated by the function POISSON-EVENTS, with the START and STOP
arguments adjusted by the value of TIME-OFFSET [ms, default 0.0]. Otherwise, the events are
generated by repeated calls to POISSON-INTERVAL, with the initial time given by START, and until
time reaches STOP. At every time increment if POISSON-INTERVAL gives a value greater than STEP, then
the time is incremented by STEP and the process repeated. If the returned poisson interval is less
than STEP, then the time is incremented by the interval, and this value is pushed onto the result
list. All times in the returned list are adjusted by the addition of TIME-OFFSET [ms].  Poisson
intervals are taken at a minimum value of MIN-INTERVAL-VALUE [ms]. All numeric arguments must be
single floats."
;  (declare (optimize (safety 0) (speed 3) (space 0) (compilation-speed 0)))
  (let ((start (s-flt start))
    ...
    (typecase LAMBDA-SPEC
      ((or cons array)
       (setq LAMBDA-SPEC (sequence-to-float-array LAMBDA-SPEC))
...
(defun shuffled-indices (length)
  ...
  (let* ((length (round length))
	 ...
	 choice-index)
    (loop for count fixnum from 1 to (the fn length)
	  for indice-length fixnum downfrom (length indices)
	  do (setq choice-index
		   (if (= 1 indice-length) 0 (round (the sf (random (float (1- indice-length)))))))
