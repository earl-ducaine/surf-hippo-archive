...
;; CONVOLVE-INPUT-ARRAY INPUT-ARRAY may be a one dimensional or two dimensional single-float array -
;; the latter case requires a fixnum :2D-INPUT-INDEX argument to specify the 1d slice (first array
;; dimension) of the input that will be used for the convolution. This function returns an array
;; whose length is equal to the length of a 1d INPUT-ARRAY or the length of the second array
;; dimension of a 2d INPUT-ARRAY. IMPULSE is a one-dimensional single-float array. A nonlinearity
;; may be applied to the result of the convolution - the meaning of the :NONLINEARITY and
;; :NONLINEARITY-PARAMETERS arguments correspond to the :NONLINEARITY and :PARAMETER arguments of
;; the function NONLINEARITY defined above.
;;
...
(defun boxcar (wave boxcar-length)
  (let ((output))
	...
	(push (/ (loop for val in wave
		       for count from 1 to boxcar-length
		       sum val)
		 boxcar-length)
	      output))))
...
(defun boxcar-sf (wave boxcar-length)
  ...
  (let ((output))
	...
	(push (/ (loop for val single-float in wave
		       for count fixnum from 1 to (the fn boxcar-length)
...
(defun 1-pole (original-data original-time cutoff-freq sample-freq &optional offset-w-first-sample)
  (let* (
	 ...
	 (data-time (convert-data-time-lists original-data original-time dt))
...
(defun convolver (list1 list2)
  (let* ((array-1 (list-to-array list1))
    ...
    (array-to-list (convolve-input-array array-2 array-1 :threshold nil))))

(defun convolver (list1 list2)
  (declare (optimize (safety 0) (speed 3) (space 0)))
  (let* ((array-1 (the 1d-flt (list-to-array list1)))
	 (array-2 (the 1d-flt (list-to-array list2)))
...
(defun convolve (impulse input &key threshold dc-bias output-length resting-value
			 return-list 
  ...
  (let* ((sum 0.0)
	 ...
	 (input-array (sequence-to-float-array input))
	 (input-length (the fn (length (the 1d-flt input-array))))
	 (impulse-array (sequence-to-float-array impulse))
