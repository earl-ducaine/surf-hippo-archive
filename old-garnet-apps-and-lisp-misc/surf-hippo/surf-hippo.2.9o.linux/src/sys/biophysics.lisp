...
(defun effective-default-extracellular-concentration-from-ion-perms (ion-perms )
  (declare (optimize (safety 1) (speed 3) (space 0)))
  (loop for ion-perm in ion-perms
	sum (* (the sf (default-extracellular-concentration (car ion-perm)))
	       (the sf (cadr ion-perm)))
	into result single-float
	finally (return result)))
...
(defun boltzman-characteristic (k v-half)
  (loop for voltage from -100.0 to 50
	collect (/ 1 (1+ (exp (/ (- v-half voltage) k))))))
...
(defun constant-field-equation (voltage conc-in conc-out permeability valence
					&optional (gating-term 1.0))
  "VOLTAGE is in mV, CONC-IN and CONC-OUT are in mM, PERMEABILITY in cm3/s, GATING-TERM and VALENCE
are dimensionless. Returns current in nA. All arguments and returned value are single-floats. This
form is that used by Mcc-Hug-92."
  (declare (optimize (safety 1) (speed 3) (space 0))
  ...
  (let* ((voltage-in-volts (* 1.0e-3 voltage)) ; Convert mV to V
    ...
    (if (= 0.0 voltage) (/ a-conc-numerator (* valence *F/RT*)) (/ (* a-conc-numerator voltage-in-volts) (- 1 exp-term)))))
...
(defun constant-field-equation-double (voltage conc-in conc-out permeability valence gating-term)
  "Double-float version of CONSTANT-FIELD-EQUATION (except for VALENCE, which is a single-float).
Returns current in nA (double-float)."
  (declare (optimize (safety 1) (speed 3) (space 0))
...
(defmacro fast-constant-field-equation (voltage conc-in conc-out zsqd-ff/rt zf/rt gating-term)
  `(let* ((voltage-in-volts (* ,voltage 1.0e-3))
     ...
     (if (= 0.0 voltage-in-volts) (/ a-conc-numerator ,zf/rt) (/ (* voltage-in-volts a-conc-numerator) (- 1 exp-term)))))
...
;; CONSTANT-FIELD-CURRENT-COEFFICIENT For use in channel or synapse current evaluations, where the
;; gating term and permeability are coefficients of this term. Returns a double float.
(proclaim '(inline constant-field-current-coefficient))
(defun constant-field-current-coefficient (elt &key valence voltage voltage-double
					       conc-in conc-in-double conc-out conc-out-double)
  (declare (optimize (safety 1) (speed 3) (space 0)))
  (let* ((voltage (or voltage-double
		      (and voltage (coerce-to-double voltage))
		      (element-voltage elt)))
	 ...
	 (conc-in (the df (or conc-in-double
			      (and conc-in (coerce (the sf conc-in) 'double-float))
	 ...
	 (conc-out (the df (or conc-out-double
			       (and conc-out (coerce (the sf conc-out) 'double-float))
			       (and cint (if (conc-int-type-intra-p cint-type)
					   (conc-int-transmembrane-conc cint)
					   (conc-int-shell-1-free-conc-n cint)))
...
(defun temperature-kelvin-to-centigrade (new-temp-k)
  (s-flt (- new-temp-k 273.16)))

(defun temperature-centigrade-to-kelvin (new-temp-c)
  (s-flt (+ new-temp-c 273.16)))
...
;; SCALED-SIGMOID-RATE Adapted from Equation 33 of Des-Man-Sej-94.
(defun scaled-sigmoid-rate (voltage &key (scale 1.0) (steepness 1.0) (v-half 0.0)
				    (base-rate 0.0))
  "VOLTAGE, STEEPNESS and V-HALF are in mV. BASE-RATE and SCALE are in 1/ms. The minimum/maximum
values are given by BASE-RATE and (BASE-RATE + SCALE), respectively. Note that the slope for VOLTAGE
= V-HALF is [(0.25 * SCALE) / STEEPNESS]."
...
(defun scaled-exponential-rate (voltage &key (scale 1.0) (steepness 1.0) (v-half 0.0)
					(base-rate 0.0) (max-rate -1.0))
  "VOLTAGE, STEEPNESS and V-HALF are in mV. BASE-RATE, SCALE, MAX-RATE are in 1/ms. The
minimum/maximum values are given by BASE-RATE and (BASE-RATE + SCALE), respectively. Note that the
slope for VOLTAGE = V-HALF is SCALE / STEEPNESS."
...
(defun scaled-exponential-soft-rate (voltage &key (scale 1.0) (steepness 1.0) (v-half 0.0)
					     (base-rate 0.0) max-rate)
  "VOLTAGE, STEEPNESS and V-HALF are in mV.  BASE-RATE, SCALE, MAX-RATE are in 1/ms. The
minimum/maximum values are given by BASE-RATE and (BASE-RATE + SCALE), respectively. Note that the
slope for VOLTAGE = V-HALF is SCALE / STEEPNESS. If MAX-RATE is nil, then it is ignored."
  (declare (optimize (speed 3) (space 0) (safety 1))
	   (single-float voltage scale steepness v-half base-rate))
...
(defun squeezed-exponential (voltage &key (v-half 0.0)
				     (k 1.0)
				     (tau-max -1.0)
				     (tau-min 0.0))
  "Exponential rate function with minimum and maximum rates given by the reciprocal of TAU-MAX
[default -1.0] and TAU-MIN [default 0.0], respectively. When TAU-MAX is NIL or negative, then the
minimum rate is 0.0. TAU-MAX cannot be 0. V-HALF [default 0.0] and the steepness K [default 1.0] are
assumed to be in the same units as VOLTAGE, typically in mV. Returns single float value."
  (when (and (numberp tau-max) (= tau-max 0))
    (sim-error "In SQUEEZED-EXPONENTIAL TAU-MAX cannot be 0."))
  (let ((value (+ (if (and (numberp tau-max) (> tau-max 0))
		      (/ 1 (+ tau-max tau-min)) 0)
		  (exp-w-limits (/ (- voltage v-half) k)))))
    (s-flt (if (and (= value 0) (= tau-min 0)) 0.0 (/ 1.0 (+ tau-min (/ 1 value)))))))



(defun v-half-prime (vhalf scale steepness)
  (- vhalf (* steepness (log scale))))

(defun k-prime (scale v-half steepness)
  (* scale (exp (/ (- v-half) steepness))))
...
(defun scaled-exponential-rate-double (voltage &key (scale 1.0d0) (steepness 1.0d0) (v-half 0.0d0)
					(base-rate 0.0d0) (max-rate -1.0d0))
  "VOLTAGE, STEEPNESS and V-HALF are in mV. BASE-RATE, SCALE, MAX-RATE are in 1/ms. The
minimum/maximum values are given by BASE-RATE and (BASE-RATE + SCALE), respectively. Note that the
slope for VOLTAGE = V-HALF is SCALE / STEEPNESS."
  (declare (optimize (speed 3) (space 0))
...
(defun scaled-slanted-step-rate (voltage &key (scale 1.0) (steepness 1.0) (v-half 0.0)
					 (base-rate 0.0))
  "VOLTAGE, STEEPNESS and V-HALF are in mV. BASE-RATE and SCALE are in 1/ms. The minimum/maximum
values are given by BASE-RATE and (BASE-RATE + SCALE), respectively. Note that the slope at VOLTAGE
= V-HALF is [SCALE / STEEPNESS]." 
  (declare (optimize (speed 3) (space 0))
	   (single-float voltage scale steepness v-half base-rate))
  (+ base-rate
     (if (= 0 scale) 0.0
...
(defun constant-field-equation-exponential-term-double-lookup (voltage-index valence)
  ...
  (aref (the (simple-array double-float (* *)) *constant-field-equation-exponential-term-array*)
	valence voltage-index))
