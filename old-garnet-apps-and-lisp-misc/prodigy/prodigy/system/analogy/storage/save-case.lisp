...
(defvar *store-state-p* nil)
(defvar *max-class-elts* 100)
...
(defun assign-store-vars (ofile objects)
  ...
  (cond
    ...
    ((eq (caar objects) 'object-is)
     (assign-store-vars ofile (append (list (cdar objects)) (cdr objects))))
    (t
     (let* ((elt-class (car (last (car objects))))
	    (objs (butlast (car objects)))
	    (short-name (or (cdr (assoc elt-class *class-short-names*))
			    elt-class)))
       (format ofile "~%   ")
       (dolist (obj objs)
	 (let* ((var-number (get-var-number elt-class))
		(var-name
		 ;;Added conditional and second clause. [cox]
		 (if *class-short-names*
...
(defun get-var-number (elt-class)
  (let ((numb (random *max-class-elts*)))
    (if (member numb (cdr (assoc elt-class *vars-not-to-use*)))
	(setf numb (get-var-number elt-class))
        (let ((class-entry (assoc elt-class *vars-not-to-use*)))
	  (if class-entry
	      (rplacd class-entry (push numb (cdr class-entry)))
	      (push (cons elt-class (list numb)) *vars-not-to-use*))))
    numb))
...
(defun write-to-file  (prodigy-result objects case-name case-dir failed)
  (write-case-header-to-file
   prodigy-result objects case-name (concatenate 'string case-dir "headers/") failed)
  (unless failed 
    (let ((filename (concatenate 'string case-dir case-name ".lisp"))
	  (solution-path
	   (p4::path-from-root
...
(defun write-case-header-to-file (prodigy-result objects case-name case-dir failed)
  (let ((filename (concatenate 'string case-dir case-name ".lisp")))
    (with-open-file (ofile filename :direction :output
			   :if-exists :rename-and-delete
			   :if-does-not-exist :create)
      ...
      (format ofile "))~%")
      (format ofile "~%(setf problem-solved ~%   ~S)"
...
(defun list-down (parent-node node-list ofile)
	...
	(when (p4::protection-node-p child-node)
	  (setf child-node (cadr node-list))
...
(defun store-goal-node (parent-node node ofile)
    ...
    (when (not (null (p4::literal-neg-goal-p (p4::goal-node-goal node))))
      ...
      (format ofile "~%         (list (p4::a-or-b-node-instantiated-op (find-node ~S)))"
	      (p4::nexus-name (car (p4::goal-node-introducing-operators node))))
      (format ofile ")~%"))
)
