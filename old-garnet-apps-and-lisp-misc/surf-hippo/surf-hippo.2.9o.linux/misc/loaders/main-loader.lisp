...
(defun touch-file (filename)
  (when (probe-file filename)
    ;; (unix::run-program "touch" (list filename))
    (format t "Trying to 'touch' file: ~s~%" filename)
    (asdf:run-shell-command "touch" filename)))
...
(defun update-main-system-variables ()
  (setq *SURF-HOME*
	...
	(let ((surfhome (uiop/os:getenv "SURFHOME"))
	      ;; (home (cdr (assoc :HOME lisp::*environment-list*)))
	      (home (uiop/os:getenv "HOME")))
	  (if surfhome
  ...
  (setq *printer* (uiop/os:getenv "PRINTER"))
...
(unless (find :garnet *features*)
  (load (concatenate 'string (uiop/os:getenv "HOME") "/garnet-loader" )))

(load (concatenate 'string *surf-home* "/misc/loaders/surf-hippo-loader"))

;; All bin files should now be moved after compile at the end of the
;; COMPILE-SOURCE-DIRECTORY call for each subssytem.
;; (when surf-hippo-compile
;;   (unix::run-program (concatenate 'string *surf-home*
;; 				  (cond
