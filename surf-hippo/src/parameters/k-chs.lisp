...
(channel-type-def '(KTSLOW-ficker-92
        (gbar-source . absolute)
        (gbar-density . 500.0)
        (e-rev . -80.0)
        (ion-permeabilities . ((K 1.0)))
...
(channel-type-def '(KTSLOW-ficker-92-act-power2
	(parent-type . KTSLOW-ficker-92) 
        (v-particles . ((KTSLOWX-ficker-92-power-2 2)
			(KTSLOWY-ficker-92 1)))))

(channel-type-def '(KTSLOW-ficker-92-act-power3
	(parent-type . KTSLOW-ficker-92) 
        (v-particles . ((KTSLOWX-ficker-92-power-3 3)
			(KTSLOWY-ficker-92 1)))))

(particle-type-def '(KTSLOWX-ficker-92
	(class . :hh-ext)
...
(particle-type-def '(KTSLOWX-ficker-92-power-2
	(class . :hh-ext)
	;; Fit from fig. 3, taking into account current to conductance conversion, with e-rev = -80,
	;; exponent 2
	(v-half . -17.0)
...
(particle-type-def '(KTSLOWX-ficker-92-power-3
	(class . :hh-ext)
	;; Fit from fig. 3, taking into account current to conductance conversion, with e-rev = -80,
	;; exponent 3
	(v-half . -24.7)
...
(particle-type-def `(KTSLOWy-ficker-92
	(class . :hh-ext)
