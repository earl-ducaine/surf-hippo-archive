...
(defun MAKE-EVENTS-RENEWAL (array syn-type)
  (GAMMA-ARRIVAL-TIMES array *user-start-time* *user-stop-time* 1.0
		       (get-element-parameter syn-type 'thinning-factor)))
...
(defun MAKE-EVENTS-SWITCHING (array syn-type &optional (start *user-start-time*))
  ...
  (let ((thinnings (get-element-parameter syn-type 'thinning-step-acons)))
...
(defun PSTH-DISTRIB (&key (nb 100) (bin 1.0) (bin-skip 19)
  ...
  (let* ((values
    ...
    (let ((win-psth (PLOT-PSTH values
      ...
      (s-value win-psth :x-label-horizontal-position :center)
