...
(defmacro markov-rate-array-value-with-q10 (prt)
  `(if *ignore-q10*
    ...
    (the df (* (aref (the vec-df *markov-rate-array*) 0)
	     (particle-type-q10-rate-factor (particle-type ,prt))))))

(defmacro markov-rate-value-with-q10 (prt value)
  `(if *ignore-q10*
    ,value
    (the df (* ,value (particle-type-q10-rate-factor (particle-type ,prt))))))
	      

(defmacro set-markov-rate-array-q10-factor (value)
  `(setf (aref (the vec-df *markov-rate-array*) 1) ,value))
...
;; For converting lists referencing state symbols and transition functions into a list that
;; references state indices.


;; This is not needed anymore...  For backward compatibility....
(defmacro parse-markov-STATE-TRANSITION-FUNCTIONS (from-to-rate-coeff-list &optional states)
  from-to-rate-coeff-list)
...
(defmacro markov-state-trans-list-from-state (state-transition-list)
  `(nth 0 ,state-transition-list))
(defmacro markov-state-trans-list-to-state (state-transition-list)
  `(nth 1 ,state-transition-list))
(defmacro markov-state-trans-list-rate-def (state-transition-list)
  `(nth 2 ,state-transition-list))
(defmacro markov-state-trans-list-part-arg-p (state-transition-list)
  `(nth 3 ,state-transition-list))
(defmacro markov-state-trans-list-rate-def-part-arg-p (state-transition-list)
  `(cddr ,state-transition-list))



(defmacro return-markov-rate (val)
  "This macro wraps around the code for any state transition function with the particle as its arg,
where VAL is the double-float result of the function. The purpose of this macro is to pass the
double-float rate to the markov particle evaluation code via the global *MARKOV-RATE-ARRAY*."
  `(progn (setf (aref (the vec-df *markov-rate-array*) 0) ,val)
    nil))
...
(defun set-markov-particle-type-parameters (type original-parameters)
  (let* ((states (get-a-value 'states original-parameters))
    ...
    ;; The 'OPEN-STATES entry in the particle type parameter library entry is a list of the open
    ;; state symbols.

    (setf (particle-type-open-state-array type)
	  (list-to-array-generic 
	   (loop for state in states collect (true-p (member state (get-a-value 'open-states original-parameters))))))
    ...
    (element-parameter type 'markov-state-labels
			   (loop for state in states collect (intern (format nil "STATE-~A" state))))
  ...
  (let* ((particle-type-state-transition-array (particle-type-state-transition-array type))
    ...
    (element-parameter type 'non-zero-transitions
			   (loop for from-index from 0 to (1- index-limit)
				 nconc
				 (loop for to-index from 0 to (1- index-limit)
  ...
  (let ((trans-array (the (simple-array * (* *)) (particle-type-state-transition-array type)))
    ...
    (loop for from-index fixnum from 0 to (1- (array-dimension trans-array 0)) do
	  (loop for to-index fixnum from 0 to (1- (array-dimension trans-array 1)) 
		when (arrayp (aref trans-array from-index to-index))
		do (setf (aref trans-array from-index to-index) nil)))
	  ...
	  (let ((from-state-position (position (markov-state-trans-list-from-state state-transition-list) states))
	    ...
	    (setf (aref trans-array from-state-position to-state-position)
		  (let ((rate-def (markov-state-trans-list-rate-def state-transition-list)))
		    (if (markov-state-trans-list-part-arg-p state-transition-list)
			(extract-function-from-atom rate-def)
  ...
  (let ((open-states (particle-type-open-state-array (particle-type prt))))
    (setf (particle-state-n+1-double prt)
	  (loop for state-index fixnum from 0 for state in (particle-state-arrays prt)
		when (aref (the (simple-array boolean (*)) open-states) state-index)
...
(defun solve-markov-explicit (flow-arrays prt)
  ...
  (loop for to-state-index fixnum from 0 to (nb-states-1 prt) do
	(let* ((to-state (prt-state prt to-state-index))
	  ...
	  (when *debug-markov-particle*
	    (format t " Setting State-n+1 ~d to ~,6f, with dsdt ~,6f, time-step ~,6f~%"
		    to-state-index (particle-aref-state-n+1 to-state) dsdt (*markov-time-step*)))))
  nil)
...
(defun setup-a-and-k-for-ss (a-array k-vector nb-states-1)
  (declare (optimize (speed 3) (space 0)
	   ...
	   (type (simple-array double-float) k-vector))
...
;; SOLVE-MARKOV-MATRIX Implicit method. For initial state, normally we solve the system of nullspace
;; equations with the last one substituted with the conservation condition (sum of all states = 1).
;; The setup for this situation is accompished by SETUP-A-AND-K-FOR-SS, called by SETUP-A-AND-K. On
;; the other hand, if the particle or its type has (element-parameter prt 'initial-state), then this
...
;; For N states, FLOW-ARRAYS is a list of N double-float arrays of length N. MAX-RATE is used only
;; for explicit method.
...
(defun debug-markov-implicit-solve (prt rate-array a k)
  ...
  (loop for from-state-index fixnum from 0 to (nb-states-1 prt) do
	(loop for from-state-index-temp fixnum from 0 to (nb-states-1 prt) 
	      unless (= 0 (aref rate-array from-state-index-temp from-state-index)) do
	      (format t " rate ~A->~A: ~,4e~%"
		      from-state-index-temp from-state-index (aref rate-array from-state-index-temp from-state-index))))
  (format t "  A matrix:~%")
...
(defun v-inf-markov-particle-plot-lists (prt-type &optional (concentration *v-inf-markov-particle-plot-concentration*))
  (let* ((prt-type (element-type prt-type))
	 ...
	 (nb-voltages (round (/ (- 100 *particle-look-up-table-min-voltage*) *particle-look-up-table-precision*)))
    ...
    (element-parameter (particle-concentration-particle dummy-particle) 'concentration (d-flt concentration))
    (setq volts
	  (loop for voltage from (d-flt *particle-look-up-table-min-voltage*)
		by 0.10d0 ; *particle-look-up-table-precision*
		to 50
		for count from 0
		...
		(loop for state-index fixnum from 0 to (1- nb-states) do
		      (setf (aref inf-array state-index count)
			    (particle-aref-state-n+1 (prt-state dummy-particle state-index))))
    ...
    (loop for state-index fixnum from 0 to (1- nb-states)
	  collect
	  (list volts
		(loop for count from 0 to (1- nb-voltages) collect (aref inf-array state-index count))))))


(defun plot-markov-conc-dep-particle-type (prt-type
					   &key concentration ; mM
					   (new-plot nil) overlay channel-type (what '(:alpha_&_beta :steady_state)))
  (let ((*overlay-all-plots* t)
	(low -5)
	(high 0)
	(prt-type (element-type prt-type)))
    (loop for key in what unless (eq key :tau) do
	  (if (numberp concentration)
	      (let ((*v-inf-markov-particle-plot-concentration* (d-flt concentration)))
		(plot-v-particles prt-type :what key :overlay overlay :channel-type channel-type :new-plot new-plot))
	      (loop for exponent from low to high do
		    (let ((*v-inf-markov-particle-plot-concentration* (expt 10.0d0 exponent)))
		      (plot-v-particles prt-type :what key :overlay overlay :channel-type channel-type :new-plot new-plot))))
	  (add-comment *twin* (format nil "Concentration: 10.0e~D - 10.0e~DmM" low high)))))


(proclaim '(notinline eval-markov-particle-type))
(defun eval-markov-particle-type (prt-type initial-state
					   &optional (markov-initial-dt 0.001d0) eval-only-this-particle)
  (declare (optimize
	    (safety 0)
	    (speed 3) (space 0)
	    (compilation-speed 0)))
		...
		(let ((transition (aref transition-array from-state-index to-state-index)))
		  (when transition
		    (format t "From State ~A to State ~A: ~,2e~%"
			    ...
			    to-state-index
			    (typecase transition
			      (array (aref transition (voltage-to-voltage-index voltage)))
			      (t 0.0)))))))))
      ...
      (choose-list-values-from-keys
       (loop for to-state-index fixnum from 0
	     ...
	     (loop for from-state-index fixnum from 0 to (1- number-of-states)
		   ...
		   when (arrayp (aref transition-array from-state-index to-state-index))
		   collect (list (format nil "~A to ~A" from-label to-label)
				 (list (format nil "~A to ~A" from-label to-label) from-state-index to-state-index))))
      ...
      (choose-list-values-from-keys
       (loop for to-state-index fixnum from 0
	     ...
	     (loop for from-state-index fixnum from 0 to (1- number-of-states)
		   for from-label in (ELEMENT-PARAMETER type 'markov-state-labels)
		   when ; (arrayp (aref transition-array from-state-index to-state-index))
		   (aref transition-array from-state-index to-state-index)
		   collect (list (format nil "~A to ~A" from-label to-label)
				 (list from-state-index to-state-index))))
       nil :label (format nil "Choose State Transitions for ~A" (particle-type-name type))))))

(defun plot-markov-particle-type-rates (type &key concentration ; mM
					     channel-type
					     from-idx-to-idxs
					     (prompt-for-overlay t) separate-plots use-menu)
  (when (element type 'particle-type)
    (let* ((type (element type 'particle-type))
	   ...
	   rates labels)
      (element-parameter (particle-concentration-particle dummy-prt) 'concentration
			 (d-flt *v-inf-markov-particle-plot-concentration*))
      (loop for to-state-index fixnum from 0
	    for to-label in (ELEMENT-PARAMETER type 'markov-state-labels) do
	    (loop for from-state-index fixnum from 0 to (1- number-of-states)
		  for from-label in (ELEMENT-PARAMETER type 'markov-state-labels)
		  when (or (not use-menu)
			   (member (list from-state-index to-state-index) from-idx-to-idxs :test 'equal))
		  do
		  (let ((transition (aref transition-array from-state-index to-state-index)))
		    (when transition
		      ...
		      (push
		       (typecase transition
			 (array transition)
			 (number (list-of-nums (length voltages)
					       (markov-rate-value-with-q10 dummy-prt transition)
					       0.0))
			 (function
			  (loop for voltage in voltages do
	...
	(if separate-plots
	    (loop for label in labels for rate in rates do
		  (plot-timed-data rate (list label) voltages
				   :title (if channel-type
					      ...
					      (format nil "~a Particle Type: ~A Markov Rates"
						      (element-name type) label))
				   :width *particle-kinetics-plot-width* :height *particle-kinetics-plot-height*
				   :x-are-fns t :y-label-vertical-position :upper-right :prompt-for-overlay t
				   :x-min -100 :x-max 40 :x-origin -100 :y-min 0.0 :y-label "1/ms" :x-label "mV"))
	    (plot-timed-data rates labels voltages
			     ...
			     :width *particle-kinetics-plot-width* :height *particle-kinetics-plot-height*
			     :x-are-fns t :y-label-vertical-position :upper-right :prompt-for-overlay prompt-for-overlay
			     :overlay (not prompt-for-overlay) :ACCOMODATE-all-overlays (not prompt-for-overlay)
			     :x-min -100 :x-max 40 :x-origin -100 :y-min 0.0 :y-label "1/ms" :x-label "mV")))
...
(defun plot-markov-particle-type-steady-state (prt-type &key channel-type concentration (prompt-for-overlay t))
  (let ((v-inf-particle-plot-lists (v-inf-markov-particle-plot-lists prt-type concentration)))
    (plot-timed-data
     ...
     :overlay (not prompt-for-overlay) :ACCOMODATE-all-overlays (not prompt-for-overlay)
     :title (if channel-type
		(format nil "~a Channel Particle ~A: Markov Steady State"
			(element-name channel-type) (element-name prt-type))
     ...
     :x-origin  *particle-look-up-table-min-voltage* :x-inc 20.0 :x-origin-tick t :x-label "mV"
      ...
      (loop for label in labels for rate in rates do
	    (let* ((forward-rates (array-to-list (car rate)))
	      ...
	      (cond (separate-plots
		     (plot-timed-data ss (list label) voltages
				      :width *particle-kinetics-plot-width* :height *particle-kinetics-plot-height*
				      :x-are-fns t
				      :y-label "SS" :x-label "mV" :x-min -100 :x-max 40 :x-origin -100 :y-min 0.0 :y-max 1.0
				      :title (format nil "~A - ~A SS" (particle-type-name type) label))
		     (plot-timed-data taus (list label) voltages
				      :width *particle-kinetics-plot-width* :height *particle-kinetics-plot-height*
				      :x-are-fns t
				      :y-label "ms" :x-label "mV" :x-min -100 :x-max 40 :x-origin -100 :y-min 0.0 
	    ...
	    (unless separate-plots
	      ...
	      (plot-timed-data all-taus (reverse labels) voltages
			       :prompt-for-overlay t :y-label-vertical-position :upper-right
			       :width *particle-kinetics-plot-width* :height *particle-kinetics-plot-height*
			       :x-are-fns t
			       :y-label "ms" :x-label "mV" :x-min -100 :x-max 40 :x-origin -100 :y-min 0.0 
...
(defun edit-markov-transition-functions (type)
  (let* ((type (element type))
	 ...
	 (edited-from-to-functions
	  (loop for from-to-function in
		(choose-list-values-from-keys key-functions nil :label "Choose Markov Transition Functions to Edit")
		collect
		(list (first from-to-function)
		      ...
		      (setup-edited-function (third from-to-function)
					     ...
					     (format nil "~A: Edit ~A to ~A"
						     ...
						     (state-label-in-markov-states
						      type (markov-state-trans-list-from-state from-to-function))
						     (state-label-in-markov-states
						      type (markov-state-trans-list-to-state from-to-function))))))))
    (element-parameter
     type 'STATE-TRANSITIONS 
     (delete-duplicates (concatenate 'list (ELEMENT-PARAMETER type 'STATE-TRANSITIONS) edited-from-to-functions)
			:test 'TEST-TOP-TWO-OF-LISTS))
...
(defun find-markov-particle-state-data (particle state-index)
  (let ((particle (element particle 'particle)))
    (when particle
      (reverse
       (typecase state-index
	 (fixnum (element-parameter
		  particle (nth state-index (element-parameter (particle-type particle) 'markov-state-labels))))
