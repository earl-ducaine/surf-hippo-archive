...
(defun 3dplot (array &key (theta 0.0) (phi 0.0) (scale 3.0) (gain 1.0) (aspect 1.0)
  ...
  (declare (optimize (safety 1) (speed 3) (space 1))
	   (ignore width height ))
  (let* ((grid-size-x (the sf (or grid-size-x grid-size)))
	 (grid-size-y (the sf (or grid-size-y grid-size)))
	 (scale (s-flt scale))
    ...
    (let ((v-agg (make-v-agg virtual-polyline
      ...
      (s-value win :visible t)		; Need this here to make virtual agg work right.
      (draw-3d-axises win)
      (when comment (add-comment win comment))
      (opal:update win))
    win))
...
(defun 3dplot-menu (win)
  (let* ((dummy1 (float (rad-to-deg (g-value win :phi))))
	...
	(let ((dummy1 nil) (dummy2 nil) (dummy22 nil) dummy17 dummy26
	  ...
	  (cond-every
	   (dummy22 (s-value win :font (s-value win :plot-axis-font (font-menu (g-value win :plot-axis-font) (g-value win :title))))))))
...
(defun setup-3dplot-win (win title array grid-size-x grid-size-y grid-size theta phi scale)
  (unless win
    (let ((temp *standard-graphics-output*) *circuit-drawn*)
  ...
  (initialize-graphics-window win)
  (ph::add-plotting-interactors win)
...
(defun load-virtual-polyline-item-array (array temp-array line-style filling-style win)
  (declare (optimize (safety 1) (speed 3) (space 1)))
  (loop for x from 0 to (the fn (1- (array-dimension array 0))) do
	(loop for y from 0 to (the fn (1- (array-dimension array 1))) do
	      (setf (aref temp-array x y) (x-y-cons-histology-win (aref temp-array x y) win))))
  (list-to-array-generic
   (loop for y from 0 to (the fn (- (array-dimension array 1) 2)) by 1 nconcing 
	 (loop for x from 0 to (the fn (- (array-dimension array 0) 2)) by 1 
	       collect (list (concatenate 'list
					  (aref temp-array x y)
...
(defun draw-3d-axises (&optional win)
  (when win 
    (let* ((win win)
	   (data-array (g-value win :2darray))
      ...
      (loop for grid-max-corner in grid-max-corners
	    ...
	    do (add-line (get-win-view-plane-x grid-max-corner win)
			 (get-win-view-plane-y grid-max-corner win)
			 (get-win-view-plane-x grid-min-corner win)
			 (get-win-view-plane-y grid-min-corner win)
      ...
      (loop for corners in (list grid-max-corners grid-min-corners) do
		...
		((null (cdr  corner)))
	      (add-line (get-win-view-plane-x (car corner) win)
			(get-win-view-plane-y (car corner) win)
			(get-win-view-plane-x (cadr corner) win)
			(get-win-view-plane-y (cadr corner) win)
			agg :thickness 1.0 :color 'red :where :front))))))
...
(defun 3d-histo-plot (x-data x-incs y-data y-incs &key (base-offset 0) (increment 1)
  ...
  (3dplot (make-2d-histo-array x-data y-data  x-incs y-incs :increment increment
	  ...
	  :x-offset x-offset :y-offset y-offset :grid-size grid-size :grid-size-x grid-size-x
