...
#|
(defun channel-node (ch)
  (let ((cell-elt (channel-cell-element ch)))
    (typecase cell-elt
      (segment (segment-node-2 cell-elt))
      (t (soma-node cell-elt)))))
|#
...
(defun number-CHANNEL-TYPE-CHANNELS (type &optional only-active)
  (let ((type (element type 'channel-type)))
    (when (and type (not (and only-active (channel-type-block type))))
      (loop with ch = (channel-type-first-channel type)
	    while  ch
	    when (or (not only-active)
		     (and (not (channel-block ch))
			  (not (= (channel-gbar ch) 0.0d0))))
	    sum 1 into total
	    do (setq ch (channel-next-channel ch))
	    finally (return total)))))

(defun NB-ACTIVE-CHS-OF-TYPE (type)
  "Returns the number of channels of the associated with TYPE and satisfying the conditions for being
active."
  ...
  (let ((result 0))
    ...
    (channel-type-iterator
     ...
     do (when (channel-active-p ch) (setq result (1+ result))))
    result))
    ...
    (when ch-type
      (setf (channel-type-particle-types-and-powers ch-type)
	    (loop for particle-type-and-power in particle-types-and-powers
		  ...
		  do (sim-error (format nil "~A particle type hasn't been loaded!"
					(car particle-type-and-power)))
		  else
		  collect (cons (element (car particle-type-and-power) 'particle-type)
				(cadr particle-type-and-power)))))))

(defun reorder-channels-of-type (type &optional ch last-ch)
  (declare (optimize (safety 0) (speed 3) (space 0) (compilation-speed 0)))
  (when (and (channel-type-p type) *enable-reorder-channels-of-type*)
    (if ch
	(let ((last-channel-of-type (or last-ch (last-channel-of-type type))))
	  (if last-channel-of-type
	      (setf (channel-next-channel last-channel-of-type) ch)
	    (setf (channel-type-first-channel type) ch)))
      (progn
	(clear-channels-of-type type)
	(let ((last-ch nil))
	  (loop for ch being the hash-value of (channel-hash-table)
		when (eq (channel-type ch) type)
		do
		(reorder-channels-of-type type ch last-ch)
		(setq last-ch ch)))))))

(defun last-channel (ch)
  (if (and ch (channel-next-channel ch))
      (last-channel (channel-next-channel ch))
      ch))

(defun last-channel-of-type (type)
  (last-channel (channel-type-first-channel type)))

(defun clear-channels-of-type (type)
  (declare (optimize (safety 0) (speed 3) (space 0) (compilation-speed 0)))
  (setf (channel-type-first-channel type) nil)
  (loop for ch being the hash-value of (CHANNEL-HASH-TABLE)
	when (eq type (channel-type ch))
	do (setf (channel-next-channel ch) nil)))

(defun get-channels-of-type (type)
  (loop for ch being the hash-value of (CHANNEL-HASH-TABLE)
	when (eq type (channel-type ch))
	collect ch))
  ...
  (let ((type-name (element-name type 'channel-type))
	...
	(format t "))~%")))
      ...
      (element-document-extras type)
					;    (if (channel-type-parameters type)
					;	(format t "        (parameters . ~a)~%" (channel-type-parameters type)))
      (format t "                   ))~%")
      (format t "~%~%")
      (loop for type-power in (channel-type-particle-types-and-powers type)
...
(defun document-channel (ch)
  (let ((channel (element ch 'channel)))
    (when channel
      (format t "(create-channel ~s '~A ~A)~%"
	      (element-name (element-cell-element channel))
	      (element-name (element-type channel))
	      (if (channel-pre-synaptic-element channel)
		  (element-name (channel-pre-synaptic-element channel))
		  "")))))
...
(defun edit-channel (ch &optional called-from-type-menu)
  (let ((ch (element ch 'channel)))
    ...
    (let ((dummy1 (channel-gbar/perm-reference-value ch))
					; (dummy2 (or (element-parameter ch 'gbar-density) 0.0))
	  (dummy3 (channel-inherit-parameters-from-type ch))
					; (dummy4 (or (element-parameter ch 'gbar-source) :density))
	  dummy5 (dummy6 (channel-block ch)) dummy8 dummy9)
	 ...
	 ,(unless called-from-type-menu '(dummy5 "Edit Channel type" :boolean)))
      ...
      ;;        (case dummy4
      ;;          (:absolute (set-element-gbar-ref ch dummy1))
      ;;          (:density (set-element-gbar-density ch dummy2))))
...
(defun menu-e-rev-comment (type)
  (concatenate-strings
   ...
   (loop for ion-perm in (element-ion-permeabilities type)
	 ...
	 collect (format nil " ~d% ~a " (round (* 100 (cadr ion-perm)))
			 (string-capitalize (string (car ion-perm)))))))

(defun channel-type-menu-gbar-mod-comment (type)
  (format nil "GBAR modulation~%(applied to all ~A channels):" (element-name type)))


(defun channel-type-menu-block-comment (type)
  (concatenate-strings
   (format nil "Block all ~a channels" (channel-type-name type))
   (unless *active* (format nil " [Global Block Applied]"))))

(defun channel-type-menu-prts-comment (type class)
  (format nil "Edit parameters for the ~a ~Aparticle types"
...
(defun edit-channel-type (type)
  (when (element type 'channel-type) (menu-for-channel-types type)))

(defun edit-channels-of-type (type)
  (loop for chname in 
	(choose-list-values
	 (loop for ch in (channels)
	       when (eq type (channel-type ch)) collect (channel-name ch))
	 nil
	 :label (format nil "Choose Channel of Type ~A To Modify" (channel-type-name type)))
...
(defun menu-for-channel-types (&optional types)
  (loop for type in (or (element (if (consp types) types (list types))) (menu-for-type 'channel-type)) do
	(let (dummy1 dummy2 dummy3 dummy4 dummy5 dummy6 dummy7 dummy9 dummy10 dummy11 update-type
		     ...
		     dummy16 dummy17 dummy18 dummy19 dummy20 dummy21 dummy22 dummy23 dummy24 dummy25)
	  (loop while (or dummy7 dummy10 dummy4 dummy9 dummy12 dummy20 dummy23 dummy24 dummy25 dummy18) do
		(setq dummy1 (channel-type-gbar-density type) dummy16 (channel-type-gbar-ref type)
		      dummy17 (channel-type-gbar-source type)
		      dummy2 (channel-type-e-rev type) dummy3 (channel-type-block type)
		      dummy4 nil dummy5 nil dummy6 nil dummy7 nil
		      dummy8 nil dummy9 nil dummy10 nil dummy12 nil
		      dummy13 (channel-type-name type) dummy14 (channel-type-q10 type)
		      dummy15 (channel-type-reference-temp type) dummy11 (channel-type-variable-e-rev type)
		      dummy18 nil
		      dummy19 (if (channel-type-use-defined-e-rev type) :use_defined :use_ion_permeabilities)
		      dummy20 nil dummy21 nil
		      dummy22 (or (element-parameter type 'gbar-modulation) 1.0)
		      dummy23 nil dummy24 nil dummy25 nil)
		(let ((menu-list
		       `((dummy16 ,(case (channel-type-iv-relation type)
				     (:ohmic "Absolute Conductance [uS]")
				     (:constant-field "Absolute Permeability [cm3/s]"))
			  :float)
			 (dummy1
			  ,(case (channel-type-iv-relation type)
			     (:ohmic "Conductance Density [pS/um2]")
			     (:constant-field "Permeability Density [1.0e-6cm3/s/um2]"))
			  :float)
			 (dummy17
			  ,(case (channel-type-iv-relation type)
			 ...
			 (dummy22 ,(channel-type-menu-gbar-mod-comment type) :float)
			 ,(unless (eq (channel-type-iv-relation type) :constant-field)
			    `(dummy2 ,(format nil "Fixed Reversal Potential [mV] (actual ~,2emV)"
				       (current-element-type-e-rev type))
			      :float))
			 ,(unless (eq (channel-type-iv-relation type) :constant-field)
			    `(dummy19 ,(menu-e-rev-comment type) :choose (:use_defined :use_ion_permeabilities)))
			 ,(unless (eq (channel-type-iv-relation type) :constant-field)
			    `(dummy20 "Edit ion permeabilities" :boolean))
			 ,(when (channel-type-conc-int-type-params type)
			    `(dummy21 "Edit concentration integrator parameters" :boolean))
			 ...
			 (dummy25
			  ,(format nil
			    "Revamp channel type (and associated particle types)~%definitions from current library")
			  :boolean)
			 (dummy23 "Restore any edited parameters above" :boolean))))
		  (cond-every
		   ((channel-type-particle-types-and-powers type)
		    (push `(dummy4 ,(channel-type-menu-prts-comment type 'v-dep) :boolean) menu-list)
		    (push `(dummy7 "Plot voltage-dependent particles:"
			    :x-choose ,(channel-type-particles-plot-options type)) menu-list))
		   ((and (not (eq (channel-type-iv-relation type) :constant-field))
			 (member 'ca (channel-type-ion-permeabilities type) :key 'car))
		    (push '(dummy11 "Dynamically calculate e-rev based on [Ca++]" :boolean) menu-list))
		   ((channel-type-conc-particle-types-and-powers type)
		    (push `(dummy9 ,(channel-type-menu-prts-comment type 'conc-dep) :boolean) menu-list)
		    (push '(dummy10 "Plot concentration-dependent particles:" :x-choose (:steady_state :tau)) menu-list))
		  ...
		  (choose-variable-values
		   ...
		   :text (ADD-LINEFEEDS-TO-STRING-LIST
			  (list (when (independent-element-gbars-p type) "Some channels have independent GBAR parameters.")
		  ...
		  (if dummy25 (create-channel-type type nil t)
			...
			(setf (channel-type-use-defined-e-rev type) (eq dummy19 :use_defined))
			(set-element-name type dummy13)
			(when dummy18 (edit-channels-of-type type))
			(setq update-type
			      (or update-type
				  (not (and (= (channel-type-gbar-density type) dummy1)
					    (= (or (element-parameter type 'gbar-modulation) 1.0) dummy22)
					    (= dummy16 (channel-type-gbar-ref type))
					    (eq dummy17 (channel-type-gbar-source type))
					    (= (channel-type-e-rev type) dummy2)
					    (eq (channel-type-block type) dummy3)
					    (eq (channel-type-variable-e-rev type) dummy11)
					    (= (channel-type-q10 type) dummy14)
					    (= (channel-type-reference-temp type) dummy15)))))
			(element-parameter type 'gbar-modulation dummy22)
			(setf (channel-type-gbar-density type) dummy1
			      (channel-type-gbar-ref type) dummy16
			      (channel-type-gbar-source type) dummy17
			      (channel-type-e-rev type) dummy2
			      (channel-type-block type) dummy3
  ...
  (setf (channel-type-conc-int-type-params type)
	(loop for conc-int-type-param in (channel-type-conc-int-type-params type)
	      collect (let ((dummy1 (s-flt (* 100 (cadadr conc-int-type-param)))))
			(choose-variable-values
			 `((dummy1 ,(format nil "Percentage of current feeding shell ~A" (caadr conc-int-type-param))
			    :float))
			 :label (format nil "Current of ~A into concentration intergrator ~A"
					(element-name type) (car conc-int-type-param)))
			(list (car conc-int-type-param)
			      (list (caadr conc-int-type-param) (/ dummy1 100.0d0))))))
  (channel-type-iterator
   (ch type) do (setf (channel-conc-ints-params ch)
		      (parse-conc-int-info-for-element type (element-cell-element ch))))
  (set-conc-integrators-parameters))



(defun create-channel-type (type-symbol &optional actual-type-symbol update-parameters)
  "TYPE-SYMBOL is a symbol or a channel type; in the former case it must match the CAR of one of the
lists contained in channel type model parameter library. Returns the channel type structure, whether
is was already made or not. If the type was already made, and UPDATE-PARAMETERS is T, the type will
be updated according to the current description loaded in parameter library. This will create
particle types (v-dep and conc) according the entries in the V-PARTICLES and CONC-PARTICLES a-list
entries."
  ...
  (let* ((type (unless actual-type-symbol
		 (if (channel-type-p type-symbol)
		     type-symbol
		     (gethash (string type-symbol) (CHANNEL-TYPE-HASH-TABLE)))))
	...
	(sim-error (format nil "~A has a recursive parent type!" parent-type-symbol)))
      (unless original-parameters (sim-error (format nil "Don't know anything about channel type ~A!" type-symbol)))
      (when actual-type-symbol (setq type-symbol actual-type-symbol))
      (unless type
	(setq type (if parent-type-symbol
		       (create-CHANNEL-TYPE parent-type-symbol type-symbol update-parameters)
		       (make-CHANNEL-TYPE :name (string type-symbol)))))
      ...
      (setf (channel-type-particle-types-and-powers type)
	    (loop for v-part-type-and-order in (get-a-value 'v-particles original-parameters) collecting
		  (cons (create-particle-type (car v-part-type-and-order) nil update-parameters)
			(cadr v-part-type-and-order)))
	    (channel-type-conc-particle-types-and-powers type)
	    (loop for ca-part-type-and-order in (get-a-value 'conc-particles original-parameters) collecting
		  (cons (create-conc-particle-type (car ca-part-type-and-order) nil update-parameters)
	...
	(element-parameter type 'CONC-INT-TYPE-E-REV-PARAMS
			   (massage-conc-int-type-params (get-a-value 'CONC-INT-TYPE-E-REV-PARAMS original-parameters)))))
      (setf (gethash (string type-symbol) (CHANNEL-TYPE-HASH-TABLE)) type)
      (setq *channel-type* type)
      (when update-parameters (set-channels-parameters t type)))
    type))

(defun revamp-channel-type-parameters ()
  "Run through all the current channel types and update their parameters (including those for the associated particle types)
according to the current parameters in the channel type, particle type, and conc particle type parameter libraries."
  (loop for type in (channel-types) do (create-channel-type (element-name type) nil t)))
  
(defun get-channel-simple-name ()
  (loop for candidate from (max 1 *channel-simple-name-counter*)
	until (not (gethash candidate (channel-hash-table)))
...
(defun rename-channels-simple (&optional (channels (channels)))
  ...
  (loop for seg in channels do
	(let ((name (get-channel-simple-name)))
	  (set-element-name seg name 'channel))))

(defun create-channel (cell-element-or-name channel-type-symbol &key pre-synaptic-element conc-int-delta)
  "A channel label should be added to the name of the cell element to generate the 'channel-name'. The node for the channel is the same as the cell
element node. SET-CHANNEL-PARAMETERS sets the gbar of the channel."
  (unless *only-load-passive*
    (let* (*print-pretty*
	   (cell-element (element-cell-element cell-element-or-name))
	   (node (element-node cell-element))
	   (type (create-channel-type channel-type-symbol))
	   (channel-type-symbol (channel-type-name type))
	   (channel-name (if *use-simple-names*
			   (GET-CHANNEL-SIMPLE-NAME)
					; (1+ (hash-table-count (CHANNEL-HASH-TABLE)))
			   (format nil "~A-~A" (node-name node) channel-type-symbol)))
	   (current-channel (gethash channel-name (CHANNEL-HASH-TABLE))))
      (cond
	  ...
	  (format t "CREATE-CHANNEL: channel ~a already defined~%" channel-name)))
       ...
       ((or (not current-channel)
	    (confirm-alternate-name-creation-and-update-*PROMPT-FOR-ALTERNATE-ELEMENT-NAMES*
	     (format nil "Make new name for duplicate channel type ~A on ~A" channel-type-symbol (if node (node-name node) "Undefined Dest."))))
	(let ((channel-name (if current-channel (check-element-name channel-name 'channel) channel-name)))
	  (when channel-name
		...
		(element-parameter ch 'conc-int-delta conc-int-delta))
	      ...
	      (reorder-channels-of-type type ch)
	      (setf (gethash channel-name (CHANNEL-HASH-TABLE)) ch)))
	  (set-channel-parameters (gethash channel-name (CHANNEL-HASH-TABLE)))
	  (setq *enable-channel-type-membrane-parameter-update* t
		...
		*channel* (gethash channel-name (CHANNEL-HASH-TABLE)))
	  *channel*))))))
...
(defun create-channels (element types)
  ...
  (when (element-cell-element element)
    (dolist (channel-type (coerce-to-list types))
      (create-channel (element-cell-element element) channel-type))))


;; ADD-CHANNEL-AD-LIB Adds channel of name TYPE-SYMBOL to targets in TARGET-NAMES, possibly
;; after the cell has been defined.
(defun add-channels-ad-lib (targets type)  
  (add-channel-to-targets targets type))  
...
(defun add-channel-to-targets (targets type)
  ...
  (loop for target in targets 
	when (element-cell-element target)
	do (create-channel (element-cell-element target) type)))
...
;; ADD-CHANNEL-TYPE Adds channel of name TYPE-SYMBOL to all cells.
...
(defun add-channel-to-cells (cells types &optional (percentage-of-channel-nodes 100))
  ...
  (let ((*monitor-circuit-modifications* *monitor-circuit-modifications*))
    ...
    (loop for cell-element in (cell-elements cells)
	  when (or (not percentage-of-channel-nodes)
		   (= 100 percentage-of-channel-nodes)
		   (< (random 100) percentage-of-channel-nodes))
	  do (create-channels cell-element types))))


(defun channel-type-particle-types (element)
  (loop for v-part-type-and-order in (channel-type-particle-types-and-powers element)
	collect (car v-part-type-and-order)))

(defun channel-type-conc-particle-types (element)
  (loop for ca-part-type-and-order in (channel-type-conc-particle-types-and-powers element)
	collect (car ca-part-type-and-order)))

(defun get-current-channel-type-names ()
  (loop for type in (channel-types) when (channel-type-first-channel type) collect (channel-type-name type)))

(defun get-current-channel-types ()
  (loop for type in (channel-types) when (channel-type-first-channel type) collect type))

(defun get-current-channel-types-menu (&optional (label "Channel Types In Circuit"))
  (loop for name in (choose-list-values (get-current-channel-type-names) nil :do-all-at-once t :label label)
	collect (element name 'channel-type)))


(defun are-there-channels ()
  (true-p *channel*))

(defun print-channels ()
  (PRINT-MODEL-PARAMETERS "channel"))

(defun print-channel (ch)
  (let ((ch (element ch 'channel)))
    (format t "Channel ~a~A: ~A, cell ~a; ~A~A"
	    (channel-name ch)
	    (cond ((channel-type-block (channel-type ch)) " (type blocked)")
		  ((channel-block ch) " (blocked)")
		  (t ""))
	    (element-name (element-type ch))
	    (cell-name (node-cell (channel-node ch)))
	    (print-element-gbar-string ch)
    ...
    (when *simulation-initialized*
      (format t "~&    Conductance ~,2e ~a, current ~,2e ~a @ ~,2e ms"
	      (channel-conductance ch) (channel-conductance-units)
	      (get-channel-current-not-inline ch) (channel-current-units)
	      *real-time*))
    (format t "~%~%")))
  
(defun channel-conductance-units ()
  (plot-list-info-units-label
   (find '*plot-channel-conductances* *plot-lists-info* :key 'car)))
  
(defun channel-current-units ()
  (plot-list-info-units-label
   (find '*plot-channel-currents* *plot-lists-info* :key 'car)))


(defun print-channel-types ()
  (PRINT-MODEL-PARAMETERS "channel-type"))

(defun channels-of-type (type &optional cell-element)
  "Return a list of channels of TYPE that are associated with members of CELL-ELEMENT [atom or list].
Members of CELL-ELEMENT may refer explicitly to a cell type or specific cell, or may be associated
with a cell element. If CELL-ELEMENT is NIL, then all channels of TYPE are returned."
  (let ((type (element type 'channel-type)))
    (when type
...
(defun channels-of-ion-type (ion-type)
  (loop for type in (channel-types-of-ion-type ion-type)
	when type
	nconc (copy-list (channel-type-channels type))))
...
(defun print-channel-type (type &optional (always t))
  (let ((type (element type 'channel-type)))
    (when (or (channels-of-type type) always)
      (format t "Channel Type ~a: " (channel-type-name type))
      (print-membrane-element-iv-parameters type)
      (when (channel-type-block type) (format t "   This channel type is blocked!~%"))
	...
	(loop for particle-type-and-power in (channel-type-particle-types-and-powers type) do
	      (format t "~a (power ~a) " (particle-type-name (car particle-type-and-power))
		      (cdr particle-type-and-power)))
	(format t "~%"))
	...
	(loop for particle-type-and-power in (channel-type-conc-particle-types-and-powers type) do
	      (format t "~a (power ~a) " (conc-particle-type-name (car particle-type-and-power))
		      (cdr particle-type-and-power)))
	(format t "~%"))
	...
	(loop for conc-int-type-param in (channel-type-conc-int-type-params type) do
	      (format t "      Feeds conc int type ~A, shell ~A (~,1f%)~%"
		      (car conc-int-type-param)
		      (caadr conc-int-type-param)
		      (s-flt (* 100 (cadadr conc-int-type-param))))))
...
(defun print-channel-type-brief (type)
  (let* ((type (element type 'channel-type))
	 (NB-ACTIVE-chS-OF-TYPE (NB-ACTIVE-chS-OF-TYPE type)))
    (format t "Channel Type ~a: ~D examplaire~p  " (channel-type-name type) NB-ACTIVE-chS-OF-TYPE NB-ACTIVE-chS-OF-TYPE)
    ...
    (when (channel-type-block type) (format t " (**BLOCKED**)"))
    (format t "~%")))
...
(defun print-create-channel (channel)
  (format t
	  "(create-channel ~s ~s"
	  (element-name (element-cell-element channel))
	  (channel-type-name (channel-type channel)))
  (when (channel-pre-synaptic-element channel)
    (format t " ~s" (element-name (channel-pre-synaptic-element channel))))
  (format t ")~%"))

(defun block-all-channels ()
  (loop for syn in (channels) do (setf (channel-block syn) t)))

(defun block-all-channel-types ()
  (loop for syn in (channel-types) do (setf (channel-type-block syn) t)))

(defun block-channel-types (types)
  (loop for type in (coerce-to-list types)
	when (element type 'channel-type) do (setf (channel-type-block (element type 'channel-type)) t)))

(defun unblock-all-channels ()
  (unblock-all-channel-types)
  (loop for syn in (channels) do (setf (channel-block syn) nil)))

(defun unblock-all-channel-types ()
  (loop for syn in (channel-types) do (setf (channel-type-block syn) nil)))


(defun unblock-channel-types (types)
  (loop for type in (coerce-to-list types)
	when (element type 'channel-type) do (setf (channel-type-block (element type 'channel-type)) nil)))

(defun blocked-channel-types ()
  (loop for type in (channel-types)
	when (or (not *active*) (and (instance-in-cell type) (channel-type-block type)))
	collect type))

(defun set-channel-types-parameters ()
  (loop for type in (channel-types) do (set-channel-type-parameters type)))
...
;; SET-CHANNELS-PARAMETERS - Note that SET-CHANNEL-PARAMETERS is defined in element_functions.lisp
(defun set-channels-parameters (&optional (update-fixed-e-rev t) type (update-channel-gbars t))
  (let ((type (element type 'channel-type)))
    (if type
	(channel-type-iterator
	 (ch type)
	 do (set-channel-parameters ch update-fixed-e-rev update-channel-gbars))
	(loop for ch being the hash-value of (CHANNEL-HASH-TABLE)
	      do (set-channel-parameters ch update-fixed-e-rev update-channel-gbars)))
    nil))
...
(defun set-channel-type-parameters (type)
  (let ((type (element type 'channel-type)))
    (when type
      (channel-type-iterator
       (ch type)
       do (set-channel-parameters ch t))))
  nil)



(defun set-channel-type-param (type param &optional value update)
  (let* ((type (element type 'channel-type))
	 (return-value (when type
			 (case param
			   ...
			   ((:permeability-ref permeability-ref :gbar-ref gbar-ref)
			    (if value
				(setf (channel-type-gbar-ref type) (s-flt value))
				(channel-type-gbar-ref type)))
			   ((:permeability-source permeability-source :gbar-source gbar-source)
			    (if value
				(setf (channel-type-gbar-source type) value)
				(channel-type-gbar-source type)))
			   ((:gbar-density gbar-density :permeability-density permeability-density)
			    (if value
				(setf (channel-type-gbar-density type) (s-flt value))
				(channel-type-gbar-density type)))
			   ((:e-rev e-rev) (if value
					       (setf (channel-type-e-rev type) (s-flt value))
					       (channel-type-e-rev type)))
			   ((:block block) (if value
					       (setf (channel-type-block type) (= 1 value))
					       (channel-type-block type)))))))    
    (when (and value return-value)
      (setq *enable-channel-membrane-parameter-update* t)
      (if update (set-channels-parameters nil type) (setq *recheck-circuit-elements-parameters* t)))
    return-value))
  ...
  (when channels
    (loop for ch in channels
	  collect (loop for prt in (concatenate 'list (channel-conc-particles ch) (channel-particles ch))
			collect (typecase prt
				  (conc-particle (conc-particle-double-floats prt))
				  (particle (particle-double-floats prt))))
	  ...
	  (set-several-element-parameters type `( ;; (active-channels ,channels)
						 (channel-particles ,channel-particles-list)
						 (particle-info ,channel-type-particle-info-list)
						 (node-fixnums ,node-fixnums-list)
						 (node-floats ,node-floats-list))))
    nil))
...
(defun channel-type-active-p (type)
  (let ((type (element-type type)))
    (when type
      (and (not (channel-type-block type))
	   (channel-type-first-channel type)
	   (channel-type-iterator
	    (ch type)
	    when (channel-active-p ch t)
	    do (return t)
	    finally (return nil))))))
  ...
  (loop for type in *channel-type-list* do
	(unless (channel-type-variable-e-rev type)	
	  (let* ((cell-types-in-circuit (list-of-all-things-in-circuit 'cell-type))
	    ...
	    (channel-type-iterator
	     (ch type)
	     unless (or (channel-block ch)	  
			(channel-conc-ints-params ch))
	     do (update-element-fixed-e-rev ch fixed-e-rev type)))))
  nil)
...
(defun init-channels ()
  (loop for type in (channel-types)
	...
	(channel-type-iterator
	 (ch type)
	 do (setf (channel-conductance ch) 0.0d0
		  (channel-current ch) 0.0d0)))
		  
  (when *active* (init-channels-e-rev) (eval-all-channels t)))
...
(defun channel-voltage (ch)
  (node-voltage-n (channel-node ch)))

(proclaim '(inline channel-voltage-n+1))
(defun channel-voltage-n+1 (ch)
  (node-voltage-n+1 (channel-node ch)))
...
(defun channel-constant-field-current-n-1 (ch &optional permeability conc-in conc-out valence)
  (declare (optimize (safety 0) (speed 3) (space 0) (compilation-speed 0)))
...
(proclaim '(inline get-channel-current))
(defun get-channel-current (ch &key conc-in conc-out valence voltage)
  ...
  (case (channel-type-iv-relation (channel-type ch))
    (:CONSTANT-FIELD
	 ...
	 (channel-constant-field-current :permeability (channel-conductance ch)
					 :conc-in conc-in :conc-out conc-out :valence valence :voltage voltage)
	 (channel-constant-field-current-dumb ch :permeability (channel-conductance ch)
					      :conc-in conc-in :conc-out conc-out :valence valence :voltage voltage)))
    (t (channel-ohmic-current ch))))


(proclaim '(inline get-channel-current-complete))
;; GET-CHANNEL-CURRENT-complete Returns the channel current nA of CH at time N, a double float.
...
;; GET-CHANNEL-CURRENT-N+1 Returns the channel current nA of CH at time N+1, a double float.
...
;; mV
(defun get-channel-e-rev (channel)
...
(defun get-channel-conc-in (ch &optional fast-conc-in-calculation)
...
(defun get-channel-conc-out (ch &optional fast-conc-out-calculation)
...
(defmacro particle-looper (parts part-types-and-powers conductance)
  `(loop for particle in ,parts
    for particle-type-and-power in ,part-types-and-powers
    do
    (setq ,conductance (* ,conductance
			(ch-power-double-macro (particle-aref-state-n+1 (particle-double-floats particle))
					       (cdr particle-type-and-power))))))

(defmacro conc-particle-looper (parts part-types-and-powers conductance)
  `(loop for conc-particle in ,parts
    for conc-particle-type-and-power in ,part-types-and-powers
    do
    (setq ,conductance (* ,conductance
			(ch-power-double-macro (conc-particle-aref-state-n+1 (conc-particle-double-floats conc-particle))
					       (cdr conc-particle-type-and-power))))))

(defun CHANNEL-ACTIVE-P (channel &optional fast)
  "T when a CHANNEL satisfies the condition for being ACTIVE: the channel or type is not blocked, and
the channel conductance is not 0.0. If CHANNEL is a pointer to a channel, then the FAST flag may be used."
  (declare (optimize (speed 3) (space 0) (safety 1)))
  (let ((channel (if fast channel (element channel 'channel))))
    (and channel
	 (not (or (channel-block channel)
		  (channel-type-block (channel-type channel))
...
(defun save-current-during-eval-p (type &optional fast)
  ...
  (true-p (or (element-conc-int-type-params type fast)
	    (let ((element-iv-parameters (element-iv-parameters type fast)))
	...
	(let* ((type-params (channel-type-parameters type))
	  ...
	  (channel-type-iterator
	   ...
	   do (let* ((conductance (channel-gbar ch))
		...
		(particle-looper
		 (channel-particles ch) (channel-type-particle-types-and-powers type) conductance)
		(conc-particle-looper
		 (channel-conc-particles ch) (channel-type-conc-particle-types-and-powers type) conductance)
		(when conductance-function
		  (setq conductance (* conductance (the df (funcall (the function conductance-function) ch)))))
		(when static-voltage-dependence
		  (setq conductance
			(* conductance (aref (the vec-flt static-voltage-dependence) (node-prt-v-index channel-node)))))
...
(defun plot-channel-types (&optional channel-types)
  (plot-channels channel-types))

(defun plot-channels (&optional channel-types)
  (loop for type in
	(or (coerce-to-list channel-types)
...
(defun plot-channel-v-particles (type &key (new-plot nil) overlay (what '(:steady_state :tau :alpha_&_beta)) query)
  (declare (ignore query))
  (let ((type (element type 'channel-type)))
    (when type
      (let ((*create-new-plot-windows* new-plot))
	(loop for prt-type-power in (channel-type-particle-types-and-powers type)
	      when (and (eq :markov (particle-type-class (car prt-type-power)))
			(element-parameter (car prt-type-power) 'concentration-particle-type))
	      do (plot-markov-conc-dep-particle-type
		  (car prt-type-power) :what what :overlay overlay :new-plot new-plot :channel-type type)
	      else collect prt-type-power into prt-types-powers
	      finally
	      (when prt-types-powers
		(plot-v-particles prt-types-powers :what what :overlay overlay :new-plot new-plot :channel-type type)))))))
...
(defun channel-type-particle-plot-comment (channel-type)
  (let ((channel-type (element channel-type 'channel-type)))
	...
	(:ohmic
	 (format nil "~A~%~A"
		 (case (channel-type-gbar-source channel-type)
		   (:density (format nil "Gbar Dens ~,2e pS/um^2"
				     (channel-type-gbar-density channel-type)))
		   (:absolute (format nil "Gbar ~,2e uS"
				      (channel-type-gbar-ref channel-type))))
		 (cond ((channel-type-variable-e-rev channel-type)
			"Conc-dep E-rev")
		       ((channel-type-use-defined-e-rev channel-type)
			(format nil "E-rev ~a mV" (channel-type-e-rev channel-type)))
		       (t "Ion-perm-dep E-rev"))))
	(:constant-field
	 (format nil "~A~%Constant Field Model"
		 (case (channel-type-gbar-source channel-type)
		   (:density (format nil "Perm Dens ~,2e 1.0e-6cm3/s/um2"
				     (channel-type-gbar-density channel-type)))
		   (:absolute (format nil "Perm ~,2e cm3/s"
				      (channel-type-gbar-ref channel-type))))))))))
...
(defun plot-channel-conc-particles (type &key (new-plot nil) overlay  (what '(:steady_state :tau))
					 ...
					 increment-voltage)
  (plot-conc-particle nil :prt-types-powers (channel-type-conc-particle-types-and-powers (element type 'channel-type))
		      ...
		      :increment-voltage increment-voltage))

;;; MENU-FOR-CHANNEL-PARAMETERS
(defun menu-for-channel-parameters ()
  (let (dummy1 dummy2 dummy3 dummy4 dummy5 dummy6 dummy7)
    (choose-variable-values `((dummy1 "Edit parameters of channel types" :boolean)
			      ...
			      ; ,(when *circuit-loaded* `(dummy4 "Add channel types randomly?" :boolean))
			      )
			    :label "Setting Up Channel Parameters")
    (cond-every
     (dummy5 (menu-for-removing-channel-types))
     (dummy4 (menu-for-adding-channel-types))
...
(defun menu-for-removing-channels (target-elt)
  (loop for name in
	(choose-list-values
	 (loop for channel in (get-node-elements-of-type target-elt 'channel) collect (channel-name channel))
...
(defun menu-for-adding-channel-types (&optional target-elt)
  ...
  (setf (MODEL-PARAMETER-type-LIBRARY (type-symbol-model 'channel-type))
	(delete-duplicates (MODEL-PARAMETER-type-LIBRARY (type-symbol-model 'channel-type))
  ...
  (if (and target-elt
	...
	(loop for channel in original-channels
	      ...
	      else do (setq new-channel-type-names
			    (string-remove (channel-type-name (channel-type channel)) new-channel-type-names))))
      (if target-elt
	...
	(loop for type-symbol in new-channel-type-names
	      ...
	      (add-channel-type
	       type-symbol
	       (get-integer 100 100 0 "Add randomly to this percentage of nodes:"
			    (format nil "Adding Channel Type ~a at Random" type-symbol))))))))
  ...
  (let ((type (element type 'channel-type)))
    (when type
      (let* ((use-all-particles (eq include-particle-types :all))
	    ...
	    (gbar (or gbar (and (eq (channel-type-gbar-source type) :absolute)
				(channel-type-gbar-ref type))
		      1.0)))	
	   ...
	   (if (channel-type-conc-particle-types-and-powers type)
	       (apply '* (loop for conc-prt-type-power in (channel-type-conc-particle-types-and-powers type)
			       ...
			       collect (conc-particle-type-ss (car conc-prt-type-power) conc-in
							      :power (cdr conc-prt-type-power)
							      :voltage voltage)))
	       1.0)
	   gbar
	   (case (channel-type-iv-relation type)
	     (:CONSTANT-FIELD (constant-field-equation-double voltage (d-flt conc-in)
							      (d-flt conc-out) 1.0d0
							      (s-flt valence) 1.0d0))
...
(defun channel-ss-current (channel voltage &optional concentration)
  (let* ((channel (element channel))
    ...
    (* (if (channel-type-particle-types-and-powers type)
	   ...
	   (apply '* (loop for conc-prt-type-power in (channel-type-conc-particle-types-and-powers type)
			   collecting 
			   (conc-particle-type-ss (car conc-prt-type-power) concentration
						  :power (cdr conc-prt-type-power)
						  :voltage voltage)))
	   1.0)
...
(defun eval-all-channels (&optional (first-iteration-of-step t))
  (declare (optimize (safety 0) (speed 3) (space 0) (compilation-speed 0)))
  (loop for ch-type in *channel-type-list* do
	(let* ((type-params (channel-type-parameters ch-type))
	       (static-voltage-dependence (get-a-value 'static-voltage-dependence type-params))
	       (conc-int-p (true-p (channel-type-conc-int-type-params ch-type)))		 
	       (valence (when conc-int-p (get-a-value 'effective-default-valence type-params)))
	       (save-e-rev (and conc-int-p first-iteration-of-step (channel-type-variable-e-rev ch-type)))
	       (fixed-conc-in (and conc-int-p (not (get-a-value 'intracellular-conc-ints type-params))
				   (get-a-value 'effective-default-intracellular-concentration type-params)))
               (fast-conc-in-calculation (and conc-int-p
                                              (not fixed-conc-in)
                                              (= 1 (length (the cons (channel-type-conc-int-type-params ch-type))))))
	       (fixed-conc-out (and conc-int-p (not (get-a-value 'extracellular-conc-ints type-params))
				    (get-a-value 'effective-default-extracellular-concentration type-params)))
	       (iv-relation (channel-type-iv-relation ch-type))
	       (constant-field (eq iv-relation :constant-field))
	       (conductance-function (get-a-value 'conductance-function type-params)))
	  (loop for ch in (get-a-value 'active-channels type-params)
		for node-double-floats in (get-a-value 'node-floats type-params)
		for node-fixnums in (get-a-value 'node-fixnums type-params)
		for particle-floats-sequence in (get-a-value 'channel-particles type-params)
		for ch-type-prt-type-info in (get-a-value 'particle-info type-params)
 		do
		(let ((conductance (channel-gbar ch)))
		  (declare (double-float conductance))
		  (loop for particle-floats in particle-floats-sequence	; This handles both v-dep and conc-dep partcles.
			for power fixnum in (car ch-type-prt-type-info) 
			do (setq conductance
				 (* conductance (ch-power-double-macro (particle-aref-state-n+1 particle-floats) power))))
		  (when conductance-function
		    (setq conductance (* conductance (the df (funcall (the function conductance-function) ch)))))
		  (when static-voltage-dependence
		    (setq conductance
			  (* conductance
			     (aref (the vec-flt static-voltage-dependence) (node-prt-v-index (channel-node ch))))))

		  (when (cadr ch-type-prt-type-info)
					; (or conc-int-p (cadr ch-type-prt-type-info)) ; Flag to enable conductance saving.
		    (setf (channel-conductance ch) conductance))
		  (when (or CONSTANT-FIELD conc-int-p)
		    (setf (channel-current ch)
			  (if CONSTANT-FIELD
			      (channel-constant-field-current
                               :conductance conductance ; Really is permeability
                               :conc-in (the df (or fixed-conc-in (get-channel-conc-in ch fast-conc-in-calculation)))
                               :conc-out (the df (or fixed-conc-out (get-channel-conc-out ch)))
                               :voltage (node-aref-particle-voltage node-double-floats)
                               :valence valence)
			      (channel-ohmic-current ch))))
		  (unless (node-has-ideal-voltage-source (channel-node ch))
		    (if CONSTANT-FIELD
			(setf (node-aref-current node-double-floats)
			      (+ (node-aref-current node-double-floats) (channel-current ch)))
			(progn
			  (setf (node-aref-current node-double-floats)
				(- (node-aref-current node-double-floats) (* conductance (channel-e-rev ch))))
			  (setf (node-aref-jacobian node-double-floats)
				(+ (node-aref-jacobian node-double-floats) conductance)))))
		  (when save-e-rev
		    (setf (channel-e-rev ch) (element-e-rev-from-shells (channel-conc-ints-params ch))))))))
  nil)
|#
...
(defun eval-all-channels (&optional (first-iteration-of-step t))
  ...
  (loop for ch-type in *channel-type-list* do
	(let* ((type-params (channel-type-parameters ch-type))
	       (static-voltage-dependence (get-a-value 'static-voltage-dependence type-params))
	       (conc-int-p (true-p (channel-type-conc-int-type-params ch-type)))
	       (constant-field (eq (channel-type-iv-relation ch-type) :constant-field))
	       (conc-int-p-and-constant-field (and conc-int-p constant-field))
	       (conc-int-p-or-constant-field (or conc-int-p constant-field))
	       (valence (when conc-int-p-or-constant-field (get-a-value 'effective-default-valence type-params)))
	       (save-e-rev (and conc-int-p first-iteration-of-step (channel-type-variable-e-rev ch-type)))
	       (fixed-conc-in (and conc-int-p-or-constant-field
				   (not (get-a-value 'intracellular-conc-ints type-params))
				   (get-a-value 'effective-default-intracellular-concentration type-params)))
               (fast-conc-in-calculation (and conc-int-p-or-constant-field
                                              (not fixed-conc-in)
                                              (= 1 (length (the cons (channel-type-conc-int-type-params ch-type))))))
	       (fixed-conc-out (and conc-int-p-or-constant-field
				    (not (get-a-value 'extracellular-conc-ints type-params))
				    (get-a-value 'effective-default-extracellular-concentration type-params)))
	       ;; The 0.0's are dummies.
	       (vf/rt (if conc-int-p-and-constant-field (* (the sf valence) *F/RT*) 0.0))
	       (vsqd-ff/rt (if conc-int-p-and-constant-field (* (the sf valence) faraday vf/rt) 0.0))
	  ...
	  (loop for ch in (get-a-value 'active-channels type-params)
		...
		(let ((conductance (channel-gbar ch)))
		  ...
		  (when static-voltage-dependence
		    (setq conductance
			  (* conductance
			     (aref (the vec-flt static-voltage-dependence) (node-fixnum-aref-prt-v-index node-fixnums)))))

		  (when (cadr ch-type-prt-type-info) ; Flag to enable conductance saving.
		    (setf (channel-conductance ch) conductance))
		  (when (or CONSTANT-FIELD conc-int-p)
		    (setf (channel-current ch)
			  (if CONSTANT-FIELD
			      (fast-constant-field-equation
			       (node-aref-particle-voltage node-double-floats)
			       (the df (or fixed-conc-in (get-channel-conc-in ch fast-conc-in-calculation)))
			       (the df (or fixed-conc-out (get-channel-conc-out ch)))
			       (the sf vsqd-ff/rt)
			       (the sf vf/rt)
			       conductance)
			    (channel-ohmic-current ch))))
...
#|
(defun init-channels-e-rev ()
  (loop for ch-type in *channel-type-list* unless (channel-type-variable-e-rev ch-type)	do
	(let* ((cell-types-in-circuit (list-of-all-things-in-circuit 'cell-type))
	       (fixed-e-rev (when (= (length cell-types-in-circuit) 1)
			      (current-element-type-e-rev ch-type (car cell-types-in-circuit)))))
	  (loop for ch in (element-parameter ch-type 'active-channels)
		unless (channel-conc-ints-params ch) do	(update-element-fixed-e-rev ch fixed-e-rev ch-type)))))
|#
