;;;;;;;;;;;;;;;;;;;; 
;;;
;;; NOTE THAT SOME FUNCTIONS IN FUNCTIONS.LISP AND TRANSFORMATIONS.LISP (PACKAGE
;;; FE) ASSUME THAT PRECONDITIONS (EVEN SINGLE LITERAL PRECONDS) ARE
;;; CONJUNCTIONS STARTING WITH THE SYMBOL 'AND.
;;;

;;; Finished removing all forall, exists precondtions and conditional effects
;;; from operators in order to simplify the domain. Also changed all inference
;;; rules (DO-BLOCKADE, SPECIALIZE-INNEFFECTIVE-2-DESTROY, and
;;; SPECIALIZE-INNEFFECTIVE-2-ISOLATE) to operators. [cox 29may98]
...
;;;
;;; Belongs in functions.lisp. Created [cox 3jun98]
;;;
(defun my-equal (var const)
  (eq var (p4::object-name-to-object 
	   const
...
;    (<new-loc> (and LOCATION (diff <loc> <new-loc>)))
    (<pop1> (and POPULATION (my-equal <pop1> 'us-population1)))
    (<pop2> (and POPULATION (my-equal <pop2> 'us-population2)))
    (<pop3> (and POPULATION (my-equal <pop3> 'portuguese-population)))
    (<pop4> (and POPULATION (my-equal <pop4> 'romanian-population))))    
   (and
    ;; Removed forall clauses and replaced with overly specific set of literals. [cox 28may98]
    (removed-from-loc <force> <loc> <pop1>)
    (removed-from-loc <force> <loc> <pop2>)
    (removed-from-loc <force> <loc> <pop3>)
    (removed-from-loc <force> <loc> <pop4>)
...
#|
;;; This version removed all citizens of a particular nationality. Removed the
;;; conditional effects.
;;;
(OPERATOR EVACUATE-NON-COMBATANTS-BY-AIRLIFT
  (params <perimeter> <airport> <transport>
;;;Needed no longer and reduces search.
;  <new-loc>
	  <citizens>)
  (preconds
   ((<loc> COUNTRY)
    (<province> (and COUNTRY (diff <loc> <province>)))
;    (<new-loc> (and COUNTRY 
;			 (diff <loc> <new-loc>)))
    (<home-country> (and COUNTRY 
			 (diff <loc> <home-country>)))
    (<perimeter> EVAC-LOCATION) ; Was location
    (<airport> AIRPORT)
    (<transport> AIRLIFT)
    (<force> SPECIAL-FORCE-MODULE)
    )
...
;redundant    (airport-secure-at <loc> <airport>)
  ...
  (effects
   ((<citizens> POPULATION))
  ((if (and 
;	(province-of <loc> <province>) ;redundant
	(loc-at-p <citizens> <province>)
	(citizen-of <home-country> <citizens>))
   ((add  (removed-from-loc <force> <loc> <citizens>)
;;;	  (loc-at-p <citizens> <new-loc>)
	  )
    (del  (loc-at-p <citizens> <province>)))))) ;<province> was <loc>
  )
|#

(OPERATOR EVACUATE-NON-COMBATANTS-BY-AIRLIFT
  (params <perimeter> <airport> <transport> <citizens>)
  (preconds
   ((<loc> COUNTRY)
    (<province> (and COUNTRY (diff <loc> <province>)))
    (<citizens> POPULATION)
...
(OPERATOR SECURE-UNDEFENDED-PERIMETER
  ...
  (preconds
   ...
   (and 
    (loc-at <perimeter> <loc>)
    ;; Below handled by control-rule now
...
;;; Used to have single operator. [cox 27may98]
#|
(OPERATOR SECURE-PERIMETER
  ...
  (preconds
   ...
   (and 
    (loc-at <perimeter> <loc>)
    ;; Removed exists clauses and replaced with overly specific set of literals. [cox 29may98]
    (~ (is-deployed malayasian-brigade <perimeter>))
;    (~ (exists ((<enemy-force> enemy-ground-unit))
;	    (is-deployed <enemy-force> <perimeter>)))
    (is-deployed  <infantry-unit> <loc>)
    ))
...
(OPERATOR SECURE-DEFENDED-PERIMETER
  (params <infantry-unit> <perimeter> <loc> <enemy-force>)
  (preconds
   (
    (<loc> COUNTRY)
    (<perimeter> LOCATION)
    (<infantry-unit> (or INFANTRY SPECIAL-FORCE-MODULE))
    (<enemy-force> ENEMY-GROUND-UNIT)
    )
   (and 
    (loc-at <perimeter> <loc>)
    (made-ineffective-by <enemy-force> <infantry-unit>)
;;; The following is asserted by attack and cutoff operators
;    (is-deployed  <infantry-unit> <loc>)
    ))
  (effects
   ()
...
;      )
    ...
    (<enemy-force> (and ENEMY-GROUND-UNIT (my-equal <enemy-force> 'malayasian-brigade)))
    )
   (and 
    ;; Control rule now handles binding, but this is still here for case
    ;; replay.
    (loc-at <stronghold> <loc>) 
...
(OPERATOR PROVIDE-SECURITY-FOR-US-MILITARY-PERSONNEL
  ...
  (preconds
   ((<loc> COUNTRY)
    (<military-base1> (and LOCATION (my-equal <military-base1> 'CubiPt-NAS)))
    (<military-base2> (and LOCATION (my-equal <military-base2> 'SangleyPt-airbase)))
    )
   (and
    ;; Removed forall clauses and replaced with overly specific set of literals. [cox 29may98]
    (is-secure <military-base1>)
    (is-secure <military-base2>)
...
;;; NOTE that LOC stands for lines of communication. I did not know this
;;; originally.
;;;
(OPERATOR DEFEND-STRONGPOINTS-AND-LOCS
  ...
  (preconds
   ((<loc> COUNTRY)
    (<fort1> (and BUILDING (my-equal <fort1> 'Fort-Magsaysay)))
    (<fort2> (and BUILDING (my-equal <fort2> 'Fort-Bonifacio)))
    (<airport> (and AIRPORT (my-equal <airport> 'Mactan-Intl-Aprt)))
    )
   (and
    ;; Removed forall clauses and replaced with overly specific set of literals. [cox 29may98]
    (is-secure <fort1>)
...
;;; [cox 22may98]
(CONTROL-RULE SELECT-SUPPORT-BINDINGS
  (if (and (current-operator SUPPORT)
	   (type-of-object <a-unit> AIR-FORCE-MODULE)
	   (type-of-object <m> MISSION-NAME)
	   (true-in-state (mission-of <a-unit> <m>))
	   ))
  (then select bindings ((<mission> . <m>) (<air-unit> . <a-unit>) ))
  )
...
(CONTROL-RULE SELECT-DEPLOY-BINDINGS
  (if (and (current-operator DEPLOY)
	   (current-goal (is-deployed <a-unit> <location>))
;	   (type-of-object <a-unit> AIRCRAFT)
	   (true-in-state (is-active <a-unit>))
	   ;; Is this necessary since <air-unit> is determined by the goal
	   ;; (i.e., the effects)?
	   (false-in-state-forall-values (isa-C141 <a-unit>)) 
	   (true-in-state (loc-at <a-port> <location>))
	   ;; Note that in other implmentations of the domain, an operator may
	   ;; be able to repair an airport (i.e., make it usable).
	   (true-in-state (is-usable <a-port>))
	   ))
  (then select bindings 
	((<air-unit> . <a-unit>) (<loc> <location>) (<airport> <a-port>)))
  )


;;; [cox 31may98]
(CONTROL-RULE SELECT-ATTACK-BINDINGS
  (if (and (current-operator ATTACK)
	   (current-goal (is-destroyed-by <enemy> <infantry>))
	   (true-in-state (is-deployed <enemy> <shold>))
	   (true-in-state (loc-at <shold> <location>))
	   ))
  (then select bindings 
	((<enemy-force> . <enemy>) (<stronghold> . <shold>) 
	 (<infantry-force> . <infantry>) (<loc> . <location>)))
  )
...
(CONTROL-RULE SELECT-OP-4-IS-DEPLOYED-GOAL
...
;;; [cox 22may98]
(CONTROL-RULE SELECT-OP-4-NAVAL-UNIT-MOVEMENT
  (if (and (current-goal (is-deployed <naval-unit> <loc>))
	   (is-disjunct-types-p <naval-unit> CARRIER-BATTLE-GROUP SHIP)
	   ))
  (then select operator DEPLOY-SHIPS))


;;; [cox 27may98]
(CONTROL-RULE REJECT-OP-DEPLOY-SHIPS
  (if (and (current-goal (is-deployed <unit> <loc>))
	   (negation-p is-disjunct-types-p <unit> CARRIER-BATTLE-GROUP SHIP)
	   ))
  (then reject operator DEPLOY-SHIPS))

(defun negation-p (funct &rest args)
  (not (apply funct args))
  )

(defun is-disjunct-types-p (instance type1 type2)
  (or 
   (type-of-object instance type1)
   (type-of-object instance type2))
  )


;;; No longer legitimate because more than one operator exists to move ground
;;; units (e.g., send-dog-with-police). Would like to reject multiple
;;; operators, but this is not allowed. Instead, we just allow other rules to
;;; pick operators for moving ships and air units. This rule is commented
;;; out. [cox 22may98]
#|
...
(CONTROL-RULE SELECT-EVAC-BY-AIRLIFT-BINDINGS
  (if (and (current-operator EVACUATE-NON-COMBATANTS-BY-AIRLIFT)
	   (current-goal (removed-from-loc <f> <location> <cit>))
	   (true-in-state (province-of <location> <prov>))
	   (true-in-state (loc-at <peri> <prov>))
	   (true-in-state (loc-at <a-port> <prov>))
	   (true-in-state (near <a-port> <peri>))
	   (true-in-state (isa-c141 <trans>))
	   ;;Redundant
;	   (true-in-state (loc-at-p <cit> <province>))
	   ))
  (then select bindings 
	((<loc> . <location>) (<province> . <prov>) (<citizens> . <cit>) 
	 (<perimeter> . <peri>) (<airport> . <a-port>) (<transport> . <trans>) 
	 (<force> . <f>)))
  )
...
(CONTROL-RULE SELECT-SECURE-STRONGHOLD-BINDINGS
  ...
  (then select bindings ((<loc> . <z>) (<stronghold> . <s-hold>) 
			 (<infantry-force> . <X>) (<security-force> . <y>))))
...
(CONTROL-RULE SELECT-MEU-SOC
  (if (and (current-operator SECURE-UNDEFENDED-PERIMETER)
	   (current-goal (is-secure <Y>))
	   (type-of-object <Y> EVAC-LOCATION)
	   (is-disjunct-types-p <X> MEU-SOC INFANTRY)
	   (true-in-state (is-active <X>))
	   (true-in-state (is-ready <X>))
	   ))
...
(CONTROL-RULE SELECT-STRONGHOLD-OP-4-IS-SECURE-GOAL
  (if (and (current-goal (is-secure <Y>))
	   (type-of-object <Y> BUILDING)
	   ))
  (then select operator SECURE-STRONGHOLD)) 


(CONTROL-RULE SELECT-DEFENDED-PERIMETER-OP-4-IS-SECURE-GOAL
  (if (and (current-goal (is-secure <perimeter>))
	   (type-of-object <perimeter> LOCATION)
;	   (type-of-object <enemy-force> ENEMY-GROUND-UNIT)
	   (true-in-state (is-deployed <enemy-force> <perimeter>)
			  ((<enemy-force> ENEMY-GROUND-UNIT)))
	   ))
  (then select operator SECURE-DEFENDED-PERIMETER)) 


(CONTROL-RULE SELECT-UNDEFENDED-PERIMETER-OP-4-IS-SECURE-GOAL
  (if (and (current-goal (is-secure <perimeter>))
	   (type-of-object <perimeter> LOCATION)
;	   (type-of-object <enemy-force> ENEMY-GROUND-UNIT)
	   (false-in-state-forall-values (is-deployed <enemy-force> <perimeter>)
					 (<enemy-force> ENEMY-GROUND-UNIT))
	   ))
  (then select operator SECURE-UNDEFENDED-PERIMETER)) 
...
;;;
(OPERATOR SPECIALIZE-INNEFFECTIVE-2-DESTROY
  (mode lazy)
...
(OPERATOR SPECIALIZE-INNEFFECTIVE-2-ISOLATE
  (mode lazy)
  (params <unit> <enemy-unit>)
  (preconds
   ((<unit> FORCE-MODULE)
