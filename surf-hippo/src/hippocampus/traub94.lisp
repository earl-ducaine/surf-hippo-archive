...
(defun traub-CA3-branched (cell-name &key (cell-origin (list 0.0 0.0 0.0)) (soma-type *trb-94-soma-type*))
  ...
  (let ((cell (create-cell cell-name
			   :cell-type (create-cell-type "CA3-traub-pyramidal"
							:membrane-resistivity 50000.0
							:cytoplasmic-resistivity 200.0
							:soma-resistivity 50000.0
							:soma-specific-capacitance 0.75
							:dendrite-specific-capacitance 0.75
							:dendrite-v-leak -60.0
							:soma-v-leak -60.0) :cell-origin cell-origin)))
    (element-parameter *cell-type* 'g-axial-computation :average-g) ; This is what traub uses?
    (case soma-type
      (:symmetric-cylinder
       (create-symmetric-cylinder-soma cell-name 30.0
				       :location-1 '(0.0 -12.75 0.0) :location-2 '(0.0 12.75 0.0)))
    ...
    (let ((axons '("Axon-1" "Axon-2" "Axon-3" "Axon-4" "IS")))
      ...
      (add-channel-in-segment-with-density axons
					   '((na-ax-trb94 5000.0)
					     (Kdr-ax-trb94 2500.0))))
    ...
    (add-channel-in-segment-with-density '("shaft") '((na-trb94 30.0)
						      (Kdr-trb94 200.0)
    ...
    (add-channel-in-segment-with-density-motif "a" 0 '((ca-dendrite-trb94 10.0)
						       (ahp-dendrite-trb94 8.0)
						       (kc-dendrite-trb94 40.0))
					       cell)

    (add-channel-in-segment-with-density-motif "b" 0 '((ca-dendrite-trb94 10.0)
						       (ahp-dendrite-trb94 8.0)
						       (kc-dendrite-trb94  40.0)
						       (ka-trb94 5.0))
					       cell)

    (add-channel-in-segment-with-density-motif "c" 0 '((na-trb94 10.0)
						       (Kdr-trb94 150.0)
						       (ca-dendrite-trb94 10.0)
						       (ahp-dendrite-trb94  8.0)
						       (kc-dendrite-trb94  80.0)
    ...
    (add-channel-in-segment-with-density-motif "f" 0 '((na-trb94 30.0)
						       (Kdr-trb94 200.0)
						       (ca-dendrite-trb94 10.0)
						       (ahp-dendrite-trb94 8.0)
						       (kc-dendrite-trb94 80.0)
    ...
    (add-channel-in-segment-with-density-motif "g" 0 '((ca-dendrite-trb94 20.0)
						       (ahp-dendrite-trb94 8.0)
						       (kc-dendrite-trb94 40.0))
					       cell)

    (add-channel-in-segment-with-density-motif "h" 0 '((ca-dendrite-trb94 30.0)
						       (ahp-dendrite-trb94 8.0)
						       (kc-dendrite-trb94 120.0))
					       cell)

    (add-channel-in-segment-with-density-motif "i" 0 '((ca-dendrite-trb94 30.0)
						       (ahp-dendrite-trb94 8.0)
						       (kc-dendrite-trb94 120.0))
					       cell)
					     
    (add-channel-in-segment-with-density-motif "j" 0 '((ca-dendrite-trb94 10.0)
						       (ahp-dendrite-trb94 8.0)
						       (kc-dendrite-trb94 40.0))
					       cell)
  
    (add-channel-in-segment-with-density-motif "k" 0 '((ca-dendrite-trb94 10.0)
						       (ahp-dendrite-trb94  8.0)
						       (kc-dendrite-trb94 40.0))
					       cell)
...
(defun traub-CA3-B ()
  ...
  (setq *beep-after-gc* nil
	*plot-nodes* '("shaft" "c-1" "i-2221" "IS" "Axon-4")
	*plot-soma-nodes* '("traub-soma")
	;*plot-node-elements*  '("Axon-4" "IS" "virtual-soma" "i-2221")
;	*plot-soma-nodes* '("traub-soma")
	*pwl-isource-di-dt* 10.0
	; *use-variable-e-rev* NIL
	*absolute-voltage-error* 0.1
;	user-stop-time 100.0
;	*plot-isource-currents-p t
;	*plot-channel-currents-p t *plot-shell-1-concentrations-p t *plot-conc-particles-p t
;	*plot-particles-p t
	; *SOURCE-PULSE-LISTS* '(("traub-soma-isrc" (1.0 250.0 1.0)))
	)
  (let ((*isource-electrode-resistance* 0.0))
    (add-source *soma* 'isource)))

(push 'traub-CA3-B *CIRCUIT-FUNCTIONS*)
