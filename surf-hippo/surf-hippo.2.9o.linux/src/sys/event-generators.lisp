...
;;; *************** *************** *************** ***************
;;;
;;; These apply for axons and voltage controlled and light/light-auto controlled synapses.
...
(defun event-generator (event-element &optional only-slot-value)
  "Returns the value of the :EVENT-GENERATOR slot of EVENT-ELEMENT. If NIL, and ONLY-SLOT-VALUE is
NIL, then returns EVENT-ELEMENT."
...
;; All EVENT-ELEMENTS should be of the same type.
;; The 'event-followers parameters list includes the event generator.
  ...
  (when (and *SETUP-EVENT-GENERATORS-AND-FOLLOWERS*
    ...
    (let (event-element-control-designator sublist)
      ...
      (loop for event-element in event-elements
	    ...
	    unless (setq sublist (find event-element-control-designator
	    ...
	    ;;
	    ;; Thus for each unique control-designator, there will be an event-generator, taken as
	    ;; the first element of the associated list, with the remaining elements in the list
	    ;; assigned as event-followers.
...
(defun user-setup-event-generators-and-followers (event-generator event-followers)
  "Sets the :EVENT-GENERATOR slot of EVENT-GENERATOR and the EVENT-FOLLOWERS to EVENT-GENERATOR, and
assigns the list of EVENT-FOLLOWERS to EVENT-GENERATOR. Ensures that EVENT-GENERATOR is also a
member of the actual event-followers."
...
voltage-dependent or autonomous synapse."
	 ...
	 (if original-event-time-p
	     ...
	     (loop for time single-float in (axon-spike-times event-generator)
		   collect (+ (axon-propagation-delay event-element)
			      ...
			      time)))))
      (synapse
       ;; For synapses, the holder is another synapse whose :EVENT-TIMES slot holds the spike-times that the
       ;; current synapse uses. The holder is different than the generator in the case
       ;; where the conversion of the frequency-waveform into events is done synapse by synapse,
       ;; ie. when *CONVERT-LIGHT-RESPONSE-TO-EVENTS-FOR-EACH-SYNAPSE* is T. The holder is the
       ;; same as the generator when the generator events are used for constructing the open
       ;; activation times of all its followers.
