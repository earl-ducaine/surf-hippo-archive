...
(defstruct model
  ...
  output-data-structure-variables	; A list of global-var-list-of-structures, such as
					; *PLOT-CHANNEL-CURRENTS-STRUCTURES* and the like, that are
					; processed by the model's :SAVE-OUTPUT-DATA-ROUTINE

  output-data-enabled			; True or false depending on whether there is a non-nil
					; value in the :OUTPUT-DATA-STRUCTURE-VARIABLES list.
  
;;  output-data-keys			; Keys such as :VOLTAGE-DATA or :VOLTAGE-DERIVATIVE-DATA
					; used to reference data lists in individual elements.
  ...
  ;;     (CURRENT-VALUE-FUNCTION SOMA-VOLTAGE-VALUE)
  ...
  ;;     (CURRENT-VALUE-FUNCTION SOMA-VOLTAGE-DERIVATIVE-VALUE)
  ...
  output-model-name			; For models of types, e.g. channel types, this holds the
					; name of the associated model of the type instances (ie "channel").
  edit-routine
  eval-routine
  print-routine
  short-print-routine
...
(defstruct (node-double-floats (:type (vector double-float)))
  ...
  ; For membrane elements that are not evaluated at every time step.
;;  (element-const-jacobian 0.0d0	:type double-float)
  ...
  ;; For membrane elements that are not evaluated at every time step.
  ;; (element-const-current 0.0d0	:type double-float)
...
;; (defmacro node-double-floats-aref-element-const-jacobian (node-dfs) `(aref (the node-df-array ,node-dfs) 5))
...
;; (defmacro node-double-floats-aref-element-const-current (node-dfs) `(aref (the node-df-array ,node-dfs) 9))
...
;; (defmacro node-aref-element-const-jacobian (node-dfs) `(node-double-floats-aref-element-const-jacobian ,node-dfs))
...
;; (defmacro node-aref-element-const-current (node-dfs) `(node-double-floats-aref-element-const-current ,node-dfs))
...
;; (defmacro node-element-const-jacobian-double (node) `(node-aref-element-const-jacobian (node-double-floats ,node)))
...
;; (defmacro node-element-const-current-double (node) `(node-aref-element-const-current (node-double-floats ,node)))
...
(defstruct (node (:print-function (lambda (elt stream depth)
  ...
  color-index				; If an integer, used as color index
  
  (relative-location '() :type list)	; (X Y Z) relative to the cell origin.
  (absolute-location '() :type list)	; (X Y Z) in brain coordinates, equal to sum of :CELL :CELL-ORIGIN
...
(defmacro node-prt-v-index (node)
...
(defstruct (soma (:print-function (lambda (elt stream depth) (declare (ignore depth)) (format stream "<Soma ~a>" (element-name elt)))))
  ...
  ;;  (g-leak*v-leak 0.0d0 :type double-float) ; mS*mV Just to save a multiply in eval-soma
  ;;  (capacitance zero :type single-float)	; nF
...
; (setf (documentation 'soma-v-leak 'function) "Leak reversal potential in mV (requires explicit structure)")
(setf (documentation 'soma-capacitance 'function) "Membrane capacitance in nF (requires explicit structure)")
(setf (documentation 'soma-inherit-parameters-from-type 'function)
      "When T, membrane parameters are inherited from the cell-type (requires explicit structure).")

(defmacro soma-cell (soma)
  `(node-cell (soma-node ,soma)))
...
(defmacro segment-capacitance (segment)
    "Membrane capacitance in nF."
    `(segment-aref-capacitance (segment-guts ,segment)))
(defmacro segment-g-leak*v-leak (segment) `(segment-aref-g-leak*v-leak (segment-guts ,segment)))
(defmacro segment-g-axial (segment)
  "Axial conductance in uS."
  `(segment-aref-g-axial (segment-guts ,segment)))
(defmacro segment-g-leak (segment)
  "Membrane leak conductance in uS."
  `(segment-aref-g-leak (segment-guts ,segment)))
...
(defstruct (segment (:print-function (lambda (seg stream depth)
  ...
  dummy-proximal-node-location		;For segments that connect to the soma, this node just gives
					;the physical proximal endpoint location, relative to the
					;cell soma. Also for segments whose physical proximal node
					;location does not match with another element's location, so
					;that the electrical connection is not exactly congruent
  ...
  ;; intracellular (cytoplasmic) resistivity Ra.
    
  ;; (... (Ri-coeff . val) ...)


  ;; The following parameters are inherited from the cell-type when the segment is created. 
  ;;  (... (membrane-resistivity . val)...) Ohms Square Centimeter
  ;;  (...(specific-capacitance . val)...)  uF / Square Centimeter
  ;;  (...(cytoplasmic-resistivity . val)...) ; Ohm Centimeter
  ...
  ;; A simple method for including spines is to adjust segment capacitance and resistivity by factor
  ;; related to membrane area of spines. 

  ;;  (...(membrane-area-coefficient . 2.0)...)

  (branch-node-index nil :type (or null fixnum)) ; If this segment is connected to a Hines node,
					; then this is the index for that node. 
...
(setf (documentation 'segment-theta 'function)
      "Elevation angle relative to proximal branch orientationm, in radians (requires explicit structure).")
(setf (documentation 'segment-phi 'function)
      "Azimuth angle relative to proximal branch orientationm, in radians (requires explicit structure).")

(defmacro segment-cell (segment)
  `(node-cell (segment-node-2 ,segment)))
...
(defmacro isource-current (isrc)	; nA
  `(aref (the (simple-array single-float *) (isource-floats ,isrc)) 0))

(defstruct (isource (:print-function (lambda (elt stream depth)
				       (declare (ignore depth))
				       (format stream "<Current Source ~a>" (element-name elt)))))	     
  name
  ...
  ;;  This is a 2D list ([2,N], where N is the number of breakpoints), with time points in the first sublist
  ;;  and source values in the second sublist.
  ...
  (period zero	:type single-float)	; When PERIOD is not = 0, then the waveform for this source
					; is repeated after every PERIOD milliseconds.
...
(setf (documentation 'isource-resistance 'function)
      "Series resistance, in Mohms. The voltage measured at this node will be offset by the IR drop across
this resistance.")
...
(defstruct (vsource (:print-function (lambda (elt stream depth)
  ...
  ;;  (dvdt-double 0.0d0 :type double-float)
  (last-voltage zero :type single-float)
  (voltage zero	:type single-float)
  parameters				; Holds information such as reference pulse-list or waveform args
  )

(setf (documentation 'vsource-resistance 'function)
      "Series resistance, in Mohms, for the non-ideal vsource. Must be >0.")

;; ********************************************************************
;; SYSTEM-OF-DIFFERENTIAL-EQUATIONS Structure
;; ********************************************************************
...
(defstruct membrane-element-iv-parameters

  ;; ***************************************************
  ;; Gbar (:OHMIC models) or Permeability (:CONSTANT-FIELD models) Parameters
  ;; ***************************************************

  (iv-relation :ohmic)			; The interpretation and units for the following slots depend on
					; the value of :IV-RELATION as follows:
					...
					; :GBAR-SOURCE
					;
					;    :ABSOLUTE              Refer to :GBAR-REF slot
					;    :DENSITY               Refer to :GBAR-DENSITY slot
					;
					; :GBAR-REF        Fixed Conductance (uS)       Fixed Permeability (cm3/s)
					; :GBAR-DENSITY  Conductance Density (pS/um2) Permeability Density (1.0e-6cm3/s/um2)
					;
					; Note: pS/um2 = (0.1mS per square cm)

  (gbar-source :density)

  (gbar-ref 0.0 :type single-float)	; If :GBAR-SOURCE is :ABSOLUTE, then this channel or synapse type has a fixed
					; maximum conductance or permeability in units given above, independent of the cell
					; element that it is a part of.

  (gbar-density 0.0 :type single-float)	; Otherwise, :GBAR-DENSITY, in units given above, is used to determine the maximum
					; conductance or permeability for a specific channel or synapse's GBAR.

  	
  
  ;; When T, the above parameters are inherited from the channel-type or synapse-type when the type parameters are
  ;; edited - otherwise, the channel or synapse parameters are changed only by reference to specific
  ...
  ;; ***************************************************
  (use-defined-e-rev nil :type boolean)	; If T, or if the :ION-PERMEABILITIES is NIL, the :E-REV of
					; each channel or synapse instance refers to the 
					; channel or synapse type :E-REV slot. Otherwise, the :E-REV of a
					; channel or synapse is calculated from the type
					; :ION-PERMEABILITIES slot and the appropriate :E-species
					; slots of the cell type of the channel or synapse's cell.
  
  (e-rev 0.0 :type single-float)	; mV, when there is a fixed e-rev.

  (ion-permeabilities nil :type list)	; E.g. '((K 0.7) (CA 0.2) (NA 0.1)) The permeabilities
					; should add to 1.0. These values are used to determine what
					; proportion of the total ionic current may be assigned to a
					; concentration integrator of a given ionic species, and in
					; computing the reversal potential when :USE-DEFINED-E-REV
					; is NIL.

  (conc-int-type-params nil :type list) ; For each ion that passes through this channel or synapse
  ;; type, an entry for a concentration integrator type, for the shell (1 and/or 2) and the
  ;; proportion of the current associated with the shell, e.g.:

  ;; '((CA-IN (1 0.7) (2 0.24)) (K-EXTRA (1 1)))
  
  (variable-e-rev nil :type boolean)	; When T, e-rev for each channel or synapse is referenced from the
					; channel or synapse itself, not the type.

  (block nil :type boolean)		;;; When true, block channel or synapse type completely
...
(defstruct (membrane-element-iv-values (:type (vector double-float)))
  ...
  ;; This is an absolute value, before temperature correction, that is referenced only when the
  ;; membrane-element :INHERIT-PARAMETERS-FROM-TYPE slot is NIL. In this case, the units are
  ;; determined by the associated element type :IV-RELATION (:OHMIC or :CONSTANT-FIELD).
...
(defmacro memb-elt-iv-conductance (membrane-element-iv-values)
  `(aref (the memb-elt-iv-values-array ,membrane-element-iv-values) 0))
(defmacro memb-elt-iv-permeability (membrane-element-iv-values)
  `(aref (the memb-elt-iv-values-array ,membrane-element-iv-values) 0))
(defmacro memb-elt-iv-gbar (membrane-element-iv-values)
  `(aref (the memb-elt-iv-values-array ,membrane-element-iv-values) 1))
(defmacro memb-elt-iv-e-rev (membrane-element-iv-values)
  `(aref (the memb-elt-iv-values-array ,membrane-element-iv-values) 2))
(defmacro memb-elt-iv-current (membrane-element-iv-values)
  `(aref (the memb-elt-iv-values-array ,membrane-element-iv-values) 3))
(defmacro memb-elt-iv-gbar/perm-reference-value (membrane-element-iv-values)
  `(aref (the memb-elt-iv-values-array ,membrane-element-iv-values) 4))
...
(defstruct (channel-type (:print-function (lambda (type stream depth)
  ...
  (iv-parameters (make-membrane-element-iv-parameters))

  first-channel				; The first channel of this type
  
  (particle-types-and-powers nil :type list)
...
(defstruct (channel (:print-function (lambda (ch stream depth)
  ...
  next-channel				; The next channel of this type
  (block nil :type boolean)		; When true, block completely

  ;; Concentration integrators associated with this channel, with the shells and appropriate
  ...
  pre-synaptic-element			; When the voltage controlling this channel is referenced
					; from a node different from the channel's cell-element
					; different.

  ;; When T, some parameters are inherited from the channel-type when the channel-type parameters are
  ;; edited - otherwise, the channel parameters are changed only by reference to specific channels.
  ;; If :INHERIT-PARAMETERS-FROM-TYPE is NIL, then by default the GBAR-REFERENCE for this element is
  ;; in absolute terms.
...
(defstruct (particle-type (:print-function (lambda (elt stream depth)
  ...
  (z 0.0 :type single-float)		; particle valence
  ...
  ;;; *********************************************************************************************************
  ;;; For non :HH-EXT class particle types (including :HH class), explicit functions for the rate constants or
  ...
  ;;; for these slots are called with a single VOLTAGE argument (which is a single float).
  ;;; *********************************************************************************************************
  
  alpha-function
  beta-function

  (tau-coefficient 1.0 :type single-float) ; A scaling factor when tau(V) is derived from explicit functions of
					; alpha and beta.
  tau-function
  ...
  first-particle			; The first particle of this type
  
  ;;  (particles () :type list)		; All the particles of this type  
  parameters
  )
...
(defmacro particle-double-floats-aref-state-n+1 (particle-double-floats)
  `(particle-aref-state-n+1 ,particle-double-floats))
(defmacro particle-double-floats-aref-state-n (particle-double-floats)
  `(particle-aref-state-n ,particle-double-floats))
(defmacro particle-double-floats-aref-dsdt-n (particle-double-floats)
  `(particle-aref-dsdt-n ,particle-double-floats))
(defmacro particle-double-floats-aref-dsdt-n-1 (particle-double-floats)
  `(particle-aref-dsdt-n-1 ,particle-double-floats))
...
(defstruct (particle (:print-function (lambda (elt stream depth)
  ...
  conc-particle				; For certain particle types with a concentration
					; dependence, the link to this dependence is via this slot.
  channel
  vnode-point				; This points to the circuit node whose voltage controls this particle.

  next-particle				; The next particle of this type
...
(defstruct (conc-particle-type (:print-function (lambda (type stream depth)
  ...
  "The model for the concentration dependent particle types"
  name

  ;; The state equations of concentration particle types may be governed by the following possibilities:

  ;;
  ;;    Kinetic scheme whose forward (alpha) and backward (beta) rate constants are
  ;;    functions of concentration (:CLASS = :NTH-ORDER)
  ;;
  ;;    Kinetic scheme whose rate functions are arbitrary functions, for example of
  ;;    concentration and voltage (:CLASS = ?). 
  ;;
  ;;    Kinetic scheme whose rate functions are functions of concentration and
  ;;    voltage as given by Moczydlowski and Latorre 1983 (:CLASS = :ML).   
  ;; 
  ;;    State of particles are a static function of concentration (:CLASS =
  ;;    :STATIC), given by an array in the :CONC-DEPENDENCE slot. 
  ;;
  ...
  ;;   other         no           yes          yes       no             no     functions for alpha/beta or tau/ss

  ;; For the Moczydlowski and Latorre 1983 model the :PARAMETERS a-list has the following values:
  ;;
  ...
  (class :none)
  (alpha 0.0	:type single-float)	; Typically, for the forward rate constant, but for :ML model, for
					; the backward rate constant. 
  (beta 0.0	:type single-float)	; Typically, for the backward rate constant, but for :ML model, for
					; the forward rate constant. 
  (tau-0 0.0d0 :type double-float)
  (conc-power 1 :type fixnum)

  k1					; Voltage-dep arrays for the :ML model
  k4


  alpha-function			; The arguments for these functions are:
  beta-function				; 
  tau-function				; CONC^CONC-POWER VOLTAGE TYPE
  ss-function				;
					;
					; Assuming all of these arguments makes for a general scheme. The
					; CONC^CONC-POWER argument is the concentration of the
					; appropriate compartment raised to the conc-particle-type
					; :CONC-POWER (argument is a double-float). The VOLTAGE argument
					; is also a double-float. The TYPE is the conc-particle type. Any
					; or all of these arguements may be ignored be the
					; alpha-function, beta-function, etc.
  
  conc-dependence			; The single (non-keyword) double float argument for this function is:
					;
					;  CONC^CONC-POWER
					;
					; as described above.
  ...
  (shell 1 :type fixnum)		; Which concentration compartment controls the particle (1-3). If a
					; negative number (e.g. -1), then concentation is taken as the
					; average (or total) compartment concentration.
  ...
  first-particle			; The first conc-particle of this type
  ;;  (conc-particles () :type list)	; All the conc-particles of this type
  parameters)
...
(defmacro conc-particle-aref-state-n+1 (conc-particle-double-floats)
  `(particle-aref-state-n+1 ,conc-particle-double-floats))

(defmacro conc-particle-aref-state-n (conc-particle-double-floats)
  `(particle-aref-state-n ,conc-particle-double-floats))

(defmacro conc-particle-aref-dsdt-n (conc-particle-double-floats)
  `(particle-aref-dsdt-n ,conc-particle-double-floats))

(defmacro conc-particle-aref-dsdt-n-1 (conc-particle-double-floats)
  `(particle-aref-dsdt-n-1 ,conc-particle-double-floats))
...
(defstruct (conc-particle (:print-function (lambda (prt stream depth)
  ...
  cnode-point				; This points to the circuit node whose concentration (and possibly
					; voltage) controls this particle.
...
(defstruct (synapse-type (:print-function (lambda (elt stream depth)
					    (declare (ignore depth))
					    (format stream "<Synapse Type ~a>" (element-name elt)))))
  name

  (iv-parameters (make-membrane-element-iv-parameters))

  (control 'AUTO)
  ;; This synapse type is controlled by 'LIGHT, 'CHANNEL (the synaptic conductance is
  ;; calculated as if it is a channel, with the reference node for the voltage/concentrations
  ;; controlling the channel defined by the :PRE-SYNAPTIC-NODE of each synapse), 'VOLTAGE (the
  ;; voltage in some node triggers a stereotype conductance waveform), 'AUTO (fixed timing
  ;; parameters - events - that trigger a stereotype conductance waveform), or 'LIGHT-AUTO, in which
  ;; the events for an 'AUTO type are generated from a waveform determined by 'LIGHT synapse properties.
  ...
  ;; appropriate parameters. The SPATIAL-RF-FUNCTION entry, for light-dependent synapses,
  ;; specifies a function which returns a 2-D array that defines the spatial receptive field. If this
  ;; entry does not exist for a light-dependent synapse, then the spatial receptive field is a 2D
  ;; impulse. For the args to GAUSSIAN-RF, see the funspec. If the ADJUST-TO-RF-AREA entry is NIL or
  ;; missing, then the spatial integration is done without compensating for the support of the of the
  ;; SPATIAL RF 2-D array. Otherwise, the integration is multiplied by the area of the support. The
  ;; first case is used, for example, when the 2-D array sums up to 1.0 or 0.0 (e.g. if you have a DOG
  ;; RF, and the keyword :TOTAL-VOL 0 is included so that the array has zero overall weight).
  ...
  ;; LIGHT-INPUTS-ARRAY is a two-dimensional array storing canonical light input waveforms.
  ;; CONDUCTANCE-INPUTS-ARRAY is an array of the same size which holds canonical conductance
  ;; waveforms each of which, appropriately shifted and scaled, is referenced by a subset of the
  ;; synapses of this type. For N waveforms, and a simulation of T milliseconds, this is an N*T
  ;; single-float array.

  ;; The LIGHT-INPUTS-ARRAY is constructed as follows: For all the synapses of a given type, the
  ;; light-input profile is computed and compared with all the waveforms in LIGHT-INPUTS-ARRAY (it is
  ;; cleared at the start of the synapse input evaluations). If the current waveform is equivalent to
  ;; one of the stored waveforms, say the Mth waveform in the waveform array, within a shifting
  ;; factor (waveform-shift) and scaling factor (waveform-gain), then the index for the Mth waveform,
  ;; and the derived waveform-shift and waveform-gain values are stored in the current synapse. If
  ;; the current waveform cannot be matched to any of the stored waveforms, then the current waveform
  ;; is added to the waveform array, and the array index pointing to this waveform, and values of 0
  ;; (for waveform-shift) and 1.0 (for waveform-gain) are stored in the current synapse.

  ;; Once LIGHT-INPUTS-ARRAY has been constructed, each light input waveform is convolved with the
  ;; impulse response of the synapse type, and the result stored in CONDUCTANCE-INPUTS-ARRAY.

  ;; For a voltage-triggered or autonomous synapse (SYNAPSE-TYPE :CONTROL slot is 'VOLTAGE or 'AUTO):
  ...
  ;;  (waveform-time-fractional-part 0.0 :type single-float)
  
  ;; For a voltage-triggered (SYNAPSE-TYPE :CONTROL slot is 'VOLTAGE), these parameters have the same
  ;; meaning as those for axon types:
  ...
  first-synapse				; The first synapse of this type
...
(defstruct (synapse (:print-function (lambda (elt stream depth)
				       ...
				       (when (synapse-pre-synaptic-element elt)
					 (format stream ", pre-synaptic-elt ~a" (synapse-pre-synaptic-element elt)))
				       (format stream ">"))))
  name
  type
  cell-element				;What the synapse is a part of (soma or segment).

  next-synapse				; The next synapse of this type
  
  (block nil :type boolean)		; When true, block completely

  conc-ints-params			; Concentration integrators associated with this synapse,
					; with the shells and appropriate coefficients for calculating the
					; e-rev, e.g:

					; '((<soma-1-CA-IN> 1 0.6) (<soma-1-CA-IN> 2 0.2) (<soma-1-K-EXTRA> 1 0.2))

  pre-synaptic-element			; For light or light-auto controlled synapses, a receptive
					; field label. For voltage-dependent synapses, the controlling cell
					; element, and

  channel				; (possibly) the voltage-dependent channel of the post-synaptic membrane.

  (inherit-parameters-from-type t)	; When T, some parameters are inherited from the synapse-type when the
					; synapse-type parameters are edited - otherwise, the synapse
					; parameters are changed only by reference to specific synapses. 

  ;; If :INHERIT-PARAMETERS-FROM-TYPE is NIL, then by default the GBAR-REFERENCE for this element is
  ;; in absolute terms.
  
  ;; The following parameters are normally taken from the synapse-type when the synapse is created.
  ;; They are only relevent to the synapse instance when :inherit-parameters-from-type is NIL:

  ;; For a light dependent synapse these parameters will override location parameters implicit in
  ;; the synapse :NODE location and the offset information in the SYNAPSE-TYPE.
 
  ;;   (...(LIGHT-input-x . 0) (LIGHT-input-y . 0)...)


  ;; *********************************************************
  ;; For VOLTAGE, LIGHT-AUTO and AUTO controlled synapses ****
  ;; *********************************************************

  event-times				; This is a list of sf event initiation times (in milliseconds of real
					; time) for VOLTAGE, LIGHT-AUTO and AUTO controlled synapses - this
					; list is analagous to the :SPIKE-TIMES slot of axons.

  event-generator			; For VOLTAGE controlled synapse types, the synapse which acts as an
					; event detector for all other synapses of the same type and with the
					; same pre-synaptic node. For LIGHT and LIGHT-AUTO controlled synapse
					; types, the light response and event generation applied to this
					; synapse is used by all other synapses of the same type with the same
					; receptive field characteristics, defined by the synapse type and the
					; XY coordinates of the RF center.

  (delay 0.0 :type single-float)	; For autonomous and voltage synapses, the synapse type waveform is
					; applied at each event time plus this delay, in milliseconds.

  (fixnum-delay 0 :type fixnum)		; Calculated from the above value, in units of the waveform
					; timebase.

  transformed-events			; All events for autonomous synapses are scaled by the inverse of the
					; waveform time interval, sorted, and then put in a list of fixnums
					; stored here for the eval-auto-synapse routine. For voltage synapses,
					; this holds event times as they are detected, in a list of fixnums.

  ;; *********************************************************
  ;; For VOLTAGE controlled synapses *************************
  ;; *********************************************************

  (sub-threshold-time 0.0 :type single-float) ; The last time that the proximal node of the synapse was below
					; threshold, when the node is above threshold. When the node is below
					; threshold, then this is the current time.


  ;; *********************************************************
  ;; For LIGHT and LIGHT-AUTO controlled synapses ************
  ;; *********************************************************
  ...
  wave-ref				; Either -
					...
					;
  					;  (index shift gain) - List to reference pre-computed	
					;  conductance waveform array for light-dependent synapses,
					;  or voltage waveform for voltage-dependent synapses. This array
					;  must be type single-float, with length = simulation
					;  duration.
					;
					;  NIL - Synapse should not be evaluated during the
					;  simulation, since it is never activated.
...
#|
(defmacro synapse-delays (syn)
  `(synapse-event-times ,syn))
|#
...
;; All concentrations are in mM, and reflect concentrations of total ion (other than conc-int data,
;; which is free ion).
...
(defstruct (conc-int-type (:print-function (lambda (int stream depth)
  ...
  (class :MULTI-SHELL)			; Determines the integration method. Possibilties include
					; :MULTI-SHELL, :GENERAL, :FIRST-ORDER, :GENERIC

					; For :GENERIC, the CONC-INT-TYPE-DEF form must have either:
					;
					; C-N+1-FUNCTION, defined with a single argument being a
					; conc-int instance, and that returns a double-float
					; concentation in mM for the current time step t-N+1.
					;
					; DCDT-FUNCTION, defined with a single argument being a
					; conc-int instance, and that returns a double-float
					; derivative of the concentation in mM/ms for the current
					; time step t-N+1.
					;
					; If both are function types are given, then C-N+1-FUNCTION
					; takes precedent in the evaluation of the conc-ints.
  
  
  species				; E.g. CA, K, NA
  (valence 1.0 :type single-float)

  (enabled t)				; Concentration integrators of this type are only advanced
					; during the simulation when this is T.
  ...
  volumes				; A list of compartment names and either a number or
					; function for determining their volumes. These functions
					; must take an instance of a concentration integrator as a
					; single arg, and return the appropriate compartment volume
					; in um^3. For example:
  ;;
  ;; ((1 (lambda (cint)
  ;;	   (* (element-volume cint) .01))) 
  ;;  (2 (lambda (cint)
  ;;	   (* (element-volume cint) .09)))
  ;;  (3 2.345e3)
  ;;  (core (lambda (cint)
  ;;	      (* (element-volume cint) .5))))
  ;;   

  membrane-areas			; A list of compartment names and either a number or
					; function for determining their membrane areas. Normally
					; this will apply to shells 1 and 2 only, and are used for
					; calculating membrane areas for any pumps associated with a
					; given compartment. These functions must take an instance
					; of a concentration integrator as a single arg, and return
					; the appropriate membrane area in um^2. For example:
  ;;
  ;; ((1 (lambda (cint)
  ;;	   (* (element-area cint) .1))) 
  ;;  (2 2.34e1))

  diffusion-areas			; A list of compartment pairs and either a
					; number or function for determining their diffusional
					; areas. These functions must take an instance of a
					; concentration integrator as a single arg, and return the
					; appropriate membrane area in um^2. For example:
  ;;
  ;; (((1 3) (lambda (cint)
  ;;	       (* (element-area cint) .1))) 
  ;;  ((2 3) 1.342e2)
  ...
  (diffusion-distances 1.0e4)		; A single number or a list of compartment pairs and either
					; a number or function for determining their diffusional
					; distance. These functions must take an instance of a
					; concentration integrator as a single arg, and return the
					; appropriate distance in um. If only a single number, then
					; this applies to all compartment pairs. For example:
  ;;
  ;; (((1 3) (lambda (cint)
  ;;	       (/ (element-volume cint) (element-area cint))))
  ;;  ((2 3) 0.1)
  ;;  ((core 3) 0.2))
  ...
  ;; If either :VOLUMES, :DIFFUSION-AREAS or :MEMBRANE-AREAS is NIL, then the following
  ;; parameters are used to determine compartment volumes or areas -
  
  ;; If juxtamembrane-shell-thickness > 0, then this is the thickness of the interdigitated shells 1 and 2, microns
  ;; otherwise, the first (and only) shell volume is equal to the associated cell element volume.
  ...
  (inner-shell-thickness 0.25 	:type single-float) ; Thickness of shell 3, microns. If equal to 0,
					; then the volume of shell 3 is given by the cell element
					; volume minus the volume of the juxtamembrane shell. 
  ...
  ;; Either a list of compartment pair lists followed by their respective diffusion coefficient, or
  ;; a single number for all pairs. Diffusion coefficients are in cm^2 sec^-1. For example,
  ...
  ;; Concentration of :SPECIES on the other side of the integrator, if there is not another
					; integrator there. 
...
(defstruct (conc-int-double-floats (:type (vector double-float)))
  ...
  (total-conc-n 0.0d0 :type double-float) ; mM - used for conc-ints who control a particle which depends on
					  ; the concentration averaged over the entire element.
...
(deftype conc-int-df-array ()
  '(simple-array double-float (14)))
...
(defstruct (conc-int (:print-function (lambda (int stream depth)
  ...
  (enabled t)				; Integrator is only advanced during the simulation when this is T.
  enabled-for-this-simulation
  
  evaluate-total-concentration		; Used for conc-ints who control a particle which depends on
					; the concentration averaged over the entire element.
  ...
  transmembrane-integrator		; Concentration integrator for same species on opposite side of membrane. If
					; NIL, then transmembrane concentration of concentration integrator type
					; :SPECIES is given by that type's :TRANSMEMBRANE-CONCENTRATION.
  

  ;; For shell 1 and shell 2 (if it exists), the value in the :SHELL-x-PORES slot (where "x" is 1 or
  ;; 2) is a list of channels and synapses that communicate with the shell and a (double float)
  ;; current coefficient appropriate for that shell, e.g:

  ;; ((<CHANNEL-1> 0.34d0) (<CHANNEL-2> 0.65d0) (<SYNAPSE-1> 1.0d0) ...)

  ;; The current coefficient is the product of the channel's or synapse's relative permeability to the concentration
  ;; integrator type :SPECIES, the proportion of the channel or synapse current assigned to shell 1 or 2, as
  ;; appropriate, and a delta term. The former value is given in the :ION-PERMEABILITIES slot of the channel or
  ;; synapse type, and the latter value is given in the :CONC-INT-TYPE-PARAMS slot of the channel or synapse type.
  ;; The default is 1.0. The delta term is used when a channel which is actually distributed over several (say, N)
  ;; elements is assigned to a fewer (< N) number of elements, possibly only one. In this case delta, which is less
  ;; than or equal to one, compensates for the overestimate of the integrated current in the higher density channel
  ;; distribution. For example, if the total membrane area of the elements for which a given channel is postulated
  ;; to be assigned to is area-true, and the area of the element(s) that the channel is actually assigned to is some
  ...
  ;; ***********************************
  ;; For :GENERAL conc-int type conc-ints a list of N CONC-INT-DOUBLE-FLOAT arrays, one for each
  ;; state variable of the conc-int.
  ...
  ;; Constants of proportionality between either differences of compartments' (i and j) free
  ;; concentrations, or ionic current entering a compartment j, and d[X]_j/dt. For d[X]/dt, the
  ;; units are mM/ms. Thus, Betas for concentration differences are in units of 1/ms.
...
(defstruct (axon-type (:print-function (lambda (type stream depth)
  ...
  ;; An axonal spike is initiated when the proximal node of the axon is above :INPUT-THRESHOLD for at
  ;; least :SUPRA-THRESHOLD-DURATION-MIN milliseconds after :REFRACTORY-PERIOD milliseconds
  ;; following the last axonal spike.
  ...
  (output-waveform nil)
  (block nil :type boolean)		; When true, block axon type completely
...
(defstruct (axon-floats (:type (vector single-float)))
  ...
  (sub-threshold-time 0.0 :type single-float)) ; The last time that the proximal node of the axon
						 ; was below threshold, when the node is 
						 ; above threshold. When the node is below
						 ; threshold, then this is the current time.

(defmacro axon-voltage (axon)
  `(aref (the (simple-array single-float *) (axon-floats ,axon)) 0))

(defmacro axon-sub-threshold-time (axon)
  `(aref (the (simple-array single-float *) (axon-floats ,axon)) 1))
...
(defstruct (axon (:print-function (lambda (axon stream depth)
  ...
  (propagation-delay 1.0 :type single-float) ; ms, adjusted for temperature from axon-type reference value 



  (spike-times () :type list)           ; This is a list of sf axonal spike initiation times at the
					; proximal node of the axon. 

  event-generator			; The axon that will serve as an event detector for this axon, which must be
					; of the same type and with the same proximal-node. Similar to the same slot
					; for (voltage controlled) synapses.  
...
(defstruct (cell-type (:print-function (lambda (type stream depth)
  ...
  (soma-v-leak *e-leak* :type single-float) ; mV
  (dendrite-v-leak *dendrite-e-leak* :type single-float) ; mV

  (soma-resistivity *r-mem-soma* :type single-float) ; Ohms Square Centimeter
  (soma-shunt *soma-shunt* :type single-float) ; Ohms
  
  (membrane-resistivity *r-mem* :type single-float) ; Ohms Square Centimeter
  (cytoplasmic-resistivity *r-i* :type single-float) ; Ohm Centimeter
  
  (soma-specific-capacitance *cap-mem* :type single-float) ; uF / Square Centimeter
  (dendrite-specific-capacitance *cap-mem-dendrite* :type single-float) ; uF / Square Centimeter

  ;; In the :x-CONC-EXTRA-DEPENDENCE slots below, the value may be
  ;; :FIXED or :FOLLOWS-GLOBAL. :FOLLOWS-GLOBAL means that the associated :x-CONC-EXTRA slot is
  ;; updated with the value of the associated global variable *x-CONC-EXTRA*.
  
  ;; In the :E-x-DEPENDENCE slots below, the value may be :FIXED, :FOLLOWS-GLOBAL (the reversal
  ;; potential follows the appropriate global variable, or :FOLLOWS-CONCENTRATION (the reversal
  ;; potential is updated with the Nernst equation using *TEMPERATURE* and the cell type
  ;; :x-CONC-INTRA and :x-CONC-EXTRA slots.  
...
(defstruct (buffer-type (:print-function (lambda (int stream depth)
  ...
  (enabled t)				; buffers of this type are only advanced
					; during the simulation when this is T.
...
(deftype buffer-df-array ()
  '(simple-array double-float (2)))

(defmacro buffer-conc-n+1 (buffer)
  `(aref (the (simple-array double-float *) (buffer-double-floats ,buffer)) 0))
(defmacro buffer-conc-n (buffer)
  `(aref (the (simple-array double-float *) (buffer-double-floats ,buffer)) 1))
...
(defstruct (buffer (:print-function (lambda (int stream depth)
  ...
  (enabled t)				; Buffer is only advanced during the simulation when this is T.
  enabled-for-this-simulation
...
(defstruct (pump-type (:print-function (lambda (int stream depth)
  ...
  (enabled t)				; pumps of this type are only advanced
					; during the simulation when this is T.
...
(defmacro pump-density-n+1 (pump)
  `(aref (the (simple-array double-float *) (pump-double-floats ,pump)) 0))
(defmacro pump-density-n (pump)
  `(aref (the (simple-array double-float *) (pump-double-floats ,pump)) 1))
(defmacro pump-current (pump)
  `(aref (the (simple-array double-float *) (pump-double-floats ,pump)) 2))
...
(defstruct (pump (:print-function (lambda (int stream depth)
  ...
  (enabled t)				; Pump is only advanced during the simulation when this is T.
  enabled-for-this-simulation    
