/*
 * $Id: production.c,v 1.3 1994/08/23 10:36:43 portelli Exp $
 * $Log: production.c,v $
 * Revision 1.3  1994/08/23  10:36:43  portelli
 * For 6.2.4
 *
 * Revision 1.2  1993/11/21  17:04:56  soarhack
 * 6.1.1 checkin
 *
 * Revision 1.1  1993/06/17  20:47:45  jtraub
 * Released
 *
 * Revision 0.1  1993/06/17  20:22:54  jtraub
 * 6.1_checkin
 *
 * Revision 9.3  1993/05/10  20:31:49  jtraub
...
Changed  < to > 10/5/92*/
{
  ...
  if ((test_order_1 = CANONICAL_TEST_ORDER(c1->data.tests.value_test)) <
      (test_order_2 = CANONICAL_TEST_ORDER(c2->data.tests.value_test))) {
    return TRUE;
  } else if (test_order_1 == test_order_2 &&
           CANONICAL_TEST_ORDER(c1->data.tests.attr_test) <
           CANONICAL_TEST_ORDER(c2->data.tests.attr_test)) {
    return TRUE;
  }
  return FALSE;
}
