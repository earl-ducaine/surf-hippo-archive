...
;; *******************************************
;;
;; Light Driven Synapses and Stimuli 
;;
;; *******************************************

(defun light-controlled-p (element)
  "Returns T if ELEMENT is a LIGHT or LIGHT-AUTO controlled synapse or synapse type."
...
(defun initialize-light-synapse-setup ()
  ;; Some book-keeping, also for backward compatibility *light-stimulus-strength* is still here. Use
  ;; *light-stimulus-strength* for new code.
...
(defun active-light-synapses ()
  (flatten-list
   (loop for type in (synapse-types)
	 when (and (or (eq (synapse-type-control type) 'light)
  ...
  (loop for type in (get-synapse-types 'light)
	...
	(let ((number-syns (length (number-synapse-type-synapses type))))
	  (if (get-a-value 'light-inputs-list-of-arrays (synapse-type-parameters type))
	      (let ((length-syn-array (length (element-parameter type 'active-synapses)))
		    (length-light-list (length (element-parameter type 'light-inputs-list-of-arrays))))
		(format t "Light synapse type ~A has ~A active synapse~p (~A total) and ~A reference waveform~p.~%"
			(synapse-type-name type)
			length-syn-array length-syn-array
			number-syns
			length-light-list length-light-list))
...
(defun any-newbie-synapses (syns)
  (loop for syn in syns
	when (eq :newbie (synapse-wave-ref syn))
	do (return t)))
...
;;; SETUP-LIGHT-SYNAPSES Pre-computes the conductance waveform for light-dependent synapses. Note
;;; that light-dependent synapses which are never hit by the light stimulus have their
;;; synapse-waveform set to NIL. All time indices are taken in milliseconds.
(defun setup-light-synapses ()
  ...
  (when (and *light-stimulus* *enable-light*)
    (let ((*reuse-synapse-waveforms* (and *reuse-synapse-waveforms* (= *user-stop-time* *last-user-stop-time*))))
      ...
      (loop
       for type in (flatten-list (get-synapse-types 'light) (get-synapse-types 'light-auto))
       do (Clear-waveforms-from-light-synapse-types type)
       unless (synapse-type-block type) do ; Generate light-responses-ARRAY for the synapse types.
       (let ((synapses-of-type (synapses-of-type type)))
	 (when (and (not (synapse-type-block type))
		    (or (not (= (synapse-type-gbar-ref type) 0.0))
			(when (synapse-type-gbar-density type) (not (= (synapse-type-gbar-density type) 0.0))))
...
#|
;;; The following allows *SYNAPSE-NAMES-TO-DO-FIRST* to contain either a list of synapse names or a
;;; list of node names. For the first synapse of type TYPE referenced directly or indirectly in this
;;; list, that synapse in the the type-parameters slot (acons synapse 'fast-rf-ref-synapse) for the
;;; associated synapse type.  Right now this function is only for light synapses and for fast rf bar
;;; simulations. Returns the reference synapse is successful, NIL if no synapse of the right type is found.
(defun parse-synapses-to-do-first (type)
  (loop for syn in (flatten-list (loop for thing in (mapcar 'element *SYNAPSE-NAMES-TO-DO-FIRST*)
				       collect (typecase thing
						 (synapse thing)
						 (t (node-elements-of-type thing 'synapse)))))
...
(defun store-syn-type-input (input wave-ref input-1st input-last type)
  ...
  (let* ((params (synapse-type-parameters type))
	 ...
	 (LIGHT-INPUTS-1st (adjust-or-make-array (get-a-value 'LIGHT-INPUTS-1st params)
						 (list new-number-of-waves) 'fixnum))
	   
	 (LIGHT-INPUTS-last (adjust-or-make-array (get-a-value 'LIGHT-INPUTS-last params)
						  (list new-number-of-waves) 'fixnum))
	   
	 (LIGHT-INPUTS-base-shift (adjust-or-make-array (get-a-value 'LIGHT-INPUTS-base-shift params)
							(list new-number-of-waves) 'fixnum))
	   
	 (LIGHT-INPUTS-base-gain (adjust-or-make-array (get-a-value 'LIGHT-INPUTS-base-gain params)
						       (list new-number-of-waves) 'single-float)))

    (declare (fixnum input-index shift)
	     (single-float gain))
    ;;    (format t "~A store: wavelength ~D input-index ~D shift ~D gain ~A  input-1st ~A ~A~%"
    ;;            (synapse-type-name type) wavelength input-index shift gain  input-1st
    ;;            LIGHT-INPUTS-base-shift)
    ...
    ;;    (format t "STORE.. shift ~D  syn1st: ~d  1st-1: ~F 1st: ~F~%"
    ;;            (- shift)
    ;;            INPUT-1ST
    ;;            (aref INPUT (1- INPUT-1ST)) (aref INPUT INPUT-1ST))
...
;; GENERATE-LIGHT-ORIGIN-TRAJECTORY Generates the trajectory of moving light stimuli origin during
;; the simulation, as a list of xy points. Result into *LIGHT-ORIGIN-ARRAY*.
...
;;; SHIFT-SYNAPSE-WAVEFORM If a light synapse is identical to one whose input has been previously
;;; computed (OLD-SYN), and the stimulus is the same, just shifted in time, then we can quickly compute the
;;; input to the new synapse (NEW-SYN). 
...
;;; GET-SPATIAL-RF-ARRAY Returns spatial RF array for the synapse type. The list associated with
...
;;; SIMPLE-RF Returns a rectangular single-float array with a rotated (:ORIENTATION in degrees,
;;; default 0) simple-type receptive field (RF) whose amplitude is cosine modulated in the (rotated)
;;; x dimension and independent of the (rotated) y dimension. The wavelength of the cosine is twice
;;; :SUB-REGION-WIDTH (default 500), and the number of cycles is given by one half
;;; :NUMBER-SUB-REGIONS (default 3). The phase of the cosine is :PHASE degrees (default 0) with
;;; respect to the center of the RF. The cosine is multiplied by :AMPLITUDE (default 1), and the
;;; (rotated) y length is given by :SUB-REGION-LENGTH (default 1500). The cosine may be further
;;; modulated by a gaussian (standard deviation given by :GABOR-SD, in microns) along the (rotated)
;;; x dimension when :GABOR is non-nil. The :GRID-SIZE, :SUB-REGION-WIDTH and :SUB-REGION-LENGTH
;;; arguments are in microns. For rotated RFs, the array is sufficiently large to accomodate the
;;; entire RF, with the remainder of the array set to 0.0. Thus, for a RF with a single excitatory
;;; center subregion, two flanking inhibitory subregions, a subregion width of 500 microns, a
;;; subregion length of 1500 microns, and a grid size of 100 microns, rotated from the vertical by
;;; 20 degrees:

;;;     (SIMPLE-RF :orientation 20.0)


;;; It is probably a good idea to use "nice" arguments, i.e. dimensions that have integer ratios.
;;; Otherwise, check the resulting array to make sure that there are no unexpected edge effects or
;;; artifacts in the array. A quick check is to use:

;;;     (density-plot (SIMPLE-RF :orientation 20.0))

(defun simple-rf (&key (amplitude 1.0) (sub-region-width 500) (sub-region-length 1500)
		       (grid-size 100) (number-sub-regions 3) (orientation 0)
		       (phase 0)
		       (gabor nil) (gabor-sd 500.0)
		       rf-shape-only)
  (declare (optimize (safety 1) (speed 3) (space 1)))
...
;; Each of the arrays returned by GAUSSIAN-RF and DOG-RF are 5 * (sigma / GRID-SIZE um) on a side,
;; and assume a GRID-SIZE um square grid, where the key argument :GRID-SIZE default is 10um. Thus
;; (GAUSSIAN-RF 20 40 t) returns a 10 by 20 array.
...
(defun DOG-RF (center-sigma-x center-sigma-y surround-sigma-x surround-sigma-y center-surround-ratio
			      &key (angle 0.0) normalized-amp (grid-size 10.0)
			      total-vol
			      (gaussian-rf-width-in-sigmas *gaussian-rf-width-in-sigmas*))
  (declare (optimize (safety 1) (speed 3) (space 1)))
  (setq center-sigma-x (float center-sigma-x)
	...
	angle (float angle))
  (when total-vol
    (setq total-vol (float total-vol)))
  (let* ((gaussian-x-size (+ 1 (* 2 (truncate
    ...
    (dotimes (x gaussian-x-size)
      (dotimes (y gaussian-y-size)
	(declare (fixnum x y))
	(let* ((x-arg (the sf (* grid-size (the sf (- x x-midpoint)))))
	       (y-arg (the sf (* grid-size (the sf (- y y-midpoint)))))
	       (center-gauss-x (the sf (gaussian x-arg 0.0 center-sigma-x-sq)))
	       (center-gauss-y (the sf (gaussian y-arg 0.0 center-sigma-y-sq)))
	       (surround-gauss-x (the sf (gaussian x-arg 0.0 surround-sigma-x-sq)))
	       (surround-gauss-y (the sf (gaussian y-arg 0.0 surround-sigma-y-sq)))
	       (center-gauss-xy (the sf (* center-gauss-x center-gauss-y)))
	       (surround-gauss-xy (the sf (/ (the sf (* surround-gauss-x surround-gauss-y))
					     (the sf center-surround-ratio))))
	  ...
	  (setf (aref spatial-rf-array x y) gauss-xy))))
    (if total-vol
	(add-offset-2d-array-float spatial-rf-array
				   (the sf (/ (the sf (- total-vol (the sf (array-vol spatial-rf-array grid-size))))
					      (the sf (* (the sf (* grid-size grid-size))
							 (the sf (* gaussian-x-size gaussian-y-size)))))))
	spatial-rf-array)))
...
(defun annulus-stimulus (syn-rf-x-stimulation-frame syn-rf-y-stimulation-frame)
  ...
  (let ((distance-from-stimulus
	 (distance syn-rf-x-stimulation-frame syn-rf-y-stimulation-frame 0.0 0.0)))
...
;;; ON-BAR-STIMULUS This determines whether the bar is presently overhead the syn. Cooridinates are in
;;; the bar frame. For a moving bar the stimulation frame coordinates have been translated according
;;; to the bar's movement from its original position (in the bar frame's y-direction).
...
;;; SPOT-STIMULUS This determines whether the spot is presently overhead the syn. Cooridinates are
;;; in the spot frame. For a moving spot the stimulation frame coordinates have been translated
;;; according to the spot's movement from its original position (in the spot frame's y-direction).
...
;;; BAR-GRATING-STIMULUS This determines whether the moving bar grating is presently overhead the
;;; syn. Note that this function first translates the syn coordinates according to the bar original
;;; position, and then rotates the coordinates according to the tilt of the rotated bar (the bar
;;; frame). Coordinates are then translated according to the bar's movement from its original
;;; position (in the bar frame's y-direction).
...
;;; SINE-GRATING-STIMULUS This determines whether the moving sine grating is presently overhead the
;;; syn. Note that this function first translates the syn coordinates according to the bar original
;;; position, and then rotates the coordinates according to the tilt of the rotated bar (the bar
;;; frame). Coordinates are then translated according to the bar's movement from its original
;;; position (in the bar frame's y-direction).
...
;;; GENERATE-LIGHT-INPUT-ARRAY Returns nil if *light-input-waveform* is all zeros, ie the light
;;; never hits this synapse, else fills *light-input-waveform* with the light input over time for
;;; the light-dependent synapse syn. Thresholds the result of the spatial integration at each time
;;; point. References *LIGHT-ORIGIN-ARRAY*. Integration over synapse RF - spatial-rf array has a
;;; sigma-x by sigma-y um grid size, while the synapse and light origin coordinates are in units of
;;; 1um.
...
(defun synapse-rf-center-x (syn)
  (element-parameter syn 'light-input-x))

(defun synapse-rf-center-y (syn)
  (element-parameter syn 'light-input-y))

(defun set-synapse-rf-center-x (syn rf-center-x)
  (element-parameter syn 'light-input-x rf-center-x))

(defun set-synapse-rf-center-y (syn rf-center-y)
  (element-parameter syn 'light-input-y rf-center-y))
...
(defun plot-light-input (&optional syns)
  (loop for syn in (element (or (coerce-to-list syns)
				(choose-list-values
				 (coerce-to-list (element-name (active-light-synapses))) nil
				 :do-all-at-once nil :rank-margin 5 :direction :vertical
				 :label "Choose Light Synapses For Light Plotting"))
			    'synapse)
	when (and (synapse-wave-ref syn)
		  (or (eq 'light-auto (synapse-type-control (synapse-type syn)))
	...
	(when lights (plot-timed-data lights labels nil :delta-t 1.0 :x-max *user-stop-time* :x-min 0.0
...
(defun get-light-response-input-waveform (syn)
  ...
  (let* ((light-responses-ARRAY (element-parameter (synapse-type syn) 'light-responses-ARRAY))
    ...
    (loop for time fixnum from 0 to *user-stop-time*
	  ...
	  collect (* gain (row-major-aref (the (simple-array single-float (* *)) light-responses-ARRAY)
					  (+ row-major-aref-offset time shift)))
	  into out
