...
(defun concentration-clamp (element &optional concentration)
  "Turn off all concentration integrators associated with ELEMENT. If CONCENTRATION is a number [mM,
default NIL], then set steady-state value of the integrators to this value. Returns steady-state
concentration(s)."
  (let ((ss-concentration (when (numberp concentration) (s-flt concentration))))
    (loop for type in (delete-duplicates (coerce-to-list (element-type (cell-element-elements element 'conc-int))))
	  do (turn-off type)
	  when ss-concentration
	  do (element-parameter type 'resting-free-conc ss-concentration)
	  collect
	  (element-parameter type 'resting-free-conc))))

(defun concentration-clamp-off (element &optional concentration)
  "Turn on all concentration integrators associated with ELEMENT. If CONCENTRATION is a number [mM,
default NIL], then set steady-state value of the integrators to this value. Returns steady-state
concentration(s)."
  (let ((ss-concentration (when (numberp concentration) (s-flt concentration))))
    (loop for type in (delete-duplicates (coerce-to-list (element-type (cell-element-elements element 'conc-int))))
...
(defun default-diffusion-coefficient (type)
  "The default diffusion coefficient for the ionic species specified for the concentration integrator
type associated with TYPE, given by global variables such as *D_CA*, [cm^2 sec^-1]. If ion not
associated with a diffusion coefficient global variable, returns 0.0."
...
(defun conc-int-juxtamembrane-shell-thickness (cint)
  (conc-int-type-juxtamembrane-shell-thickness (conc-int-type cint)))

(defun conc-int-inner-shell-thickness (cint)
  (conc-int-type-inner-shell-thickness (conc-int-type cint)))
...
(defun conc-int-type-d-12 (type &optional value)
  (conc-int-type-d-shell-x-shell-y type 1 2 value))

(defun conc-int-type-d-13 (type &optional value)
  (conc-int-type-d-shell-x-shell-y type 1 3 value))

(defun conc-int-type-d-23 (type &optional value)
  (conc-int-type-d-shell-x-shell-y type 2 3 value))

(defun conc-int-type-d-3core (type &optional value)
  (conc-int-type-d-shell-x-shell-y type 3 'core value))

(defun conc-int-type-d-1core (type &optional value)
  (conc-int-type-d-shell-x-shell-y type 1 'core value))

(defun conc-int-type-d-2core (type &optional value)
  (conc-int-type-d-shell-x-shell-y type 2 'core value))

(defun conc-int-type-d-12core (type &optional value)
  (conc-int-type-d-shell-x-shell-y type 1 'core value))
	...
	(no-nils
	 (list
	  (when (conc-int-type-shell-2-p type)
	    (list '(1 2) (conc-int-type-d-12 type)))
	  (when (and (conc-int-type-shell-2-p type) (conc-int-type-shell-3-p type))
	    (list '(2 3) (conc-int-type-d-23 type)))
	  (when (conc-int-type-shell-3-p type)
	    (list '(1 3) (conc-int-type-d-13 type)))
	  (when (conc-int-type-core-p type)
	    (list '(1 core) (conc-int-type-d-1core type)))
	  (when (and (conc-int-type-shell-2-p type) (conc-int-type-core-p type))
	    (list '(2 core) (conc-int-type-d-2core type)))
	  (when (and (conc-int-type-shell-3-p type) (conc-int-type-core-p type))
	    (list '(3 core) (conc-int-type-d-3core type)))))))
...
(defun conc-int-type-distance-12 (type &optional value)
  (conc-int-type-distance-shell-x-shell-y type 1 2 value))

(defun conc-int-type-distance-13 (type &optional value)
  (conc-int-type-distance-shell-x-shell-y type 1 3 value))

(defun conc-int-type-distance-23 (type &optional value)
  (conc-int-type-distance-shell-x-shell-y type 2 3 value))

(defun conc-int-type-distance-3core (type &optional value)
  (conc-int-type-distance-shell-x-shell-y type 3 'core value))

(defun conc-int-type-distance-1core (type &optional value)
  (conc-int-type-distance-shell-x-shell-y type 1 'core value))

(defun conc-int-type-distance-2core (type &optional value)
  (conc-int-type-distance-shell-x-shell-y type 2 'core value))

(defun conc-int-type-distance-12core (type &optional value)
  (conc-int-type-distance-shell-x-shell-y type 1 'core value))
...
(defun parse-conc-int-type-diffusion-distances (type)
  (setf (conc-int-type-diffusion-distances type)
	(no-nils
	 (list
	  (when (conc-int-type-shell-2-p type)
	    (list '(1 2) (conc-int-type-distance-12 type)))
	  (when (and (conc-int-type-shell-2-p type) (conc-int-type-shell-3-p type))
	    (list '(2 3) (conc-int-type-distance-23 type)))
	  (when (conc-int-type-shell-3-p type)
	    (list '(1 3) (conc-int-type-distance-13 type)))
	  (when (conc-int-type-core-p type)
	    (list '(1 core) (conc-int-type-distance-1core type)))
	  (when (and (conc-int-type-shell-2-p type) (conc-int-type-core-p type))
	    (list '(2 core) (conc-int-type-distance-2core type))
	    (when (and (conc-int-type-shell-3-p type) (conc-int-type-core-p type))
	      (list '(3 core) (conc-int-type-distance-3core type))))))))
      ...
      (:general (let ((total-compartment-volume (element-parameter cint 'total-compartment-volume)))
		  (if (and total-compartment-volume (> (the sf total-compartment-volume) 0.0))
		      (loop for state in (conc-int-state-arrays cint)
			    ...
			    sum
			    (let ((shell-1-conc
				   (if n+1-p (conc-int-aref-shell-1-conc-n+1 state) (conc-int-aref-shell-1-conc-n state))))
			      (* (the sf volume)
...
(defun conc-int-type-shell-instantaneous-buffer-p (type &optional shell)
  (if shell
      ...
      (loop for shell from 1 to 3 when (> (conc-int-type-shell-instantaneous-buffer-ratio type shell) 0) do (return t))))
...
(defun document-conc-int-type (type)
  (let* ((type (element type 'conc-int-type))
    ...
    (when type
      ...
      (unless (= (conc-int-type-alpha-s type) 1)
	(format t "    (alpha-s . ~a)~%" (conc-int-type-alpha-s type)))
      (when (conc-int-type-volumes type)
	(format t "    (volumes . ~a)~%" (conc-int-type-volumes type)))
      (when (conc-int-type-membrane-areas type)
	(format t "    (membrane-areas . ~a)~%" (conc-int-type-membrane-areas type)))
      (when (conc-int-type-diffusion-areas type)
	(format t "    (diffusion-areas . ~a)~%" (conc-int-type-diffusion-areas type)))
      ...
      (format t "    (core-conc . ~a)~%" (conc-int-type-core-conc type))
      ...
      (format t "    (instantaneous-buffer-ratio . ~s)~%"
	      (no-nils
	       (list
		(list 1 (conc-int-type-shell-instantaneous-buffer-ratio type 1))
		(when (conc-int-type-shell-2-p type)
		  (list 2 (conc-int-type-shell-instantaneous-buffer-ratio type 2)))
		(when (conc-int-type-shell-3-p type)
		  (list 3 (conc-int-type-shell-instantaneous-buffer-ratio type 3)))
		(list :global
		      (conc-int-type-shell-instantaneous-buffer-ratio type :global)))))
      (cond-every
...
(defun print-conc-int-type (type)
  (let* ((type (element type 'conc-int-type))
	 (core-connected-to-shells (and (conc-int-type-core-p type)
					(not (and (= (conc-int-type-D-3core type) 0)
						  (= (conc-int-type-D-12core type) 0))))))
    (when type
	...
	(cond ((and (conc-int-type-shell-2-p type) (conc-int-type-shell-3-p type))
	       (format t "3 shells (interdigitated juxta-membrane pair and inner)~A~%"
		       (if core-connected-to-shells " + core" "")))
	...
	(if (= (conc-int-type-inner-shell-thickness type) 0)
      ...
      (when (conc-int-type-shell-2-p type)
	(format t
		"~%  Proportion of juxta-membrane shell assigned to shell 1 (alpha_s): ~,2e~%"
		(conc-int-type-alpha-s type))
	(format t "  Shells 1&2 interdigitation (partitioning) coefficient: ~,2e [1/um]"
		(conc-int-type-interdigitation-coefficient type)))    
      (format t "~%")
      (format t "  Resting free concentration: ~,2emM~%" (element-parameter type 'resting-free-conc))
      (case (conc-int-type-class type)
	(:multi-shell
	 (when (or (conc-int-type-shell-2-p type)
		   (conc-int-type-shell-3-p type)
...
(defun edit-first-order-cont-int-type (&optional type)
  (let* ((type (element (or type (loop for type in (conc-int-types)
				       when (eq (conc-int-type-class type) :FIRST-ORDER)
				       do (return type))
			    'conc-int-type)))
	 (dummy1 (conc-int-type-juxtamembrane-shell-thickness type))
    ...
    (choose-variable-values
     ...
     :text (ADD-LINEFEEDS-TO-STRING-LIST
	    (list (format nil "Single Compartment First Order ~A ~AIntegrator"
			  (if (conc-int-type-intra-p type) "Intracellular" "Extracellular")
			  (conc-int-type-species type))
    ...
    (setf (conc-int-type-instantaneous-buffer-enabled type) dummy14
	  ...
	  (conc-int-type-enabled type) dummy10
...
(defun edit-multi-shell-conc-int-type-diffusion-params (&optional type)
  (let* ((type (element (or type (loop for type in (conc-int-types)
				       ...
				       do (return type))
			    'conc-int-type)))
	 (dummy4 (conc-int-type-D-12 type))
    ...
    (choose-variable-values
     `((,(format  nil "Diffusion constants D are in cm^2/sec.") :comment)
       ,(when (conc-int-type-shell-2-p type)
	  '(dummy4 "D between shells 1 and 2 (D-12)" :float))
       ,(when (conc-int-type-shell-2-p type)
	  '(dummy23 "Distance between shells 1 and 2 [um]" :float))
       ,(when (conc-int-type-shell-3-p type)
	  '(dummy5 "D between shells 1 and 3 (D-13)" :float))
       ,(when (conc-int-type-shell-3-p type)
	  '(dummy24 "Distance between shells 1 and 3 [um]" :float))
       ,(when (and (conc-int-type-shell-2-p type) (conc-int-type-shell-3-p type))
	  '(dummy6 "D between shells 2 and 3 (D-23)" :float))
       ,(when (and (conc-int-type-shell-2-p type) (conc-int-type-shell-3-p type))
	  '(dummy25 "Distance between shells 2 and 3 [um]" :float))
       ,(when (and (conc-int-type-shell-3-p type) (conc-int-type-core-p type))
	  '(dummy7 "D between shell 3 and the core (D-3core)" :float))
       ,(when (and (conc-int-type-shell-3-p type) (conc-int-type-core-p type))
	  '(dummy26 "Distance between shell 3 and core [um]" :float))
	  ...
	  '(dummy28 "Distance between shell 2 and core [um]" :float))
       ,(when (and (not (conc-int-type-shell-3-p type)) (conc-int-type-core-p type))
	  '(dummy8 "D between shell 1 and the core (D-1core)" :float))
       ,(when (and (not (conc-int-type-shell-3-p type)) (conc-int-type-core-p type))
	  '(dummy27 "Distance between shell 1 and core [um]" :float))
       )
...
(defun edit-multi-shell-conc-int-type (&optional type)
  (let* ((type (element (or type (loop for type in (conc-int-types)
	 ...
	 (dummy12 (if *implicit-conc-int-integration* :implicit :explicit))
    ...
    (choose-variable-values
     `((dummy10 "Enable this concentration integrator type" :boolean)
       ...
       ,(when (conc-int-type-shell-3-p type)
	  `(dummy2 ,(concatenate-strings (format nil "Thickness of inner shell 3 [um]~%")
		     "(0 => inner shell occupies rest of element volume)") :float))
       ,(when (conc-int-type-shell-2-p type)
	  `(dummy3 ,(format nil "Interdigitated juxtamembrane shells ratio~%[area of shell 1 / area of shell 2]") :float))
       ,(when (conc-int-type-shell-2-p type)
	  `(dummy11 ,(concatenate-strings (format nil "Interdigitation coefficient [1/microns]~%")
		      "(value < 0 => shell 2 behind shell 1, diffusionally)") :float))
     ...
     :label (format nil "Edit Concentration Integrator Type ~a" (element-name type)))
    (unless dummy22
      (if dummy21 (create-conc-int-type type nil t)
	  (progn
...
(defun menu-for-conc-ints ()
  (mapcar 'edit-conc-int-type
	  (choose-list-values (namelist-of-all-things 'conc-int-type t)
			      nil :label "Select Concentration Integrator Types To Modify")))
...
;;; CREATE-CONC-INT-TYPE
(defun create-conc-int-type (type-symbol &optional actual-type-symbol update-parameters)
  (typecase type-symbol
    (conc-int-type (setq type-symbol (intern (conc-int-type-name type-symbol))))
    (string (setq type-symbol (intern type-symbol))))
  (let* ((type (unless actual-type-symbol
		 (if (conc-int-type-p type-symbol)
		     type-symbol
		     (gethash (string type-symbol) (CONC-INT-TYPE-HASH-TABLE)))))
	...
	(sim-error (format nil "~A has a recursive parent type!" parent-type-symbol)))
      (unless original-parameters (sim-error (format nil "Don't know anything about conc int type ~A!" type-symbol)))
      (when actual-type-symbol (setq type-symbol actual-type-symbol))
      (unless type (setq type (if parent-type-symbol
				  (create-CONC-INT-TYPE parent-type-symbol type-symbol update-parameters)
				  (make-CONC-INT-TYPE :name (string type-symbol)))))
      (when parent-type-symbol (push-element-parameter type 'parent-types parent-type-symbol))
      (setf (conc-int-type-class type) (or (get-a-value 'class original-parameters) :multi-shell))
      (update-element-parameters-with-new-parameters original-parameters type)
      ;; (setq original-parameters (update-element-parameters-with-new-parameters original-parameters type))
      (let* ((species (or (get-a-value 'species original-parameters) 'ca))
	     ;;	     (default-D-coeff (ion-diffusion-coefficient species))
	     (k-buffer (get-a-value 'K-buffer original-parameters))
	     (shells-w-instantaneous-buffer (get-a-value 'shells-w-instantaneous-buffer original-parameters))
	     (inst-buff-param-list (or (get-a-value 'instantaneous-buffer-ratio original-parameters)
				       (get-a-value 'instantaneous-buffer-ratios original-parameters)
				       (get-a-value 'shell-buffer-ratios original-parameters))))
	(setf (conc-int-type-volumes type) (get-a-value 'volumes original-parameters)
	      (conc-int-type-membrane-areas type) (get-a-value 'membrane-areas original-parameters)
	      (conc-int-type-diffusion-areas type) (get-a-value 'diffusion-areas original-parameters)
	      
	      (conc-int-type-diffusion-coefficient type) (or (get-a-value 'diffusion-coefficient original-parameters)
							     (get-a-value 'diffusion-coefficients original-parameters))



	      
	      (conc-int-type-diffusion-distances type) (or (get-a-value 'diffusion-distances original-parameters)
							   *default-conc-int-type-diffusion-distance*)

	      
	      (conc-int-type-instantaneous-buffer-enabled type)
	      (true-p (or (get-a-value 'K-buffer original-parameters)
			  inst-buff-param-list
			  (get-a-value 'instantaneous-buffer-enabled original-parameters)))

	      (conc-int-type-shell-1-instantaneous-buffer-ratio+1 type)
	      (s-flt (+ 1 (max 0.0 (or (and (numberp inst-buff-param-list) inst-buff-param-list)
				       (cadr (find 1 inst-buff-param-list :key 'car))
				       (and (find 1 shells-w-instantaneous-buffer) k-buffer)
				       (get-a-value 'SHELL-1-INSTANTANEOUS-BUFFER-RATIO original-parameters)
				       0.0))))
	      (conc-int-type-shell-2-instantaneous-buffer-ratio+1 type)
	      (s-flt (+ 1 (max 0.0 (or (and (numberp inst-buff-param-list) inst-buff-param-list)
				       (cadr (find 2 inst-buff-param-list :key 'car))
				       (and (find 2 shells-w-instantaneous-buffer) k-buffer)
				       (get-a-value 'SHELL-2-INSTANTANEOUS-BUFFER-RATIO original-parameters)
				       0.0))))
	      (conc-int-type-shell-3-instantaneous-buffer-ratio+1 type)
	      (s-flt (+ 1 (max 0.0 (or (and (numberp inst-buff-param-list) inst-buff-param-list)
				       (cadr (find 3 inst-buff-param-list :key 'car))
				       (and (find 3 shells-w-instantaneous-buffer) k-buffer)
				       (get-a-value 'SHELL-3-INSTANTANEOUS-BUFFER-RATIO original-parameters)
				       0.0))))
	      (conc-int-type-global-instantaneous-buffer-ratio+1 type)
	      (s-flt (+ 1 (max 0.0 (or (and (numberp inst-buff-param-list) inst-buff-param-list)
				       (cadr (find :global inst-buff-param-list :key 'car))
				       (and (find 'all shells-w-instantaneous-buffer) k-buffer)
				       (get-a-value 'global-INSTANTANEOUS-BUFFER-RATIO original-parameters)
				       0.0))))
	     
	      (conc-int-type-species type) species
	      (conc-int-type-valence type) (s-flt (or (get-a-value 'valence original-parameters) (ion-valence species))))

	(case (conc-int-type-class type)
	  (:general
	   (let ((compartments (get-a-value 'compartments original-parameters))) ; A list of labels
	     (element-parameter type 'compartment-shells (get-a-value 'compartment-shells original-parameters))
	     ...
	     (setf (conc-int-type-equation-coefficients-array type)
		   (make-differential-equation-a-array (get-a-value 'equation-coefficients original-parameters))
		   (conc-int-type-number-of-states type) (length compartments)))))
	(cond-every
	 ((or (assoc 'qten original-parameters) (assoc 'q10 original-parameters))
	  (setf (conc-int-type-q10 type) (s-flt (cdr (or (assoc 'qten original-parameters)
							 (assoc 'q10 original-parameters))))))
	 ((assoc 'reference-temp original-parameters)
	  (setf (conc-int-type-reference-temp type) (get-a-sf-value 'reference-temp original-parameters)))
	 ((assoc 'transmembrane-concentration original-parameters)
	  (element-parameter type 'transmembrane-concentration
			     (get-a-value 'transmembrane-concentration original-parameters)))		 
	 ((assoc 'pump-type-params original-parameters)
	  (setf (conc-int-type-pump-type-params type) (get-a-value 'pump-type-params original-parameters)))
	 ((assoc 'parameters original-parameters)
	  (setf (conc-int-type-parameters type)
		(nconc (get-a-value 'parameters original-parameters) (copy-list (conc-int-type-parameters type)))))
	 ((assoc 'intra-p original-parameters)
	  (setf (conc-int-type-intra-p type) (get-a-value 'intra-p original-parameters)))
	 ((and (not (eq (conc-int-type-class type) :first-order))
	       (or (assoc 'shell-2-p original-parameters) (assoc 'shell2-p original-parameters)))
	  (setf (conc-int-type-shell-2-p type) (or (get-a-value 'shell-2-p original-parameters)
						   (assoc 'shell2-p original-parameters))))
	 ((and (not (eq (conc-int-type-class type) :first-order))
	       (or (assoc 'shell-3-p original-parameters) (assoc 'shell3-p original-parameters)))
	  (setf (conc-int-type-shell-3-p type) (or (get-a-value 'shell-3-p original-parameters)
						   (get-a-value 'shell3-p original-parameters))))
	 ((and (not (eq (conc-int-type-class type) :first-order))
	       (assoc 'core-p original-parameters))
	  (setf (conc-int-type-core-p type) (get-a-value 'core-p original-parameters)))
	 ((assoc 'juxtamembrane-shell-thickness original-parameters)
	  (setf (conc-int-type-juxtamembrane-shell-thickness type)
		(get-a-sf-value 'juxtamembrane-shell-thickness original-parameters)))
	 ((assoc 'inner-shell-thickness original-parameters)
	  (setf (conc-int-type-inner-shell-thickness type) (get-a-sf-value 'inner-shell-thickness original-parameters)))
	 ((assoc 'alpha-s original-parameters)
	  (setf (conc-int-type-alpha-s type) (get-a-sf-value 'alpha-s original-parameters)))
	 
	 ((assoc 'core-conc original-parameters)
	  (setf (conc-int-type-core-conc type) (get-a-sf-value 'core-conc original-parameters)))
	 ((assoc 'interdigitation-coefficient original-parameters)
	  (setf (conc-int-type-interdigitation-coefficient type)
		(get-a-sf-value 'interdigitation-coefficient original-parameters)))
	 ((assoc 'tau original-parameters) (element-parameter type 'tau (get-a-sf-value 'tau original-parameters))))

	(parse-conc-int-type-diffusion-distances type)
	(parse-conc-int-type-diffusion-coefficient type)


	(element-parameter type 'resting-free-conc (s-flt (or (get-a-value 'resting-free-conc original-parameters)
							      (get-a-value 'core-conc original-parameters)
							      (conc-int-type-core-conc type))))

	(setf (gethash (conc-int-type-name type) (CONC-INT-TYPE-HASH-TABLE)) type)))
    (setq *conc-int-type* type)
    type))


(defun revamp-conc-int-type-parameters ()
  "Run through all the current conc-int types and update their parameters according to the current
parameters in the conc-int type parameter library." 
  (loop for type in (conc-int-types) do (create-conc-int-type (element-name type) nil t)))
...
(defun create-conc-int (cell-element type-sym)
  (let* ((node (element-node (element-cell-element cell-element)))
    ...
    (or
     ...
     (gethash name (CONC-INT-HASH-TABLE))
	 ...
	 (:general
	  (setf (conc-int-state-arrays cint)
		(loop for i from 1 to (conc-int-type-number-of-states type) collect (make-conc-int-double-floats)))
...
(defun set-state-volumes (cint)
  (let ((type (conc-int-type cint))
    ...
    (element-parameter cint 'compartment-volumes
		       (loop for volume-function in (conc-int-type-volumes type)
			     collect (if volume-function (s-flt (funcall volume-function cint)) 0.0)))
    (element-parameter
     cint
     'total-compartment-volume
     (s-flt (or (loop for volume in (element-parameter cint 'compartment-volumes) sum volume) 0.0)))
    nil))

;; For assigning conc-ints to channels and synapses - setting the :CONC-INTS-PARAMS slot. This
;; information is used for evaluating the reversal potential of the channel or synapse. 
(defun parse-conc-int-info-for-element (type cell-element)
  (let ((iv-parameters-structure (element-iv-parameters type)))
    (loop for conc-int-type-param in (or (element-parameter type 'CONC-INT-TYPE-E-REV-PARAMS)
					 (membrane-element-iv-parameters-conc-int-type-params iv-parameters-structure))
			   ...
			   (list cint (car shell-perm)
				 (d-flt (* (cadr shell-perm)
					   (cadr (find (conc-int-type-species (conc-int-type cint))
						       (membrane-element-iv-parameters-ion-permeabilities
							iv-parameters-structure)
						       :key 'car))))))))))

(defun conc-int-all-pumps (cint)
  (concatenate 'list
	       (conc-int-shell-1-pumps cint)
	       (conc-int-shell-2-pumps cint)
	       (conc-int-shell-3-pumps cint)))

(defun remove-conc-int-from-pump (conc-int)
  (loop for pump in (conc-int-all-pumps conc-int) when (eq (pump-conc-int pump) conc-int)
	do (setf (pump-conc-int pump) nil)))

(defun remove-conc-int-from-particle (conc-int)
  (loop for prt in (conc-particles) when (eq (conc-particle-conc-int prt) conc-int)
	do (setf (conc-particle-conc-int prt) nil)))

(defun remove-conc-int-type-from-particle-type (conc-int-type)
  (loop for type in (conc-particle-types) when (eq (conc-particle-type-conc-int-type type) conc-int-type)
	do (setf (conc-particle-type-conc-int-type type) nil)))

;;; **************************************************************
;;; Setup Functions
;;; **************************************************************

(defun update-conc-int-type-q10s ()
  (loop for type in (CONC-INT-TYPEs) do
	(setf (conc-int-type-q10-rate-factor type) (element-q10-rate-factor type))))


(defun remove-conc-int-type-lists (type)
  (REMOVE-element-PARAMeters type '(conc-int-variable-e-rev-flags active-conc-ints)))
				    
(defun revamp-conc-int-type-lists (type conc-ints &optional inclusion)
  (when inclusion (setq conc-ints (loop for cint in conc-ints when (funcall inclusion cint) collect cint)))
  (when conc-ints
    (element-parameter
     type 'conc-int-variable-e-rev-flags
     (loop for conc-int in conc-ints collect
	   (loop for elt in (get-node-elements-of-type conc-int '(channel synapse))
		 when (typecase elt
...
(defun conc-int-has-active-channel (cint)
  (loop for pore in (conc-int-pores cint)
	unless (pore-block-p pore) do (return t)))

(defun conc-int-pores (cint)
  (loop for shell-pores in (list (conc-int-shell-1-pores cint) (conc-int-shell-2-pores cint))
	nconc (loop for pore-info in shell-pores
		    collect (conc-int-shell-pore pore-info))))

(defun active-conc-ints ()
  (loop for conc-int in (conc-ints)
	when (conc-int-active-p conc-int)
	collect conc-int))

(defun conc-int-active-p (conc-int)
  "Only true when the CONC-INT will actually be evaluated - requires that not only the CONC-INT and
it's type be enabled, but also that the associated channel is active."
  (let* ((conc-int (element conc-int 'conc-int))
	 (conc-int-type (element-type conc-int)))
    (and conc-int
	 (conc-int-enabled conc-int)
	 (conc-int-has-active-channel conc-int)
	 (conc-int-type-enabled conc-int-type))))
...
(defun setup-conc-ints ()
  ...
  (loop for type in (conc-int-types) do
	...
	(loop for cint in (conc-int-type-conc-ints type) do (setf (conc-int-enabled-for-this-simulation cint) nil)))
  (loop for ch-type in *channel-type-list* do
	(loop for conc-int-type-param in (channel-type-conc-int-type-params ch-type) do
	      (setf (conc-int-type-enabled-for-this-simulation (element (car conc-int-type-param) 'conc-int-type)) t)))
  (loop for type in (conc-int-types) when (conc-int-type-enabled-for-this-simulation type) do
	(loop for cint in (conc-int-type-conc-ints type) do
	      (setf (conc-int-enabled-for-this-simulation cint) (conc-int-has-active-channel cint))))
  (setq *conc-int-type-list*
	(no-nils
	 (loop for type in (conc-int-types)
	       when (and (conc-int-type-enabled type) (conc-int-type-enabled-for-this-simulation type))
	       collect
	       (let ((conc-ints (loop for conc-int in (conc-int-type-conc-ints type)
				      when (and (conc-int-enabled conc-int) (conc-int-enabled-for-this-simulation conc-int))
				      collect conc-int)))
		 (when conc-ints
		   ...
		   type)))))
  (loop for type in (conc-int-types)
	do (case (conc-int-type-class type)
	     (:multi-shell
	      (when (and *implicit-conc-int-integration*
			 (not (conc-int-type-shell-3-p type))
			 (not (= (conc-int-type-D-12core type) 0)))
		...
		(setq *implicit-conc-int-integration* nil)))))
  nil)
...
;; SET-SHELL-E-REVS Sets the reversal potential of the ion integrated by the concentration
;; integrator CINT according to the concentration in the CINT's shells at *SIM-TIME-N* (i.e. the
;; result of the previous time step).
(proclaim '(inline set-shell-e-revs))
(defun set-shell-e-revs (cint)
  (declare (optimize (safety 0) (speed 3) (space 1)))
  (let* ((type (conc-int-type cint))
	 (e-rev-function (element-parameter-fast 'e-rev-function (conc-int-type-parameters type))))
    (if e-rev-function
	...
	(let* ((transmembrane-conc (conc-int-transmembrane-conc cint))
	  ...
	  (setf (conc-int-e-rev-shell-1 cint)
		(nernst-potential-conc-double-floats shell-1-inner shell-1-outer valence))
	  (when (conc-int-type-shell-2-p type)
	    (setf (conc-int-e-rev-shell-2 cint) 
		  (nernst-potential-conc-double-floats shell-2-inner shell-2-outer valence))))))
  nil)
...
(defun conc-int-membrane-current-component (cint &optional (shell 1))
  "Returns the concentration derivative [mM/ms], for non :GENERIC integrators, or the current [nA] for
:GENERIC integrators, for the compartment SHELL of CINT that is due to that compartment's associated
:SHELL-PORES and pumps. The coefficient :BETA-CURRENT-sh, specific for a given CINT, converts
channel current (nA) to d[x]/dt (mM/ms). :BETA-CURRENT-sh = 1 for :GENERIC integrators."
  (declare (optimize (safety 0) (speed 3) (space 1)))
...
(defun implicit-eval-multi-shell-conc-int (cint)
  ...
  (let* ((c1 (conc-int-shell-1-conc-n cint))
    ...
    (setf (conc-int-shell-2-conc-n+1 cint)
	  (/ (+ (* a (- (* z2 i)
			(* z3 f)))
		(* d (- (* z3 c)
			(* z1 i)))
    ...
    (setf (conc-int-shell-3-conc-n+1 cint)
	  (/ (+ (* a (- (* ee z3)
			(* h z2)))
		(* d (- (* h z1)
			(* b z3)))
...
(defun implicit-eval-multi-shell-conc-int (cint)
  (if (= (conc-int-Beta-core-3 cint) 0)
...
(defun eval-generic-dcdt-conc-int (cint dcdt-function)
  ...
  (let* ((c1 (conc-int-shell-1-conc-n cint))
    ...
    (setf (conc-int-shell-1-conc-n+1 cint)
	  (max-double-macro (conc-int-core-conc-double cint) (+ c1 (* delta-t dcdt (conc-int-type-q10-rate-factor type)))))
    nil))
...
(defun eval-generic-c-n+1-conc-int (cint c-n+1-function)
  (declare (optimize (safety 0) (speed 3) (space 1))
	   (type conc-int cint))
  (setf (conc-int-shell-1-conc-n+1 cint)
	(max-double-macro (conc-int-core-conc-double cint) (funcall c-n+1-function cint)))
  nil)
...
(defun eval-multi-shell-conc-int (cint)
  (if *implicit-conc-int-integration*
      (IMPLICIT-EVAL-MULTI-SHELL-CONC-INT cint)
      (EXPLICIT-EVAL-multi-shell-CONC-INT cint))
  nil)
...
(defun eval-first-order-conc-ints (type cints e-rev-flags)
  (let* ((params (conc-int-type-parameters type))
	 (tau (/ (the sf (or (element-parameter type 'tau) 0.0))
		 (conc-int-type-q10-rate-factor type))))
    (loop for cint in cints for e-rev-flag in e-rev-flags do
...
(defun eval-multi-shell-conc-ints (type cints e-rev-flags)
  (loop for cint in cints for e-rev-flag in e-rev-flags do
	(eval-multi-shell-conc-int cint)
	(finish-conc-int-eval cint e-rev-flag))
  nil)
...
(defun eval-all-conc-ints ()
  (declare (optimize (safety 0) (speed 3) (space 0) (compilation-speed 0)))
  (loop for type in *conc-int-type-list* do
	(let* ((type-params (conc-int-type-parameters type))
	  ...
	  (case (conc-int-type-class type)
	    ...
	    (:multi-shell (eval-multi-shell-conc-ints type cints e-rev-flags))
...
(defun advance-general-conc-int (cint)
  (declare (optimize (safety 0) (speed 3) (space 0) (compilation-speed 0)))
  (loop for state in (conc-int-state-arrays cint) do
	(setf (conc-int-aref-shell-1-conc-n state) (conc-int-aref-shell-1-conc-n+1 state)))
...
;;; SET-CONC-INTEGRATORS-PARAMETERS This must be called after the cell element dimensions are known.
...
;; JUXTAMEMBRANE-SHELL-VOLUME In um^3.
(defun juxtamembrane-shell-volume (cint &optional
					(element-concentration-volume (element-concentration-volume cint))
					element-area)
  (if (> (conc-int-juxtamembrane-shell-thickness cint) 0)
      (min element-concentration-volume
	   (* (conc-int-juxtamembrane-shell-thickness cint) ; um
	      (or element-area (element-area cint)))) ; um2
...
(defun conc-int-volume (cint shell &optional element-conc-volume element-area)
  ...
  (let ((cint (element cint 'conc-int)))
	...
	(if (not (conc-int-type-shell-p type shell))
	    0.0d0
	    (d-flt
	     (typecase volume-function
	       (function (funcall volume-function cint))
	       (number volume-function)
		...
		(let ((juxtamembrane-volume (juxtamembrane-shell-volume cint element-conc-volume element-area)))
		  (case shell		
		    ...
		    (3 (let* ((total-conc-vol (or element-conc-volume (element-concentration-volume cint)))
			      ...
			      (inner-shell-volume (* (conc-int-inner-shell-thickness cint)
...
;; SHELL-1-VOLUME In um^3.
(defun SHELL-1-VOLUME (cint &optional element-conc-volume element-area)
  (conc-int-volume cint 1 element-conc-volume element-area))

;; SHELL-2-VOLUME In um^3.
(defun SHELL-2-VOLUME (cint &optional element-conc-volume element-area)
  (conc-int-volume cint 2 element-conc-volume element-area))

;; SHELL-3-VOLUME Returns a double float in um^3.  
(defun shell-3-volume (cint &optional element-conc-volume element-area)
  (conc-int-volume cint 3 element-conc-volume element-area))
...
(defun set-conc-integrator-type-parameters (name type)
    (declare (ignore name type)))
    
(defun conc-int-shell-membrane-area (cint shell &optional element-area)
  "Returns the surface area of one face of SHELL of CINT in um^2. Optional ELEMENT-AREA is area of the
associated cell element in um^2."
  (let ((cint (element cint 'conc-int)))
    (when cint
      (let* ((type (conc-int-type cint))
	     (area-function (cadr (assoc shell (conc-int-type-membrane-areas type)))))
	(typecase area-function
	  (function (funcall area-function cint))
...
(defun conc-int-diff-area (cint shell-x shell-y &optional element-area)
  "Returns the double-float diffusional area between SHELL-X and SHELL-Y of CINT in cm^2. Optional ELEMENT-AREA is
area of the associated cell element in um^2."
  (let ((type (element-type cint))
	...
	(d-flt
	 (typecase diff-area-function
	   (number diff-area-function)
	   (function (funcall diff-area-function cint))
...
(defun interdigitation-area (cint &optional element-area)
  "Returns the diffusion membrane area between shells 1 and 2 of CINT in um^2 for the :MULTI-SHELL
class. Optional ELEMENT-AREA is area of the associated cell element in um^2."
  (let ((type (conc-int-type cint))
...
(defun set-conc-integrator-parameters (name cint)
  ...
  (let* ((type (conc-int-type cint))
	 ...
	 (total-conc-vol (element-concentration-volume cell-element)) ; um^3
    ...
    (when (conc-int-type-core-p type)
      (conc-int-core-volume cint) (* 1.0d12 core-vol-cm3)) ; um3
...
(defun set-conc-int-pumps (cint)
  (mapcar 'set-pump-area (conc-int-all-pumps cint)))


(defun set-conc-int-current-elements (conc-int)
  ;; Find all the channels and synapses that this integrator should be looking at and
  ;; collect these elements along with their current coefficients appropriate for shell 1 or 2.
  (let* ((type (conc-int-type conc-int))
	 (cell-element (conc-int-cell-element conc-int))
	 (conc-int-type-sym (read-from-string (conc-int-type-name type))))
    ...
    (loop for elt in (node-elements (element-node cell-element)) do
	  (loop for conc-int-type-param in (element-conc-int-type-params (element-type elt))
		when (eq conc-int-type-sym (the symbol (car conc-int-type-param))) do
		(loop for conc-int-type-shell-param in (cdr conc-int-type-param) do
			...
			(case (the fn (car conc-int-type-shell-param))
			  ...
			  (2 (if (conc-int-type-shell-2-p type)
				 ...
				 (sim-error
				  (format nil "Setting up ~A with channels that expect shell 2!" conc-int)))))))))))
  ...
  (let* ((type (element-type cint 'conc-int))
    ...
    (case (conc-int-type-class type)
	 ...
	 (format t "   Diffusion area [cm^2] between shell pairs:~%   "))
       (when (and (conc-int-type-shell-2-p type) (> (conc-int-type-D-12 type) 0))
	 (format t "  A-12 ~,2e " (conc-int-diff-area cint 1 2)))
       (when (and (conc-int-type-shell-3-p type) (> (conc-int-type-D-13 type) 0))
	 (format t "  A-13 ~,2e " (conc-int-diff-area cint 1 3)))
       (when (and (conc-int-type-shell-2-p type) (conc-int-type-shell-3-p type)
		  (> (conc-int-type-D-23 type) 0))
	 (format t "  A-23 ~,2e " (conc-int-diff-area cint 2 3)))
       (when core-connected-to-shells
	 (if (and (conc-int-type-shell-3-p type) (> (conc-int-type-D-3core type) 0))
	     (format t "  A-3core ~,2e" (conc-int-diff-area cint 3 'core))
	     (progn
	       (when (> (conc-int-type-D-1core type) 0)
		 (format t "  A-1core ~,2e" (conc-int-diff-area cint 1 'core)))
	       (when (and (conc-int-type-shell-2-p type) (> (conc-int-type-D-2core type) 0))
		 (format t "  A-2core ~,2e" (conc-int-diff-area cint 2 'core))))))
       (when (or (conc-int-type-shell-2-p type)
		 (conc-int-type-shell-3-p type)
		 core-connected-to-shells)
	 (format t "~%"))))))
...
(defun print-conc-int (cint)
  (let* ((cint (element cint 'conc-int))
    ...
    (when cint
      (format t "Concentration Integrator ~a: Type ~a~%"
	      (conc-int-name cint)
	      (conc-int-type-name (conc-int-type cint)))

      ;;      (when (element-parameter cint 'ss-concentration)
      ;;	(format t "   Steady-state conc ~,2emM~%" (element-parameter cint 'ss-concentration)))
      
      (unless (conc-int-type-enabled (conc-int-type cint))
	(format t "  *** Type Disabled ***~%"))
      (format t "   Shell ~A volume(s) ~a um3, total volume ~,2e um3 ~%"
	      (concatenate-strings "1"
				   (when (conc-int-type-shell-2-p type) "/2")
				   (when (conc-int-type-shell-3-p type) "/3"))
	      (concatenate-strings
	       (format nil "~,2e" (conc-int-shell-1-volume cint))
	       (when (conc-int-type-shell-2-p type)
		 (format nil "/~,2e" (conc-int-shell-2-volume cint)))
	       (when (conc-int-type-shell-3-p type)
		 (format nil "/~,2e" (conc-int-shell-3-volume cint))))
	      (conc-int-total-volume cint))
      (when *simulation-initialized*
	(format t "   Shell ~A free concentration(s) ~a mM, total conc ~,2e mM @ ~,2e ms~%"
		(concatenate-strings "1"
				     (when (conc-int-type-shell-2-p type) "/2")
				     (when (conc-int-type-shell-3-p type) "/3"))
		(concatenate-strings
		 ...
		 (when (conc-int-type-shell-2-p type)
		   (format nil "/~,2e" (conc-int-shell-3-free-conc-n cint)))
		 (when (conc-int-type-shell-3-p type)
		   (format nil "/~,2e" (conc-int-shell-3-free-conc-n cint))))
		(conc-int-total-free-conc-n+1 cint) *real-time*))
      ...
      (loop for chperlist in (list (conc-int-shell-1-pores cint) (conc-int-shell-2-pores cint))
	    ...
	    (loop for chperm in chperlist ; (conc-int-shell-1-pores cint)
		  do (format t "     ~a ~,4f~%"
			     (element-name (conc-int-shell-pore chperm))
			     (conc-int-shell-pore-perm chperm)))))))
