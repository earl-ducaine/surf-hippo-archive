/*
 * $Id: parser.c,v 1.7 1994/11/23 16:40:34 rempel Exp $
 * $Log: parser.c,v $
 * Revision 1.7  1994/11/23  16:40:34  rempel
 * for 6.2.4
 *
 * Revision 1.6  1994/08/23  10:36:08  portelli
 * For 6.2.4
 *
 * Revision 1.5  1994/06/01  18:07:31  rempel
 * fixed bug # 544
 *
 * Revision 1.4  1994/05/18  13:33:08  portelli
 * Soar 6.2.0 b
 *
 * Revision 1.3  94/05/06  20:28:32  rempel
 * *** empty log message ***
 * 
 * Revision 1.2  1993/11/21  17:02:37  soarhack
 * 6.1.1 checkin
 *
 * Revision 1.1  1993/06/17  20:47:39  jtraub
 * Released
 *
 * Revision 0.1  1993/06/17  20:22:21  jtraub
 * 6.1_checkin
 *
 * Revision 9.2  1993/05/10  19:28:55  jtraub
...
void reset_placeholder_variable_generator (void) {
  int i;
  for (i=0; i<26; i++) current_agent(placeholder_counter)[i] = 1;
}
...
test make_placeholder_test (char first_letter) {
  ...
  sprintf (namebuf, "<#%c*%lu>", first_letter,
           current_agent(placeholder_counter)[first_letter-'a']++);
...
/* -----------------------------------------------------------------
                       First Letter From Test

   When creating dummy variables, we try to give them names that start
   with a "reasonable" letter.  For example, ^foo <dummy> becomes
   ^foo <f*37>, where the variable starts with "f" because the attribute
   test starts with "f" also.  This routine looks at a test and tries
   to figure out a reasonable choice of starting letter for the next
   variable.  If it can't find a reasonable choice, it returns '*'.
----------------------------------------------------------------- */
...
Symbol *make_symbol_for_current_lexeme (void) {
  ...
  case SYM_CONSTANT_LEXEME:  return make_sym_constant (current_agent(lexeme).string);
  case VARIABLE_LEXEME:  return make_variable (current_agent(lexeme).string);
  case INT_CONSTANT_LEXEME:  return make_int_constant (current_agent(lexeme).int_val);
  case FLOAT_CONSTANT_LEXEME:  return make_float_constant (current_agent(lexeme).float_val);

  case IDENTIFIER_LEXEME:
    print ("Internal error:  ID found in make_symbol_for_current_lexeme\n");
    abort_with_fatal_error();

  default:
    print ("Internal error:  bad lexeme type in make_symbol_for_current_lexeme\n, current_agent(lexeme).string=%s\n", current_agent(lexeme).string);
    abort_with_fatal_error();
  }
...
condition *parse_value_test_star (char first_letter) {
  ...
  do {
    ...
    acceptable = FALSE;
    if (current_agent(lexeme).type==PLUS_LEXEME) { acceptable = TRUE; get_lexeme(); }
...
condition *parse_attr_value_tests (void) {
  ...
  negate_it = FALSE;
  if (current_agent(lexeme).type==MINUS_LEXEME) { negate_it = TRUE; get_lexeme(); }
...
/* AGR 544 end */
  ...
  if (id_test_to_use) fill_in_id_tests (new_conds, id_test_to_use);
  deallocate_test (attr_test);
  if (last_c) last_c->next = new_conds; else first_c = new_conds;
  new_conds->prev = last_c;
...
/* -----------------------------------------------------------------
   ...
   This routine parses the "( [goal|impasse] [<id_test>]" part of
...
#ifndef NNPSCM
    if (!strcmp(current_agent(lexeme).string,"goal")) {
#else
...
condition *parse_cond (void) {
  ...
  if (current_agent(lexeme).type==L_BRACE_LEXEME) {
    ...
    c = parse_conds_for_one_id ('g', NULL);
    if (!c) return NIL;
  }
...
rhs_value parse_function_call_after_lparen (bool is_stand_alone_action) {
  ...
  if (current_agent(lexeme).type==PLUS_LEXEME) fun_name = find_sym_constant ("+");
  else if (current_agent(lexeme).type==MINUS_LEXEME) fun_name = find_sym_constant ("-");
  else fun_name = find_sym_constant (current_agent(lexeme).string);
  if (!fun_name) {
...
/* -----------------------------------------------------------------
   ...
   xxx_PREFERENCE_TYPE (see soar.h).

   Note:  in addition to the grammar below, if there is no preference
   specifier given, this routine returns ACCEPTABLE_PREFERENCE_TYPE.
...
/* -----------------------------------------------------------------
                   Parse Preference Specifier Star

   Given the id, attribute, and rhs_value already read, this routine
...
action *parse_preference_specifier_star (Symbol *id, Symbol *attr,
                                         rhs_value value) {
  action *a;
  action *prev_a;
  rhs_value referent;
...
action *parse_attr_value_make (Symbol *id) {
  ...
  get_lexeme();
  if ((current_agent(lexeme).type!=VARIABLE_LEXEME)&&(current_agent(lexeme).type!=SYM_CONSTANT_LEXEME)) {
    print ("Expected variable or symbolic constant for attribute in RHS make action\n");
    print_location_of_most_recent_lexeme();
    return NIL;
  }
  attr = make_symbol_for_current_lexeme ();
  get_lexeme();
  all_actions = NIL;
  ...
  do {
    ...
    new_actions = parse_preference_specifier_star (id, attr, value);
...
production *parse_production (void) {
  ...
  if (name->sc.production) {
    excise_production (name->sc.production, TRUE);
  }
  ...
  while (TRUE) {
    ...
    if (!strcmp(current_agent(lexeme).string,":no-o-support")) {
      declared_support = DECLARED_NO_O_SUPPORT;
      get_lexeme();
      continue;
    }
  ...
  if (! lhs) {
    ...
    if (documentation) free_memory_block_for_string (documentation);
