...
;; Small modification by reuther Aug 1992:
;; Using Genera Lisp, I got some trouble with 'lisp:make-pathname' and the keys
;; :name, :directory and :types, because Genera Lisp converts that strings always to uppercase! 
;; Which is not very handy on UN*X systems. 
;; Now I'm using :raw-name, :raw-directory and :raw-types for Genera instead.
...
#+ANSI-90
(in-package :pdefsystem)

#+(or Genera-Release-8-0 Genera-Release-8-1)
(lisp:in-package "PDEFSYSTEM"
		 :use '("LISP")
		 :nicknames '("PDEFSYS"))

#-(or ANSI-90 Genera-Release-8-0 Genera-Release-8-1)
(lisp:in-package #-(or lispworks ANSI-90) "PDEFSYSTEM" 
            #+lispworks "PDEFSYS"
            #+ANSI-90 "PDEFSYSTEM"
            #-ANSI-90 :use #-ANSI-90 '("LISP")
            #-ANSI-90 :nicknames #-ANSI-90 '("PDEFSYS"))
...
#+(or Minima Genera Lucid Allegro)
;; Allegro added at 1.7.92 (junker)
...
#-(or Minima Genera)
(eval-when (eval load compile)
  (export (list (intern (symbol-name :make-pathname) :pdefsystem)
		(intern (symbol-name :pathname-directory) :pdefsystem))))
...

(pushnew :defsystem user::*features*)
(pushnew :our-defsystem user::*features*)
...
;; modified by BBB
; #+Genera			      `(:lisp 		:ibin)
;; we need 'lsp' to be always a valid extension (reuther)
;; can't use :ibin as 'raw-type' (see comment at the beginning)	, so I use a boring string
  #+Genera			      `(("lisp" "lsp" "")	"ibin")
; #+Genera			      `("lisp" 		,si:*default-binary-file-type*)

  #+Cloe-Runtime		      `("l"		"fas")
  #+Minima			      `("lisp"		"mebin")
;; use both variables (reuther)
  #+lucid			     `(,lcl:*load-source-pathname-types*   ,lcl:*load-binary-pathname-types*)
;;  #+lucid			     `(("lisp" "cl")   ,lcl:*load-binary-pathname-types*)
;;  suffix 'lsp' added by Allegro at 7.8.92 (junker)
  #+Allegro			      `(("cl" "lisp" "lsp")	"fasl")
...
(defun add-pathname-type (pathname types)
  ...
  (flet ((make-path (type defaults)
	   #-Genera (lisp:make-pathname :type (pretty-pathname-component type)
			       :defaults defaults)
	   #+Genera (lisp:make-pathname :raw-type (pretty-pathname-component type)
			       :defaults defaults)
        ))
    (if (consp types)
	(let (path)
...
(defun module-src-path (module)
  (let ((pathname (module-pathname module))
    ...
    (cond ((null pathname)
	   (add-pathname-type #-Genera (lisp:make-pathname :name (string (module-name module))
						  :defaults defaults)
			      #+Genera (lisp:make-pathname :raw-name (string (module-name module))
						  :defaults defaults)
			      types))
	  ((null (pathname-type pathname))
	   (add-pathname-type #-Genera (lisp:make-pathname :name (string (module-name module))
						  :defaults (merge-pathnames pathname defaults))
			      #+Genera (lisp:make-pathname :raw-name (string (module-name module))
						  :defaults (merge-pathnames pathname defaults))
			      types))
...
(defun module-bin-path (module)
  (let* ((pathname (module-binary-pathname module))
         ...
         (result (cond ((and (null pathname) (null src-pathname))
                        (add-pathname-type 
			#-Genera (lisp:make-pathname :name (string (module-name module))
                                                               :defaults defaults)
			#+Genera (lisp:make-pathname :raw-name (string (module-name module))
                                                               :defaults defaults)
                                           types))
...
named NAME."
  ...
  (let ((defaults-list (if (listp *sysdcl-pathname-defaults*)
    ...
    (dolist (defaults defaults-list)
      (let #-Genera ((file (lisp:make-pathname :name (format nil *sysdcl-name-format-string* name)
				      :defaults defaults)))
	   #+Genera ((file (lisp:make-pathname :raw-name (format nil *sysdcl-name-format-string* name)
				      :defaults defaults)))
    ...
    (dolist (defaults defaults-list)
      (let #-Genera ((file (lisp:make-pathname :name (let ((*print-case* :downcase))
					      (format nil *sysdcl-name-format-string* name))
				      :defaults defaults)))
	   #+Genera ((file (lisp:make-pathname :raw-name (let ((*print-case* :downcase))
					      (format nil *sysdcl-name-format-string* name))
				      :defaults defaults)))
	(when (probe-file file)
	  (return-from system-source-file file))))
    ...
    (dolist (defaults defaults-list)
      (let* ((sysdcl-name (format nil *sysdcl-name-format-string* name))
	     (files (remove-if #'(lambda (p)
			       ...
			       (directory defaults))))
	(when files
	  (return-from system-source-file 
            #-Genera (lisp:make-pathname :name (pathname-name (car files))
				 :defaults defaults)
            #+Genera (lisp:make-pathname :raw-name (pathname-name (car files))
				 :defaults defaults)
          ))))))
	    ...
	    (*default-pathname-defaults*
	      ...
	      #+Genera
              (lisp:make-pathname
               :host (pathname-host ,path)
               :device (pathname-device ,path)
               :raw-directory (lisp:pathname-directory ,path)
               :raw-name "foo"
               :raw-type "fasl"
               :defaults *default-pathname-defaults*)
             )))
...
;;; geaendert von BBB
(defvar *load-all-before-compile* t
...
(defun write-Makefile-for-system (system)
  ...
  (let* ( #-Genera (up-pathname (lisp:make-pathname :directory '(:relative :up)))
	  #+Genera (up-pathname (lisp:make-pathname :raw-directory '(:relative :up)))
	 (default-pathname
    ...
    (with-open-file (stream #-Genera (lisp:make-pathname :defaults default-pathname
						:name "Makefile"
						;; --- Unclear this will work on Unix.
						:type "")
			    #+Genera (lisp:make-pathname :defaults default-pathname
						:raw-name "Makefile"
						;; --- Unclear this will work on Unix.
						:type "")
			    :direction :output)
