;;;Note: relevance-table should be loaded after load-domain.lisp
(load "/afs/cs/project/prodigy/version4.0/working/domains/qual-machining/relevance-table")
;;- so :permute-application-order works (change in old-gen-apps-r)
(load "/afs/cs/project/prodigy/version4.0/working/domains/qual-machining/patch-permute")

;;redefines announce-plan: can print ops only, with numbers on the left, and
;;apply quality metric
(load "/afs/cs/project/prodigy/version4.0/working/domains/qual-machining/announce-ops-only")

(setf *which-rules* 'anycost)

;;(domain 'hd-machining :function t)
...
(Operator DRILL-SPOT-DRILL-PRESS
 (params <machine> <drill-bit> <holding-device> <part> <side> <side-pair>)
 (preconds
  ((<machine> Drill)
...
(Operator DRILL-SPOT-MILLING-MACHINE
 (params <machine> <drill-bit> <holding-device> <part> <side> <side-pair>)
 (preconds
  ((<machine> MILLING-MACHINE)
   (<drill-bit> SPOT-DRILL)
...
(Operator DRILL-HOLE-MILLING-MACHINE
 (params <machine> <drill-bit> <holding-device> <part> <side> <side-pair>)
 (preconds
  ((<machine> MILLING-MACHINE)
   (<drill-bit> TWIST-DRILL)
...
;;this allows any kind of drill-bit to be held by the milling machine,
;;but the ops only can use spot-drills and twist-drills.
...
(Operator REMOVE-HOLDING-DEVICE
  (params <machine> <holding-device>)
  (preconds
   ((<machine> Machine)
...
(let* ((possible-rules
  ...
  (let ((files (cdr (assoc val possible-rules))))
	...
	(mapcar #'(lambda (f)
		    (load (concatenate
			   'string
			   "/afs/cs/project/prodigy/version4.0/working/domains/qual-machining/" f)))
		files)
...
;;(format t "Loading compute-cost.lisp evaluation function.~%")
(load "/afs/cs/project/prodigy/version4.0/working/domains/qual-machining/eval-fns/compute-cost4")

;;(setf p4::*no-danglers* nil)
