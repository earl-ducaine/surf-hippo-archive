...
(definstr fa-has-succeeded ()        ; sammelt von WAM gefundene Bindungen ein
   ...
   (if (null *user-variabels*)
      (setq *fa-WAM-bindings* 'ok)     ; erfolgreicher Beweis ohne Bindungsgenerierung
      (setq *fa-WAM-bindings*          ; erfolgreicher Beweis und neue Bindungen generiert.
          (cons (mapcar #'(lambda (var)
                            (list (second (car var)) (show-term (cdr var) t)))
                        *user-variabels*)
                *fa-WAM-bindings*)))    ; immer fail generieren um zum naechsten choice-
...
(fa-definstr prove (source-mem result-mem goal-list)    ;Variable in goal-list mit (vari x) !
   (declare (special *source-mem*)
            (special *bound-vars*)
            (special *unbound-vars*))
   (when *fa-trace* (print (list 'prove source-mem result-mem goal-list)))
   (if (equal (top *cstack) *NP)
     (pop *cstack)
     (progn
       (setq *fa-WAM-bindings* nil)
       (setq *source-mem* source-mem)     ;setze in goal-list Bindungen ein,sammle alle         
       (setq *unbound-vars* nil)          ;ungebundenen Variablen und erzeuge die query
       (let* ((complete-goals (mapcar 'fa-setze-ein goal-list))  ;Bindungen einsetzen
              (vari-vars (mapcar #'(lambda (var)
                                      (list 'vari var))   ;"vari" vor ungeb. Variable
                                 *unbound-vars*))
              (query (cons (cons 'main vari-vars) complete-goals)))  ;query erzeugen
          (apply 'fa-call-wam (list query)))
       (if (null *fa-WAM-bindings*)
           (fa-fail)
           (when (equal *fa-WAM-bindings* 'ok)     ;WAM als prover einer voll-
                 (setq *fa-WAM-bindings* nil)      ;instanziierten Regelinstanz,
                 (fa-set-new-inst result-mem       ;keine neuen Bindungen generiert
                     (fa-newest-partial-inst source-mem))))))
   (when (not (null *fa-WAM-bindings*))
      (let ((bindings (first *fa-WAM-bindings*)))
         (setq *fa-WAM-bindings* (rest *fa-WAM-bindings*))
         (let ((take-over-list nil))
            (dolist (pair (fa-newest-partial-inst source-mem)) ; bekannte Bindungen uebernehmen
               (unless (member (car pair) *unbound-vars*)
                  (setq take-over-list (cons pair take-over-list))))
            (fa-set-new-inst result-mem take-over-list))             
         (mapcar #'(lambda (varname)  ;generierte Bindungen in *heap und memory
                      (fa-set-new-binding result-mem varname *TOP)   ;speichern
                      (put-term-on-top (second (assoc varname bindings))))
                 *unbound-vars*))
      (when *fa-WAM-bindings*
           (push *NP *cstack)
           (push *NP *cstack))))
