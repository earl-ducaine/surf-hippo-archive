/*
 * $Id: explain.c,v 1.7 1994/11/23 16:40:34 rempel Exp $
 * $Log: explain.c,v $
 * Revision 1.7  1994/11/23  16:40:34  rempel
 * for 6.2.4
 *
 * Revision 1.6  1994/08/23  10:33:19  portelli
 * For 6.2.4
 *
 * Revision 1.5  1994/05/06  20:27:21  rempel
 * fixed several bugs
 *
 * Revision 1.4  1993/11/21  16:50:44  soarhack
 * Activate Headers
 *
...
#include "soar.h"      /* Condition type defs */
#include "explain.h"
...
/* AGR 564  This bug report came complete with fixes from Frank Koss.
   ...
   for this bug are explain.c, explain.h, global_vars.h, hooks.c,
   init_soar.c, interface.c, and soar.h.  AGR 564 2-May-94  */
...
*                Need list of results to create list of actions, because
*                there is no "copy_action_list" command I can use.
**************************************************************************/

void explain_add_temp_to_chunk_list(explain_chunk_str *temp, preference *results) {
   
explain_chunk_str *chunk;

  chunk = (explain_chunk_str *)malloc(sizeof (explain_chunk_str));
  chunk->conds   = copy_cond_list(temp->conds);
  chunk->actions = (action *)copy_and_variablize_result_list (results);
...
explain_chunk_str *find_chunk(explain_chunk_str *chunk, char *name) {
  ...
  print ("\nUse explain :on before the chunk is created.\n");
...
void explain_trace_named_chunk(char *chunk_name) {
...
void explain_cond_list(char *chunk_name) {
   
explain_chunk_str *chunk;
condition *top, *bottom, *cond, *ground;
int i;
