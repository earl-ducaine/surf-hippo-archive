...
/*
 * int sock_accept_conn_no_block(int listen_socket)
 *	IN int listen_sock;	The socket we're listening for connections.
 *
 * Returns:
 *	The new socket, if successful; 0 if no connection, -1 otherwise.
 *
 * Functionality:
 *	Accept the connection, or return 0 if nothing is waiting (non-blocking)
 */
	int
sock_accept_conn_no_block(int listen_sock)
{
	int sock;	/* the new socket */
	int i = 0;
	extern int errno;
	...
	fcntl(listen_sock, F_SETFL, FNDELAY);
