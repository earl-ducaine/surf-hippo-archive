...
(defun vclamp-soma-conductance (clamp-potential-1
  ...
  "Calculates the input soma conductance as a function of time using voltage clamp at two holding
potentials CLAMP-POTENTIAL-1 and CLAMP-POTENTIAL-2 [mV]. An electrode resistance may be included
with R-ELECTRODE [Mohms]. The voltage clamp \(ideal if IDEAL-VSOURCE is T\) is applied to the soma.
Any other voltage sources in the circuit are disabled during the simulation when DISABLE-VSOURCES is
T."
...
(defun clamp-soma-conductance (clamp-command-1
			       clamp-command-2
			       &key (dt 0.1) new-plot (plot-data t) (plot-uncorrected-wave t)
  ...
  "Calculates the input soma conductance as a function of time using voltage clamp or current clamp
protocols, depending on VCLAMP-P, at two clamp command levels CLAMP-COMMAND-1 and CLAMP-COMMAND-2
[mV or nA, as appropriate]. An electrode resistance may be included with R-ELECTRODE [Mohms]. The
voltage clamp \(ideal if IDEAL-VSOURCE is T\) is applied to the soma.  Any other voltage sources in
the circuit are disabled during the simulation when DISABLE-SOURCES is T."
  (if vclamp-p
...
;; For voltage clamp, the units for the output list is the unit of WAVEs (current) divided by the
;; unit of CLAMPs (volts). R-ELECTRODE should be in units of the inverse of the output units.
;; Therefore, for example, if the WAVEs are in nA and the CLAMPs are in mV, then r-electrode should
;; be in Mohms, and the output will be in uS.

;; VOLTAGExP is true when WAVEx is a voltage trace (=> CLAMPx is current), which is the case when
;; WAVEx is from a current clamp recordings, and vica versa.n
	  ...
	  (multiple-value-bind (g-wave g-wave-time-list g-wave-during-e-rev e-rev-wave e-rev-time-list cc-volts-wave)
	    ...
	    (when plot-data
	      (plot-timed-data cc-volts-wave '(nil) g-wave-time-list ; :delta-t data-dt
			       ...
			       :title (format nil "~A: Reconstructed CC Response" *simulation-name*))
	      (plot-timed-data g-wave-during-e-rev '(nil) e-rev-wave ; :delta-t data-dt
			       :title (format nil "~A: Gin-Erev Characteristic" *simulation-name*)
			       :comment comment
...
(defun steady-state-iclamp (clamp-command &key (cc-element *soma*) (vsource *vsource*) (init-with-ss-vc t))
  (let ((cc-element (cell-element cc-element)))
    (when init-with-ss-vc
      ...
      (turn-on vsource)
      (init-with-steady-state-linear-voltage-clamp
       cc-element
       (+ (cell-type-param *cell* 'vl)
...
(defun vc/cc-g-erev (clamp-commands vc-p-list &key (data-dt 0.1)
				    ...
				    (plot-vc-current t)
				    (plot-g-waves t) (plot-e-rev-wave t)
				    (plot-g-erev-phase-plot t) plot-reconstruction
				    ...
				    (absolute-voltage-error *absolute-voltage-error*)
					; (relative-voltage-error 0.01)
				    (minimum-rel-g-for-e-rev 0.1)
				    debug break-btwn-runs
				    e-rev-plot-min e-rev-plot-max plot-voltage new-plot (plot-data t)
  ...
  (let* ((clamp-element (cell-element clamp-element))
	 ...
	 (new-plot (and new-plot plot-data))
	 (number-of-clamps (length clamp-commands))
	 (vc-p-list (if vc-p-list 
			(if (consp vc-p-list)
			    vc-p-list
			    (loop for clamp from 1 to number-of-clamps collect t))
			(loop for clamp from 1 to number-of-clamps collect all-vcs-p)))
	 (*default-scatter-size* scatter-size)
	 (*CREATE-NEW-SIMULATION-PLOTS* new-plot)
	 (*create-new-plot-windows* new-plot)
	 (*overlay-all-plots* (or *overlay-all-plots* (not new-plot)))
	 (*overlay-simulations (not new-plot))
	 (vsrc (when (or (no-nils vc-p-list) all-vcs-p init-cc-with-ss-vc) ; Do we need a vsrc?
	 ...
	 (*accomodate-all-overlays* plot-data)
	 (*plot-vsource-currents-p t)
	 *conc-int-initializations* *use-conc-int-initializations*
	 (*SAVE-DATA-STEP* 1)
	 (*use-node-voltage-initializations* t)
	 *beep-after-surf* total-conductance-wave)	
    (element-parameter vsrc 'set-default-to-start-of-pulses t)
    (when plot-voltage (enable-element-plot clamp-element))
    ;; (turn-off *isource*)
    (loop for clamp-command in clamp-commands
	  ...
	  (when (nth run-count conc-int-vclamp-init-lists)
	    (setq *conc-int-initializations* (nth run-count conc-int-vclamp-init-lists)
	  ...
	  do (if vc-p
		 ...
		 (progn
		   ...
		   (steady-state-iclamp
		    clamp-command :cc-element clamp-element :vsource vsrc :init-with-ss-vc init-cc-with-ss-vc)))
	  when (and plot-total-conductance (= run-count 1))
	  do (setq total-conductance-wave
	  ...
	  (when data-filter-function
	    (setq data-waves
		  (loop for data-wave in data-waves
			collect (sequence-to-list (funcall data-filter-function data-wave)))))
	  (setq *create-new-plot-windows* new-plot)
	  (multiple-value-bind (g-wave g-wave-time g-wave-during-e-rev e-rev-wave e-rev-wave-time cc-volts-wave)
	      (vc/cc-g-wave-linear-regression
	       (let ((min-length (loop for data-wave in data-waves minimize (length data-wave))))
		 (loop for data-wave in data-waves collect (list-head data-wave min-length)))
	       clamp-commands
	       vc-p-list
	       :print-v-g-rest print-v-g-rest
	       :g-rest g-rest :v-rest v-rest
	       :debug debug :delta-t data-dt :minimum-rel-g-for-e-rev minimum-rel-g-for-e-rev)
	    
	    (when plot-data
	      (when data-filter-function
		(plot-timed-data data-waves clamp-commands nil :delta-t data-dt
				 :x-label "ms"
				 :comment (if add-trace-analysis-to-plot
					      (concatenate-string-list
					       (no-nils
						(cons comment
						      (surf-plot-trace-analysis-strings
						       data-waves clamp-commands data-dt)))
					       :lf-count 1)
					      comment)
				 :title (format nil "~A: Filtered Clamp Data" *simulation-name*)))

	      (let ((g-waves (if total-conductance-wave (list g-wave total-conductance-wave) g-wave))
		    (g-wave-labels (if total-conductance-wave
				       '("Measured Gin" "Total Gin")
				       '(nil))))
		(when dissect-g-wave 
		  (multiple-value-bind (gex-wave ginh-wave g-rest-wave g-tot-wave output-time)
		      (dissect-g-wave-from-gtot-erev
		       e-rev-wave e-rev-wave-time g-wave g-wave-time grest eex einh
					; :start-time start-time :stop-time stop-time
		       :float-eex float-eex :float-einh float-einh)
		    (setq g-waves (no-nils (list g-wave total-conductance-wave gex-wave ginh-wave))
			  g-wave-labels (no-nils 
					 `("Measured Gin" ,(when total-conductance-wave "Total Gin") "Gex" "Ginh"))))
		  
		  (plot-vc/cc-g-erev-output (when plot-g-waves g-waves)
					    g-wave-labels g-wave-time
					    (when plot-g-erev-phase-plot g-wave-during-e-rev)
					    e-rev-wave
					    (when plot-e-rev-wave e-rev-wave-time)
					    (when plot-reconstruction cc-volts-wave)
	      ...
	      (when return-waves
		(return (values g-wave g-wave-time g-wave-during-e-rev e-rev-wave e-rev-wave-time
				cc-volts-wave data-waves))))))))



(defun plot-vc/cc-g-erev-output (g-waves g-wave-labels g-wave-time
					 g-wave-during-e-rev e-rev-wave e-rev-wave-time cc-volts-wave
					 comment comment-position e-rev-plot-min e-rev-plot-max
					 stop-time
					 &key time-inc
					 add-trace-analysis-to-plot
					 print-erev-measures
					 (print-erev-measures-min-g 0.0)
  ...
  (cond-every
   (cc-volts-wave
    (plot-timed-data cc-volts-wave '(nil) g-wave-time ; :delta-t data-dt
		     :comment (if add-trace-analysis-to-plot
				  (concatenate-string-list
				   (cons comment
					 (surf-plot-trace-analysis-strings (list cc-volts-wave) '(nil) g-wave-time))
				   :lf-count 1)
				  comment)
		     :comment-position comment-position
		     :x-max stop-time :x-label "ms" :y-label "mV"
		     :x-inc time-inc
		     :title (format nil "~A: Reconstructed CC Response" *simulation-name*)))
   (g-wave-during-e-rev
    (let ((plot-win    
	   (plot-timed-data g-wave-during-e-rev '(nil) e-rev-wave ; :delta-t data-dt
			    ...
			    :comment-position comment-position
			    :scatter t :connect-data-points nil
			    :x-symbol-width 1 :y-symbol-width 1
			    :x-origin e-rev-plot-min :x-min e-rev-plot-min :x-max e-rev-plot-max :y-min 0
	...
	(let ((max-g (max-of-list g-wave-during-e-rev)))
	  (loop for g-during-e-rev in g-wave-during-e-rev
		...
		when (> g-during-e-rev print-erev-measures-min-g)
		maximize e-rev into max-e-rev and minimize e-rev into min-e-rev
		when (= g-during-e-rev max-g) collect e-rev into e-ref-at-max-g
		finally
		(add-comment plot-win
			     (format nil "E-rev min/max ~,2f/~,2fmV (Gmin ~,2f)~%E-rev ~AmV@Gmax ~,2f"
				     min-e-rev max-e-rev print-erev-measures-min-g
				     (atomize-list e-ref-at-max-g) max-g)
   ...
   (g-waves (plot-timed-data g-waves g-wave-labels g-wave-time ; :delta-t data-dt
				 ...
				 (concatenate-string-list
				  (cons comment (surf-plot-trace-analysis-strings g-waves g-wave-labels g-wave-time))
				  :lf-count 1)
				 comment)
			     :comment-position comment-position
			     :x-label "ms" :y-label (if g-waves-in-uS-p "uS" "nS") :x-inc time-inc
			     :x-min 0 :x-max stop-time :y-min 0))
   ((and e-rev-wave e-rev-wave-time)
    (plot-timed-data e-rev-wave '(nil) e-rev-wave-time
		     ...
		     :y-label "mV" :x-label "ms" :x-inc time-inc
		     :x-min 0 :x-max stop-time
		     :scatter t :y-symbol-width 1 :x-symbol-width 1 :connect-data-points nil))))
...
(defun vc/cc-g-wave-linear-regression (data-waves clamp-commands vc-p-list
  ...
  (let* ((length-of-waves (length (the cons (car data-waves))))
	 ...
	 (g-wave '()) (g-wave-time '()) (cc-volts-wave '())
	 (g-wave-during-e-rev '()) (e-rev-wave '()) (e-rev-wave-time '()) 
    ...
    (loop for count fixnum from 0 to (1- length-of-waves)
	  ...
	  (loop for data-wave in data-waves
		...
		when vc-p collect (the sf (car data-wave)) into currents
		and collect (- clamp-cmd (* (the sf r-electrode) (the sf (car data-wave)))) into corrected-voltages
		else collect clamp-cmd into currents
		and collect (- (the sf (car data-wave)) (* (the sf r-electrode) clamp-cmd)) into corrected-voltages
		finally (setq data-waves new-data-waves)
		(multiple-value-bind (slope intercept r) (lin-reg (list currents corrected-voltages))
		  ...
		  (let ((g-value (if (eq :infinity slope)
		    ...
		    (when (and (not v-and-g-rest-supplied) (= 0 count))
		      (unless (and g-value (numberp intercept))
			(sim-error (format nil "Resting Value for G cannot be derived from an ~A slope and ~A intercept!"
					   slope intercept)))
...
(defun dissect-g-wave-from-gtot-erev (e-rev-wave e-rev-wave-time g-wave g-wave-time grest eex einh
						 &key start-time stop-time FLOAT-EEX FLOAT-EINH)
...
(defun vclamp (&key (duration *user-stop-time*)
		    (relative-voltage-error 0.01) ; check this - need fairly high accuracy to avoid
					; noisy difference of interpolations 
		    use-loaded-stimulus
  ...
  (let ((*overlay-all-plots* overlay-all-plots)
    ...
    (setq active-clamp-currents
	  (loop for vsource in vsources
		...
		(if interpolate-traces
		    (convert-data-time-lists (retrieve-single-data vsource 'current)
					     (current-sim-plot-time-list) time-resolution)
    ...
    (let (*active*
      ...
      (setq passive-clamp-currents
	    (loop for vsource in vsources
		  collect
		  (if interpolate-traces
		      (convert-data-time-lists
		       (retrieve-single-data vsource 'current) (current-sim-plot-time-list) time-resolution)
		      (retrieve-single-data vsource 'current)))))
	...
	(plot-timed-data (car currents) labels (unless interpolate-traces (current-sim-plot-time-list)) 
			 ...
			 :y-label "nA" :overlay t :comment *simulation-plot-window-comment*
			 :title "Interpolated Vsource Currents"))
      (loop for vsource in (element-name vsources)
	    ...
	    (plot-timed-data (list isolated-active-current) (list (format nil "~A Isolated Active Current" vsource))
			     ...
			     :y-label "nA" :title "Isolated Active Vsource Currents"
...
START-STIMULUS, STOP-STIMULUS, and STEP. Stimulus timing is set by STIMULUS-START-TIME [default 10.0ms] and
STIMULUS-STOP-TIME [default \(* 0.9 *USER-STOP-TIME*\)]. When STIMULUS-FUNCTION is NIL stimulus [nA] is applied via
ISOURCE [default *ISOURCE*]. Otherwise STIMULUS-FUNCTION is called for each magnitude of the stimulus:
	  ...
	  (let* ((*plot-standard-windows* (or *plot-standard-windows* (member stimulus steps-to-plot)))
		 ...
		 (*simulation-plot-window-comment*
		  (if add-step-to-plot-comment
		      (concatenate 'string *simulation-plot-window-comment*
				   (format nil " ~A ~A step" stimulus stimulus-units))
		      *simulation-plot-window-comment*)))
	    (goferit)
	      ...
	      (loop for spks-stim in spks-stims do ; Collect the first 4 intervals and steady state.
		    ...
		    (let* (		; freqs			   stims
		      ...
		      (plot-xy-data stimulus-frequencies '("First" "Second" "Third" "Fourth" "Fifth" "SS")
				    ...
				    :y-label-vertical-position :two-thirds-up :y-are-fns t
  ...
  (let* ((steps (when (consp step) step))
    ...
    (loop for clamp in (or steps (loop for clamp from start-clamp to stop-clamp by step collect clamp))
	  ...
	  (let ((*CREATE-NEW-SIMULATION-PLOTS* individual-plots)
	    ...
	    (if timeit (gotimed) (goferit)))
	  when return-source-data
	  collect *sim-plot-time-list* into time
...
(defun linear-corrected-vclp (&key step-start-amplitude step-base-potential step-start step-duration new-plots
				   ...
				   holding-potential base-pulse-list total-steps initial-step-integer-coefficient
				   (vsource *vsource*)
				   reference-original-pulse-list
  ...
  (when vsource
    (setq
     ...
     max-time-for-peak-valley (or max-time-for-peak-valley (element-parameter vsource 'max-time-for-peak-valley)
				  *user-stop-time*)
     plot-peaks-and-valleys (or plot-peaks-and-valleys (element-parameter vsource 'plot-peaks-and-valleys))
     plot-times-for-peaks-and-valleys (or plot-times-for-peaks-and-valleys
					  (element-parameter vsource 'plot-times-for-peaks-and-valleys))
     ...
     initial-step-integer-coefficient (or initial-step-integer-coefficient
					  (element-parameter vsource 'initial-step-integer-coefficient)
					  0)
	...
	(let* (				; (*use-fixed-step* nil)
	  ...
	  (let ((*enable-synapses* (not disable-synapses-for-control))
		*active*)
	    (pulse-list vsource
			    (cons (list step-start (+ step-start step-duration) step-base-potential) base-pulse-list))
	    (let (*plot-standard-windows*) (GOFERIT))
	    (setq ref-vsource-zero-input-current (element-data-dted vsource delta-t))
	    (pulse-list vsource (cons (list step-start
						(+ step-start step-duration)
						(+ step-base-potential step-start-amplitude))
					  base-pulse-list))
	    (let (*plot-standard-windows*) (GOFERIT))
...
(defun conductance-from-current-trace (reversal-potential &optional current-values voltage-values label)
  (let ((voltage-current-values
    ...
    (loop for voltage in (car voltage-current-values)
	  ...
	  (plot-timed-data gs (list "") voltages
			   :x-label "mV" :y-label "uS"
			   :title  (or label (format nil "Extracted Conductance w/E-rev ~AmV" reversal-potential))))))
...
(defun voltage-jumps (activated-synapse synapse-activation-time start-jumps jump-increment number-jumps
					v-holding v-jump
					&key new-plot (vsource *vsource*)
					(clamp-duration *user-stop-time*))
...
(defun find-rm (cell rin)
    "Finds an approximate value for membrane resistivity, under the constraint that the input resistance
of CELL is ideally RIN [Mohms]. Shrinking ranges of Rm are explored, with exponential increments
\(argument of 10\) ranging from 1 to .001 \(by decade). Rin associated with returned value is less than
than RIN."
  (let ((start-exp 0)
	out
	this-rin
