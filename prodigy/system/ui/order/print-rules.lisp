...
(defun create-op-list (instantiated-op)
  (declare (type p4::instantiated-op instantiated-op))
...
(defun create-state-p-mirror (exp state)
  (cond
    ...
    ;(AND anything)
    ( (eq 'and (car exp))
           (list 'and (create-state-p-mirror (cdr exp) state)) )

    ;(or anything)
    ( (eq 'or (car exp))
           (list 'or (create-state-p-mirror (cdr exp) state)) )
...
(defun create-satisfy-list (exp state)
  (declare (special *short-circuit-preconditions*))
  (cond
    ( (null exp)
           nil)
...
(defun process-all-precond-list (exp instop)
  ...
  (cond
    ...
    ;(~ (and ...))
    ((and (eq '~ (car exp)) (eq 'and (car (second exp))))
     (demorgan (list '~ (process-all-precond-list (second exp) instop))))

    ;(~ (or ...))
    ((and (eq '~ (car exp)) (eq 'or (car (second exp))))
     (demorgan (list '~ (process-all-precond-list (second exp) instop))))

    ;(~ (forall...))
    ((and (eq '~ (car exp)) (eq 'forall (car (second exp))))
    ...
    ;(~ (exists ...))
    ((and (eq '~ (car exp)) (eq 'exists (car (second exp))))
  ...
  ;;the possible bindings for each the vars
    (let ((bindings
	  ...
	  (setf (second (third expr))
		(p4::substitute-binding (second (third expr)) bindings))
	  (setf (third expr)
		(p4::substitute-binding (third expr) bindings)))
    ...
    ;;        (EXISTS ((#<P-O: A object> OBJECT)) (~ (HOLDING #<P-O: A object>)))
	  ...
	  (do* ((gen (cdr (assoc (second expr)
				 ...
				 :test #'equalp ;CHANGE
			       ;;aperez: expr is partially instantiated
				 :key #'(lambda (el)
					  (p4::substitute-binding el bindings)))))
		...
		result)
	       ((null choice) result)
	  ;;aperez
	  ;;make sure that the bindings are valid (similar to fix-forall.lisp)
	    (when (p4::true-bindings-p expr data choice forall-bindings
		       (p4::get-all-functions-forall expr))
