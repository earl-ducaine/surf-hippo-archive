...
(channel-type-def
 '(na-Jaffe94
   (parent-type . na-hh-fit)		; Defined in hodgkin-huxley.lisp
...
(channel-type-def
 '(kdr-Jaffe94
   (parent-type . dr-hh-fit)		; Defined in hodgkin-huxley.lisp
...
(defun alpha-m (voltage a b)
  (declare (optimize (speed 3) (space 0)) 
	   (single-float voltage a b))
  (/ (* a (- b voltage))
     (- (exp (/ (- b voltage) 10)) 1)))

(defun beta-m (voltage c d)
  (declare (optimize (speed 3) (space 0)) 
	   (single-float voltage c d))
  (* c (exp (/ (- voltage) d))))
  
;; ee since e is reserved
(defun alpha-h (voltage ee f)
  (declare (optimize (speed 3) (space 0)) 
	   (single-float voltage ee f))
  (* ee (exp (/ (- voltage) f))))

(defun beta-h (voltage g)
  (declare (optimize (speed 3) (space 0)) 
	   (single-float voltage g))
...
(channel-type-def
 `(ca-t-Jaffe-fit
   ...
   (v-particles . ((ca-t-jaffe-m-fit 2)(ca-t-jaffe-h-fit 1)))))
...
(particle-type-def
 `(ca-t-jaffe-m
   (class . :hh)
   (alpha-function . ,#'(lambda (voltage) (alpha-m voltage .2 19.26)))
   (beta-function . ,#'(lambda (voltage) (beta-m voltage 0.009 22.03)))))
...
(particle-type-def
 `(ca-t-jaffe-h
   (class . :hh)
   (alpha-function . ,#'(lambda (voltage) (alpha-h voltage 1.0e-6 16.26)))
   (beta-function . ,#'(lambda (voltage) (beta-h voltage 29.79)))))
...
(channel-type-def
 `(ca-n-jaffe-fit
   ...
   (v-particles . ((ca-n-jaffe-m-fit 2)(ca-n-jaffe-h-fit 1)))))
...
(particle-type-def
 `(ca-n-jaffe-m
   (class . :hh)
   (alpha-function . ,#'(lambda (voltage) (alpha-m voltage 0.19 19.88)))			
   (beta-function . ,#'(lambda (voltage) (beta-m voltage 0.046 20.73)))))
...
(particle-type-def
 `(ca-n-jaffe-h
   (class . :hh)
   (alpha-function . ,#'(lambda (voltage) (alpha-h voltage 1.6e-4 48.4)))
   (beta-function . ,#'(lambda (voltage) (beta-h voltage 39.0)))))
...
(channel-type-def
 `(ca-l-jaffe-fit
   ...
   (v-particles . ((ca-l-jaffe-m-fit 2)))))
...
(particle-type-def
 `(ca-l-jaffe-m
   (class . :hh)
   (alpha-function . ,#'(lambda (voltage) (alpha-m voltage 15.69 81.5)))
   (beta-function . ,#'(lambda (voltage) (beta-m voltage 0.29 10.86)))))
