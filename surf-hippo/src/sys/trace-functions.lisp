...
;
;(let ((delta-t 0.1))
;  (plot-xy-data (list (list
;                       *sim-reverse-plot-time-list*
;                       (reverse
;                        (car (retrieve-plot-data (list (list *plot-soma-nodes* `node-voltage)
;                                                       (list *plot-nodes*
;                                                             `node-voltage))))))
;                      (list (loop for i from 0.0 to 300 by delta-t collect i)
;                            (reverse
;                             (CONVERT-DATA-TIME-BASE
;                              (reverse
;                               (car (retrieve-plot-data (list (list *plot-soma-nodes* `node-voltage)
;                                                              (list *plot-nodes* `node-voltage)))))
;                              delta-t)))) '("real" "test" )))
