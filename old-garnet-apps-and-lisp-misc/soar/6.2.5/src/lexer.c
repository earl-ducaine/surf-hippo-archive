/*
 * $Id: lexer.c,v 1.12 1995/01/20 00:11:18 rempel Exp $
 * $Log: lexer.c,v $
 * Revision 1.12  1995/01/20  00:11:18  rempel
 * undid buggy change from fix of bug 628 for release of 6.2.4c
 *
 * Revision 1.11  1994/12/22  21:39:33  rempel
 * made 1 minor change relating to MS-Windows code to make X Soar work
 *
 * Revision 1.10  1994/11/23  16:40:34  rempel
 * for 6.2.4
 *
 * Revision 1.9  1994/08/23  10:35:12  portelli
 * For 6.2.4
 *
 * Revision 1.8  1994/06/03  21:20:49  rempel
 * fixed shell escape to parse and trap cd, chdir, pushd, popd, dirs
 *
 * Revision 1.7  1994/06/01  18:06:04  rempel
 * mods/fixes to alias and shell escape
 *
 * Revision 1.6  1994/05/13  18:20:43  rempel
 * added alias & directory stack
 *
 * Revision 1.5  1994/05/06  20:28:21  rempel
 * *** empty log message ***
 *
 * Revision 1.4  1994/03/15  17:29:49  rempel
 * no changes
 *
 * Revision 1.3  93/11/24  14:12:36  portelli
 * fixed 6.1.1 bug
 * 
 * Revision 1.2  93/11/21  16:59:32  soarhack
 * 6.1.1 checkin
 * 
 * Revision 1.1  1993/06/17  20:47:31  jtraub
 * Released
 *
 * Revision 0.1  1993/06/17  20:21:22  jtraub
 * 6.1_checkin
 *
 * Revision 9.2  1993/05/10  19:32:47  jtraub
...
#if !defined(__NeXT__) && !defined(__linux__)
extern double atof();
...
void determine_type_of_constituent_string (void) {
  ...
  if (possible_fc) {
    ...
    current_agent(lexeme).float_val = my_strtod (current_agent(lexeme).string,NULL,10); 
    if (errno) {
