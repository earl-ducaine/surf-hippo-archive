...
(defun HIPPO-net (cell-spacing cells-per-horizontal-side &optional cells-per-vertical-side
			       randoms all-all)
  ...
  (let* ((*add-cell-name-to-segs* t)
	...
	(let ((cell (hippo (format nil "Hippo-~d-~d" (round x)(round y)) :cell-origin (list x 0.0 y))))
    ...
    (loop for syn in (synapses-of-control 'voltage) do (setf
							(synapse-delay syn)
							(random 50.0)))
