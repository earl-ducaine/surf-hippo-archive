...
(defpackage :wh
  (:use :garnet-gadgets :cl-user :cl :kr)
  (:nicknames :windows :windows-hack))

(defpackage :ph
  (:use :wh :cl :cl-user :kr)
  (:nicknames :son-of-plot-hack :plot :plot-hack))

(defpackage :surf-hippo
  (:use :common-lisp-user :common-lisp :kr :wh :ph :lisp :user)
  (:nicknames :surf :sh))
...
(setq Surf-Hippo-Pathnames
  `(Surf-Hippo-garnet-fixes-PathName
    ...
    Surf-Hippo-Rabbit-PathName
    Surf-Hippo-Rabbit-Src
...
(defun update-surf-hippo-pathnames ()
  (setq
   ...
   Surf-Hippo-Rabbit-PathName (fixup-pathname (concatenate 'string Surf-Hippo-Pathname "/bin/rabbit/"))

   Surf-Hippo-Rabbit-Src (fixup-pathname (concatenate 'string Surf-Hippo-Pathname "/src/rabbit/"))
...
(defvar *abort-on-sim-error* t "When T SIM-ERROR aborts to top level; otherwise sim-error stays in debugger.")

(defun sim-error (&optional (message "") (abort-on-error *abort-on-sim-error*))
  "A fatal error occurred, print MESSAGE and abort if ABORT-ON-ERROR [default *ABORT-ON-SIM-ERROR*];
otherwise stay in debugger."
  ;;   (declare (ignore args))
  (inter:beep)  (inter:beep)  (inter:beep)
  (if abort-on-error
      (progn
...
(defparameter Surf-Hippo-Rabbit-Loader nil)
(setq Surf-Hippo-Rabbit-Loader (merge-pathnames "rabbit-loader"
						#+cmu "rabbit-src:" ; "rabbit:"
...
(loop for *enable-surf-hippo-compile* in '(nil t) do
      ...
      (loop for load-module-p in
	    (list load-garnet-fixes-p load-roylance-clmath-p load-gui-p load-sys-p load-parameters-p load-hippocampus-p
		  load-rabbit-p load-development-p)
	    for module-key in
	    (list :garnet-fixes :roylance-clmath :gui :sys :parameters :hippocampus :rabbit :development)
	    ...
	    (list Surf-Hippo-GARNET-FIXES-Loader Surf-Hippo-Roylance-Clmath-Loader Surf-Hippo-GUI-Loader
		  ...
		  Surf-Hippo-PARAMETERS-Loader Surf-Hippo-Hippocampus-Loader Surf-Hippo-Rabbit-Loader
