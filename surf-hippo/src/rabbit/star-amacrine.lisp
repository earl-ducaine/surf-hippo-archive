...
(defun star-amacrine (&optional (cell-name "starburst") &key (cell-origin '(0.0 0.0 0.0)) (segment-diameter 0.20) distal-channels soma-channels extras-list)
  (setq *light-stimulus-plane* :xy)
  (let* ((cell (create-cell cell-name :cell-type 'starburst :cell-origin cell-origin))
	 (soma (create-soma :cell cell :diameter 16.0)))
    (create-tree soma *star-amacrine-tree* :default-diameter segment-diameter :xy-factor 6.06 :global-extras-list extras-list)
    (setf (cell-type-notes (cell-type cell)) "Rabbit ACh starburst cell (cell 7) (~ 1.25 mm from streak), Tauchi and Masland (1984).")
    (create-element soma soma-channels)
    (create-element (distal-tips cell) distal-channels)
    cell))
