...
(defun print-states (nodes)
  (let* ( (last-node (car (last nodes)))
          (state nil))
    (dolist (n (cdr nodes))
      (when (p4::app-op-node-p n)
        (p4::maintain-state-and-goals last-node (p4::nexus-parent node))
        
        (setf state (true-literals last-node))
        (format t "~%~S~%     ~S" n state)
        
        (p4::maintain-state-and-goals (p4::nexus-parent node) last-node)
      ) ; when
    ) ; dolist
))
...
(defun k-cache-and-match (expr conditional instop partial-bindings type-decls)
  (let ((bound-preconds nil) (instantiated-preconds nil))
